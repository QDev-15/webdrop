    ...
    <section id="why" className="sec-pad position-relative" style={{ background: 'var(--dark2)' }}>
      <div className="wd-container">
        <div className="text-center sec-dark reveal mb-5">
          <div className="eyebrow">Tại sao chọn chúng tôi</div>
          <h2 className="sec-title">Không cần biết <em>kỹ thuật</em></h2>
          <p className="sec-sub">Chúng tôi lo toàn bộ. Bạn chỉ cần cung cấp nội dung và nhận website.</p>
        </div>
        <div className="rounded-3 overflow-hidden mb-4 reveal" style={{ height: 220, position: 'relative' }}>
          <img
            ...
            alt="Team làm việc" className="w-100 h-100 object-fit-cover d-block" loading="lazy"
          />
          ...
          <div style={{ position: 'absolute', bottom: 24, left: 32 }}>
            <div style={{ fontSize: 22, fontWeight: 600, color: '#fff', letterSpacing: '-.4px' }}>Đội ngũ chuyên nghiệp, tận tâm</div>
            <div style={{ fontSize: 14, color: 'rgba(255,255,255,.5)', marginTop: 4, fontWeight: 300 }}>Hỗ trợ trực tiếp qua Zalo — không chatbot</div>
          </div>
        </div>
        <div className="why-grid reveal">
          {items.map((item, i) => (
            <div key={item.num} className="why-item">
              {i < items.length - 1 && <div className="wi-sep" />}

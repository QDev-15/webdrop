...
export default function HowItWorks({ eyebrow, title, titleEm, subtitle, image, steps }: Props) {
  const stepsData = (steps && steps.length > 0) ? steps : DEFAULT_STEPS
  return (
    <section id="how" className="sec-pad" style={{ background: 'var(--surface)' }}>
      <div className="wd-container">
        <div className="text-center reveal mb-5">
          <div className="eyebrow">{eyebrow || 'Quy trình'}</div>
          <h2 className="sec-title">{title || 'Đơn giản từ'} <em>{titleEm || 'đầu đến cuối'}</em></h2>
          <p className="sec-sub">{subtitle || 'Bạn chỉ cần cung cấp nội dung. Chúng tôi lo toàn bộ phần kỹ thuật còn lại.'}</p>
        </div>
        <div className="row g-5 align-items-center">
          ...
          <div className="col-md-6 reveal reveal-d2">
            <div className="rounded-4 overflow-hidden position-relative border" style={{ borderColor: 'var(--border)', aspectRatio: '4/3' }}>
              <img src={image || 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?w=800&q=80&auto=format&fit=crop'} alt="Triển khai website" className="w-100 h-100 object-fit-cover d-block" loading="lazy" />

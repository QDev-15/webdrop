...
const packages = [
  {
    ...
    price: 'Từ 199.000đ',
    steps: [
      { n: 1, title: 'Chọn mẫu',          desc: 'Duyệt thư viện 30+ mẫu, xem demo live, chọn mẫu phù hợp ngành nghề.' },
      { n: 2, title: 'Thanh toán',         desc: 'Chuyển khoản — xác nhận trong 2 giờ làm việc.' },
      { n: 3, title: 'Nhận file ZIP',      desc: 'Download file HTML/CSS/JS + hướng dẫn chỉnh nội dung chi tiết.' },
      { n: 4, title: 'Tự chỉnh & triển khai', desc: 'Thay text, ảnh theo hướng dẫn. Upload lên bất kỳ hosting nào là chạy.' },
    ],
    suitable: ['Có kinh nghiệm kỹ thuật cơ bản', 'Muốn tự kiểm soát hoàn toàn', 'Ngân sách tối ưu'],
    cta: { label: 'Xem thư viện mẫu', href: '/templates' },
  },
  {
    id: 'goi-web-co-ban',
    name: 'Gói Web cơ bản',
    ...
    hot: true,
    steps: [
      { n: 1, title: 'Chọn mẫu & đặt hàng', desc: 'Chọn template từ thư viện, điền form brief ngắn (ngành, màu sắc, nội dung chính).' },
      { n: 2, title: 'Chúng tôi cài đặt',   desc: 'Setup hosting, domain, SSL. Điền nội dung theo brief. Thường hoàn thành trong 3–5 ngày.' },
      { n: 3, title: 'Duyệt & bàn giao',    desc: 'Review website, yêu cầu chỉnh sửa (tối đa 2 lần). Bàn giao quyền truy cập đầy đủ.' },
      { n: 4, title: 'Hỗ trợ 30 ngày',      desc: 'Hỗ trợ kỹ thuật miễn phí 30 ngày đầu qua Zalo. Sau đó có gói bảo trì tháng.' },
    ],
    suitable: ['Không rành kỹ thuật', 'Muốn website nhanh — 3 đến 5 ngày', 'Cần cài đặt trọn gói'],
    cta: { label: 'Đặt hàng ngay', href: '/templates' },
  },
  {
    id: 'goi-theo-yeu-cau',
    ...
    steps: [
      { n: 1, title: 'Trao đổi & brief',     desc: 'Cuộc gọi / Zalo 30 phút để hiểu rõ yêu cầu, ngành nghề, đối tượng khách hàng, ngân sách.' },
      { n: 2, title: 'Wireframe & ký scope', desc: 'Phác thảo cấu trúc trang, danh sách tính năng. Ký checklist scope tránh phát sinh.' },
      { n: 3, title: 'Design UI',            desc: 'Thiết kế giao diện trên Figma. Khách duyệt, chỉnh sửa đến khi ưng ý.' },
      { n: 4, title: 'Phát triển',           desc: 'Code frontend + backend theo thiết kế đã duyệt. Báo cáo tiến độ hàng tuần.' },
      { n: 5, title: 'Test & Deploy',        desc: 'Kiểm thử kỹ trên nhiều thiết bị. Deploy lên hosting, cấu hình domain, SSL.' },
      { n: 6, title: 'Bàn giao & hỗ trợ',   desc: 'Bàn giao source code (nếu chọn) hoặc bản build. Hỗ trợ 90 ngày sau bàn giao.' },
    ],
        ...
        <section style={{ background: 'var(--dark2)', padding: 'clamp(56px,8vw,88px) 0 clamp(40px,6vw,60px)', textAlign: 'center' }}>
          <div className="wd-container">
            ...
            <p style={{ fontSize: 15, fontWeight: 300, color: 'rgba(255,255,255,.45)', maxWidth: 500, margin: '0 auto 28px' }}>
              Ba gói dịch vụ — mỗi gói có quy trình rõ ràng, minh bạch từng bước.
            </p>
          ...
          <section key={pkg.id} id={pkg.id} className="sec-pad" style={{ background: pi % 2 === 1 ? 'var(--dark2)' : 'var(--bg)' }}>
            <div className="wd-container">
              ...
              <div className="reveal text-center mb-5">
                ...
                <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--accent)' }}>{pkg.price}</div>
              </div>

              {/* Steps */}
              <div className="row g-3 mb-5">
                {pkg.steps.map((s) => (
                  <div key={s.n} className="col-md-6 col-lg-4 reveal">
                    <div style={{

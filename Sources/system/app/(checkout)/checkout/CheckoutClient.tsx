...
const plans = [
  { id: 'starter', label: 'Starter', price: '1.200.000đ', desc: 'Source code + tài liệu hướng dẫn' },
  { id: 'standard', label: 'Standard', price: '2.500.000đ', desc: 'Cài đặt trọn gói · Hosting · Domain', hot: true },
  { id: 'premium', label: 'Premium', price: '12.000.000đ', desc: 'Thiết kế riêng theo yêu cầu' },
]
...
export default function CheckoutClient() {
  ...
  const [form, setForm] = useState<FormData>({ name: '', email: '', phone: '', note: '', plan: 'standard' })
  ...
  if (submitted) {
    return (
      <div className="checkout-body text-center" style={{ paddingTop: 80 }}>
        <div style={{ fontSize: 48, marginBottom: 16 }}>✅</div>
        <h2 style={{ fontSize: 24, fontWeight: 600, marginBottom: 8 }}>Đặt hàng thành công!</h2>
        <p style={{ color: 'var(--text-2)', fontWeight: 300, marginBottom: 24 }}>
          Chúng tôi sẽ liên hệ qua <strong>{form.email}</strong> hoặc <strong>{form.phone}</strong> trong vòng 2 giờ làm việc.
        </p>
        <a href="/" className="btn-primary-wd" style={{ display: 'inline-block', textDecoration: 'none', padding: '12px 28px' }}>
          Về trang chủ
        </a>
      </div>
    )
        ...
        <div className="row g-4">
                      ...
                      <label key={p.id} style={{ cursor: 'pointer' }}>
                        ...
                        <div style={{
                          ...
                          <div className="d-flex justify-content-between align-items-center">
                            ...
                            <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)', flexShrink: 0 }}>{p.price}</div>
                          </div>
                        </div>
                      </label>
                    ))}
            ...
            {step === 3 && (
              <div className="panel">
                ...
                <div className="panel-body">
                  ...
                  <button className="btn-primary-wd" style={{ width: '100%', padding: 13, fontSize: 14 }}
          ...
          <div className="col-lg-5">
            <div className="checkout-summary">
              ...
              <div className="sum-row"><span>Gói {selectedPlan.label}</span><span>{selectedPlan.price}</span></div>
              {form.plan === 'standard' && <div className="sum-row"><span>Hosting 1 năm</span><span style={{ color: 'var(--accent)' }}>Miễn phí</span></div>}
              <div className="sum-row total"><span>Tổng cộng</span><span>{selectedPlan.price}</span></div>

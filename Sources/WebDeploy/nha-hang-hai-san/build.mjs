...
let configOut = readFileSync(join(root, 'api', 'config.php'), 'utf8')
configOut = configOut.replace(/define\('APP_KEY',\s*'[^']*'\)/, `define('APP_KEY', '${appKey}')`)
if (appUrl) {
  configOut = configOut.replace(/define\('APP_URL',\s*'[^']*'\)/, `define('APP_URL', '${appUrl}')`)
  console.log(`  APP_URL: ${appUrl}`)
}
if (isLocal) {
  configOut = configOut.replace(/define\('APP_ENV',\s*'[^']*'\)/, `define('APP_ENV', 'development')`)

...
const empty: PricingForm = {
  ...
  is_featured: false, cta_text: 'Yêu cầu demo', cta_link: '/lien-he',
  ...
  useEffect(() => {
    ...
    api.get<{ id: number; name: string; price: string; description: string; features: string; is_featured: number; cta_text: string; cta_link: string; sort_order: number; status: string }>(`/pricing-plans/${id}`)
      .then(d => setForm({
        ...
        is_featured: d.is_featured === 1, cta_text: d.cta_text ?? 'Yêu cầu demo', cta_link: d.cta_link ?? '/lien-he',
    ...
    <div style={{ maxWidth: 640 }}>
      ...
      <form onSubmit={handleSubmit} className="card">
        <div className="form-group">
          ...
          <input type="text" className="form-control" value={form.name} onChange={e => set('name', e.target.value)} placeholder="Vd: Professional" required />
        </div>
        <div className="form-group">
          <label className="form-label">Giá *</label>
          <input type="text" className="form-control" value={form.price} onChange={e => set('price', e.target.value)} placeholder="Vd: [Từ X triệu]/tháng hoặc [Liên hệ]" required />
        </div>
        <div className="form-group">
        ...
        <div className="form-group">
          ...
          <textarea className="form-control" value={form.features} onChange={e => set('features', e.target.value)} placeholder={'1 giải pháp lõi\nSetup & onboarding trong 2 tuần\nHỗ trợ email trong giờ hành chính'} rows={6} />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="form-group">
            <label className="form-label">Chữ nút CTA</label>
            <input type="text" className="form-control" value={form.cta_text} onChange={e => set('cta_text', e.target.value)} placeholder="Yêu cầu demo" />
          </div>
          <div className="form-group">
        ...
        <div className="form-group">
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
            ...
            <span className="form-label" style={{ margin: 0 }}>Đánh dấu "Phổ biến nhất" (nổi bật)</span>
          </label>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="form-group">

  ...
  return (
    <div className="admin-page">
      ...
      <form onSubmit={handleSave} className="admin-form" style={{ marginTop: 24, maxWidth: 760 }}>
        ...
        {activeTab === 'general' && <>
          <Field label="Tên cửa hàng" name="site_name" value={val('site_name')} onChange={set} placeholder="Tươi Mỗi Ngày" />
          <Field label="Slogan" name="site_slogan" value={val('site_slogan')} onChange={set} placeholder="Thực phẩm sạch, tươi mỗi ngày" />
          ...
          <Field label="Giờ mở cửa" name="working_hours" value={val('working_hours')} onChange={set} placeholder="7:00 – 21:00 (Cả tuần)" />
        ...
        {activeTab === 'home' && <>
          ...
          <Field label="Tag (dòng badge phía trên tiêu đề)" name="hero_tag" value={val('hero_tag')} onChange={set} placeholder="100% Organic — Đạt chuẩn VietGAP" />
          <Field label="Ghi chú dưới nút CTA" name="hero_note" value={val('hero_note')} onChange={set} placeholder="Giao hàng lạnh trong 2–4 giờ tại nội thành · Hoàn tiền nếu không hài lòng" />
          <div className="form-row">
            ...
            <Field label="Nhãn" name="hero_float_label" value={val('hero_float_label')} onChange={set} placeholder="nông trại liên kết trên khắp Việt Nam" />
          </div>

          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Cam kết của chúng tôi (4 mục)</p>
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="form-row" style={{ marginBottom: 4 }}>
              <div className="form-group" style={{ flex: 1 }}>
                <label>Mục {i} — Bootstrap Icon (không có tiền tố "bi-")</label>
                <input type="text" value={val(`commit${i}_icon`)} onChange={e => set(`commit${i}_icon`, e.target.value)} placeholder="flower1" />
              </div>
          ...
          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Quy trình "Từ nông trại đến bàn ăn" (4 bước)</p>
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="form-row" style={{ marginBottom: 4 }}>
              <div className="form-group" style={{ flex: 1 }}>
          ...
          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Câu chuyện nông trại — Dòng 1</p>
          <Field label="Badge nhỏ" name="story1_badge" value={val('story1_badge')} onChange={set} placeholder="Nông trại liên kết" />
          <div className="form-row">
            <Field label="Tiêu đề — phần in đậm" name="story1_title_strong" value={val('story1_title_strong')} onChange={set} placeholder="120+ nông trại trên" />
            <Field label="Tiêu đề — phần cuối" name="story1_title_post" value={val('story1_title_post')} onChange={set} placeholder="khắp Việt Nam" />
          </div>
          ...
          <div className="form-row">
            ...
            <Field label="Gạch đầu dòng 3" name="story1_list3" value={val('story1_list3')} onChange={set} />
          </div>
          <Field label="Ảnh minh họa" name="story1_image" value={val('story1_image')} onChange={set} placeholder="https://..." />

          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Câu chuyện nông trại — Dòng 2</p>
          <Field label="Badge nhỏ" name="story2_badge" value={val('story2_badge')} onChange={set} placeholder="Cam kết chất lượng" />
          <div className="form-row">
            ...
            <Field label="Tiêu đề — phần cuối" name="story2_title_post" value={val('story2_title_post')} onChange={set} placeholder="không biến đổi gen" />
          </div>
          ...
          <div className="form-row">
            ...
            <Field label="Gạch đầu dòng 3" name="story2_list3" value={val('story2_list3')} onChange={set} />
          </div>
          ...
          <Field label="Tiêu đề" name="newsletter_title" value={val('newsletter_title')} onChange={set} placeholder="Nhận ưu đãi & tin nông sản mới" />
          <Field label="Mô tả" name="newsletter_desc" value={val('newsletter_desc')} onChange={set} placeholder="Đăng ký để nhận mã giảm giá 10.000đ cho đơn hàng đầu tiên." />
        </>}

        {activeTab === 'reviews' && <>
          <p style={{ color: 'var(--text-3)', fontSize: 13, marginBottom: 16 }}>4 đánh giá hiển thị ở section "Được tin dùng mỗi ngày" trên trang chủ.</p>
          {[1, 2, 3, 4].map(i => (
            <div key={i} style={{ padding: '12px 0', borderTop: '1px solid var(--border)', marginTop: 8 }}>
              ...
              <div className="form-row">
                ...
                <Field label="Địa điểm" name={`review${i}_location`} value={val(`review${i}_location`)} onChange={set} placeholder="Quận 2, TP.HCM" />
        ...
        {activeTab === 'seo' && <>
          <Field label="Meta Title" name="meta_title" value={val('meta_title')} onChange={set} placeholder="Tươi Mỗi Ngày — Thực Phẩm Sạch, Tươi Mỗi Ngày" />
        ...
        {activeTab === 'social' && <>
          <Field label="Facebook URL" name="facebook" value={val('facebook')} onChange={set} placeholder="https://www.facebook.com/tuoimoingay" />
          <Field label="Instagram URL" name="instagram" value={val('instagram')} onChange={set} placeholder="https://www.instagram.com/tuoimoingay" />
        ...
        {activeTab === 'contact' && <>
          <Field label="Email liên hệ" name="site_email" value={val('site_email')} onChange={set} type="email" placeholder="lienhe@tenshop.vn" />
          ...
          <Field label="Ghi chú giờ giao hàng" name="contact_delivery_note" value={val('contact_delivery_note')} onChange={set} placeholder="Giao hàng lạnh trong 2–4 giờ tại nội thành" />
        ...
        {activeTab === 'smtp' && <>
          ...
          <Field label="From Email" name="smtp_from" value={val('smtp_from')} onChange={set} placeholder="no-reply@tenshop.vn" />
          <Field label="From Name" name="smtp_from_name" value={val('smtp_from_name')} onChange={set} placeholder="Tươi Mỗi Ngày" />
        </>}
        ...
        {activeTab === 'cloudinary' && <>
          ...
          <Field label="Thư mục lưu ảnh" name="cloudinary_folder" value={val('cloudinary_folder')} onChange={set} placeholder="tuoi-moi-ngay" />
        </>}

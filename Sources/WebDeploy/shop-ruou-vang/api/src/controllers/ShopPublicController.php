...
class ShopPublicController {
    ...
    public function products(): void {
        ...
        // AI (shop-ruou-vang): catalog toolbar (index.html) lọc danh mục theo slug (vang-do, vang-trang...)
        // thay vì id — hỗ trợ thêm category_slugs song song với category_ids có sẵn.
        $categorySlugs = array_values(array_filter(array_map('trim', explode(',', $_GET['category_slugs'] ?? ''))));
        ...
        // AI (shop-ruou-vang): filter riêng cho rượu vang — xuất xứ / nồng độ cồn (bucket) / dịp dùng
        // (occasion lưu dạng CSV trong cột, mỗi giá trị match bằng LIKE '%value%').
        $origins   = array_values(array_filter(array_map('trim', explode(',', $_GET['origin'] ?? ''))));
        $abvBuckets = array_values(array_filter(array_map('trim', explode(',', $_GET['abv'] ?? ''))));
        $occasions  = array_values(array_filter(array_map('trim', explode(',', $_GET['occasion'] ?? ''))));
        ...
        if ($categorySlugs) {
            $ph = implode(',', array_fill(0, count($categorySlugs), '?'));
            $where[] = "p.category_id IN (SELECT id FROM product_categories WHERE slug IN ($ph))";
            array_push($params, ...$categorySlugs);
        }
        ...
        if ($origins) {
            ...
            $where[] = "p.origin IN ($ph)";
            array_push($params, ...$origins);
        }
        if ($occasions) {
            $clauses = [];
            foreach ($occasions as $o) { $clauses[] = "(',' || p.occasion || ',') LIKE ?"; $params[] = '%,' . $o . ',%'; }
            $where[] = '(' . implode(' OR ', $clauses) . ')';
        }
        if ($abvBuckets) {
            // Bucket khớp đúng logic abvBucket() phía client gốc: <12 | 12-13 | 13-14 | >=14
            $clauses = [];
            foreach ($abvBuckets as $b) {
                $clauses[] = match ($b) {
                    'duoi-12' => 'p.abv < 12',
                    '12-13'   => 'p.abv >= 12 AND p.abv < 13',
                    '13-14'   => 'p.abv >= 13 AND p.abv < 14',
                    'tren-14' => 'p.abv >= 14',
                    default   => '0',
                };
            }
    ...
    public function sitemap(): void {
        ...
        // Catalog sản phẩm nằm ngay ở trang chủ (Mode A — không có route /san-pham riêng), nên KHÔNG
        // liệt kê '/san-pham' ở đây — chỉ trang chi tiết mới có route '/san-pham/:slug' (xem bên dưới).
        $staticRoutes = ['/', '/bo-suu-tap', '/khuyen-mai', '/ve-chung-toi', '/lien-he', '/gio-hang', '/chinh-sach-bao-mat', '/dieu-khoan'];

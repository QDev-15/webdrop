...
class UserController {
    ...
    public function index(array $p): void {
        ...
        $items = $this->db->query("SELECT id, name, email, role, created_at FROM users ORDER BY id");
        Response::json($items);
    }
    ...
    public function store(array $p): void {
        ...
        if (empty($b['name']) || empty($b['email']) || empty($b['password'])) {
            Response::error('Tên, email và mật khẩu không được để trống.'); return;
        }
        $exists = $this->db->queryOne("SELECT id FROM users WHERE email=?", [$b['email']]);
        if ($exists) { Response::error('Email đã tồn tại.', 409); return; }
        $id = $this->db->execute(

...
export default function MenuCategoryList() {
  ...
  async function load() {
    try { setCats(await api.get<Category[]>('/product-categories')) }
  ...
  async function handleDelete(id: number) {
    if (!confirm('Xóa danh mục này? Các sản phẩm trong danh mục sẽ không bị xóa nhưng sẽ mất danh mục.')) return
          ...
          <form onSubmit={handleSave}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div className="form-group">
                ...
                <input type="text" className="form-control" value={form.name} onChange={e => set('name', e.target.value)} placeholder="Vd: Bánh Kem" required />
              </div>
              <div className="form-group">
          ...
          <tbody>
            {cats.map(cat => (
              <tr key={cat.id}>
                ...
                <td>{cat.item_count} sản phẩm</td>
                <td>{cat.sort_order}</td>
                <td><span className={`badge badge-${cat.status}`}>{cat.status === 'published' ? 'Hiện' : 'Ẩn'}</span></td>
                <td>

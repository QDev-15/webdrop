...
const empty: PricingForm = {
  name: '', price: '', unit: '/ tranh', description: '', features: '',
...
export default function PricingForm() {
  ...
  useEffect(() => {
    ...
    api.get<Record<string, unknown>>(`/pricing-plans/${id}`)
      .then(d => setForm({
        name: String(d.name ?? ''), price: String(d.price ?? ''), unit: String(d.unit ?? '/ tranh'),
    ...
    <div style={{ maxWidth: 640 }}>
      ...
      <form onSubmit={handleSubmit} className="card">
        <div className="form-group">
          ...
          <input type="text" className="form-control" value={form.name} onChange={e => set('name', e.target.value)} placeholder="Vd: Gói Bộ Nhân Vật" required />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
          <div className="form-group">
            <label className="form-label">Giá *</label>
            <input type="text" className="form-control" value={form.price} onChange={e => set('price', e.target.value)} placeholder="6tr – 12tr₫" required />
          </div>
          <div className="form-group">
            <label className="form-label">Đơn vị</label>
            <input type="text" className="form-control" value={form.unit} onChange={e => set('unit', e.target.value)} placeholder="/ bộ 3–5 nhân vật" />
          </div>
        </div>
        <div className="form-group">
          <label className="form-label">Mô tả ngắn</label>
          <textarea className="form-control" value={form.description} onChange={e => set('description', e.target.value)} placeholder="Phù hợp với nhu cầu nào" rows={2} />
        </div>
        <div className="form-group">
          <label className="form-label">Tính năng (mỗi dòng 1 mục)</label>
          <textarea className="form-control" value={form.features} onChange={e => set('features', e.target.value)} placeholder={'Character design 3–5 nhân vật\nCharacter sheet 3 góc nhìn + bảng biểu cảm'} rows={6} />
        </div>

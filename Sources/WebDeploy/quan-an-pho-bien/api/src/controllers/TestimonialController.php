...
class TestimonialController {
    ...
    public function store(array $p): void {
        ...
        $id = $this->db->execute(
            "INSERT INTO testimonials (author_name, author_title, author_avatar, content, rating, source, sort_order, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            [
                $b['author_name'],
                $b['author_title'] ?? '',
                $b['author_avatar'] ?? '',
    ...
    public function update(array $p): void {
        ...
        $this->db->execute(
            "UPDATE testimonials SET author_name=?, author_title=?, author_avatar=?, content=?, rating=?, source=?, sort_order=?, status=? WHERE id=?",
            [
                $b['author_name'],
                $b['author_title'] ?? '',
                $b['author_avatar'] ?? '',

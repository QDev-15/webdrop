    ...
    <div style={{ maxWidth: 640 }}>
      ...
      <form onSubmit={handleSubmit} className="card">
        <div className="form-group">
          ...
          <input type="text" className="form-control" value={form.title} onChange={e => set('title', e.target.value)} placeholder="Tiêu đề chính của slide (hỗ trợ HTML: &lt;br/&gt;, &lt;em&gt;)" required />
          <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 4 }}>Có thể dùng HTML: &lt;em&gt;chữ nghiêng&lt;/em&gt;, &lt;br/&gt; để xuống dòng</div>
        </div>
        <div className="form-group">
          <label className="form-label">Mô tả</label>
          <textarea className="form-control" value={form.subtitle} onChange={e => set('subtitle', e.target.value)} placeholder="Nội dung mô tả slide" rows={3} />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="form-group">
            <label className="form-label">Chữ nút</label>
            <input type="text" className="form-control" value={form.button_text} onChange={e => set('button_text', e.target.value)} placeholder="Vd: Xem thực đơn" />
          </div>
          <div className="form-group">
            <label className="form-label">Liên kết nút</label>
            <input type="text" className="form-control" value={form.button_link} onChange={e => set('button_link', e.target.value)} placeholder="/thuc-don" />
          </div>
        </div>
        <div className="form-group">
          <ImageField label="Ảnh slide (nền hero)" value={form.image} onChange={v => set('image', v)} />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="form-group">

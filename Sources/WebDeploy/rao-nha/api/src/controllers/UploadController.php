...
class UploadController {
    ...
    // Upload công khai (từ tài khoản người đăng tin, KHÔNG qua kiểm duyệt nội dung trước khi lưu file) —
    // KHÔNG cho phép SVG/icon: SVG có thể chứa <script>/event handler, là rủi ro XSS lưu trữ nếu ảnh tin
    // đăng bị mở trực tiếp ngoài thẻ <img>. Ảnh bất động sản cũng không có lý do chính đáng để là icon/SVG.
    private const ALLOWED_PUBLIC = ['image/jpeg','image/png','image/webp','image/gif'];
    ...
    public function upload(array $p): void {
        Auth::require();
        $this->handleUpload(self::ALLOWED);
    }

    /** Dùng bởi tài khoản người đăng tin (account công khai) để tải ảnh cho tin đăng — KHÔNG dùng session admin. */
    public function uploadPublic(array $p): void {
        AccountAuth::require();
        $this->handleUpload(self::ALLOWED_PUBLIC);
    }

    private function handleUpload(array $allowed): void {
        ...
        if (!in_array($mime, $allowed, true)) {
            Response::error('Chỉ hỗ trợ ảnh (JPG, PNG, WEBP, GIF).', 400); return;
        }

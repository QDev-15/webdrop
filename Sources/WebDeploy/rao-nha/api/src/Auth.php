...
class Auth {
    ...
    public static function start(): void {
        if (session_status() === PHP_SESSION_NONE) {
            // Sessions stored next to DB — nằm trong docroot nên PHẢI có .htaccess riêng chặn truy cập
            // trực tiếp (không được chỉ trông cậy vào rule chặn "api/database/" ở .htaccess gốc — file
            // session không có phần mở rộng nên có thể lọt qua rule chặn theo extension trên 1 số cấu hình host).
            ...
            session_name('RaoNha');  // unique per site — must be alphanumeric only (PHP 8 rejects hyphens)
            session_start();
        }
    }

            ...
            <div key={p.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 15 }}>
                  {p.name} {!!p.is_featured && <span className="badge badge-new" style={{ marginLeft: 8 }}>Phổ biến nhất</span>}
                </div>
                <div style={{ fontSize: 13, color: 'var(--text-2)' }}>{p.tag} — {p.price} {p.unit}</div>
              </div>

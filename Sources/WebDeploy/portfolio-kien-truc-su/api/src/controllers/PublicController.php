...
class PublicController {
    ...
    public function featuredProjects(array $p): void {
        $items = $this->db->query("SELECT * FROM projects WHERE status='published' AND featured=1 ORDER BY sort_order, id LIMIT 3");
        Response::json($items);
    }
    ...
    public function submitContact(array $p): void {
        ...
        // Gộp loại nhu cầu vào message vì bảng contacts core chỉ có
        // name/email/phone/subject/message/status (không mở rộng cột).
        $needType = trim($b['need_type'] ?? '');
        $fullMessage = trim(($needType ? "Loại nhu cầu: $needType\n\n" : '') . $message);

        $this->db->execute(
            "INSERT INTO contacts (name, email, phone, subject, message, status) VALUES (?, ?, ?, ?, ?, 'new')",
            [$name, trim($b['email'] ?? ''), $phone, 'Yêu cầu tư vấn thiết kế kiến trúc', $fullMessage]
        );
        Response::json(['ok' => true, 'message' => 'Cảm ơn bạn đã gửi thông tin! Chúng tôi sẽ phản hồi trong vòng 24 giờ.'], 201);
    }

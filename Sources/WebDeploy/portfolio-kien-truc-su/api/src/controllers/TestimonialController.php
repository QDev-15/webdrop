...
class TestimonialController {
    ...
    public function store(array $p): void {
        Auth::require();
        $b = bodyJson();
        if (empty($b['author_name'])) { Response::error('Tên khách hàng không được để trống.'); return; }
        $id = $this->db->execute(
            "INSERT INTO testimonials (author_name, author_title, author_avatar, content, rating, sort_order, status) VALUES (?,?,?,?,?,?,?)",
            [
                $b['author_name'], $b['author_title'] ?? '', $b['author_avatar'] ?? '', $b['content'] ?? '',
                (int)($b['rating'] ?? 5), (int)($b['sort_order'] ?? 0), $b['status'] ?? 'published',
            ]
        );
    ...
    public function update(array $p): void {
        ...
        $this->db->execute(
            ...
            [
                $b['author_name'], $b['author_title'] ?? '', $b['author_avatar'] ?? '', $b['content'] ?? '',
                (int)($b['rating'] ?? 5), (int)($b['sort_order'] ?? 0), $b['status'] ?? 'published', $p['id'],
            ]
        );

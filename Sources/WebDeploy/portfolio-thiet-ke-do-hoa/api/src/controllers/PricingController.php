...
class PricingController {
    ...
    public function store(array $p): void {
        ...
        $id = $this->db->execute(
            "INSERT INTO pricing_plans (name, price, note, features, is_featured, badge_text, cta_text, cta_link, sort_order, status) VALUES (?,?,?,?,?,?,?,?,?,?)",
            [
                $b['name'], $b['price'], $b['note'] ?? '', $b['features'] ?? '',
                (int)($b['is_featured'] ?? 0), $b['badge_text'] ?? 'Được chọn nhiều nhất',
                $b['cta_text'] ?? 'Chọn gói này', $b['cta_link'] ?? '/lien-he',
    ...
    public function update(array $p): void {
        ...
        if (empty($b['name']) || empty($b['price'])) { Response::error('Tên gói và giá không được để trống.'); return; }
        $this->db->execute(
            "UPDATE pricing_plans SET name=?, price=?, note=?, features=?, is_featured=?, badge_text=?, cta_text=?, cta_link=?, sort_order=?, status=? WHERE id=?",
            [
                $b['name'], $b['price'], $b['note'] ?? '', $b['features'] ?? '',
                (int)($b['is_featured'] ?? 0), $b['badge_text'] ?? 'Được chọn nhiều nhất',
                $b['cta_text'] ?? 'Chọn gói này', $b['cta_link'] ?? '/lien-he',

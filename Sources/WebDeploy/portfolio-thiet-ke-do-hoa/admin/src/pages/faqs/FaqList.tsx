    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Hiển thị trang Dịch vụ ({items.length} câu hỏi)</div>
        </div>
          ...
          {items.map(f => (
            <div key={f.id} className="card" style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 4 }}>{f.question}</div>
                <div style={{ fontSize: 13, color: 'var(--text-2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.answer}</div>
              </div>

...
export default function FaqForm() {
  ...
  useEffect(() => {
    ...
    api.get<Record<string, unknown>>(`/faqs/${id}`)
      .then(d => setForm({
        question: String(d.question ?? ''), answer: String(d.answer ?? ''),
        page: String(d.page ?? 'dich-vu'), sort_order: Number(d.sort_order ?? 0), status: String(d.status ?? 'published'),
      }))
    ...
    <div style={{ maxWidth: 640 }}>
      ...
      <form onSubmit={handleSubmit} className="card">
        ...
        <div className="form-group">
          ...
          <textarea id="answer" className="form-control" rows={4} value={form.answer} onChange={e => set('answer', e.target.value)} required />
        </div>

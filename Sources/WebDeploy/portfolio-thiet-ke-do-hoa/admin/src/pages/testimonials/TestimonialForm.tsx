  ...
  useEffect(() => {
    ...
    api.get<Record<string, unknown>>(`/testimonials/${id}`)
      .then(d => setForm({
        ...
        author_avatar: String(d.author_avatar ?? ''), content: String(d.content ?? ''),
        rating: Number(d.rating ?? 5), sort_order: Number(d.sort_order ?? 0), status: String(d.status ?? 'published'),
      }))
      ...
      <form onSubmit={handleSubmit} className="card">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="form-group">
            ...
            <input id="author_name" type="text" className="form-control" value={form.author_name} onChange={e => set('author_name', e.target.value)} required />
          </div>
          <div className="form-group">
            <label className="form-label" htmlFor="author_title">Vai trò / bối cảnh</label>
            <input id="author_title" type="text" className="form-control" value={form.author_title} onChange={e => set('author_title', e.target.value)} placeholder="Sáng lập, Lụa Atelier" />
          </div>
        </div>
        <div className="form-group">

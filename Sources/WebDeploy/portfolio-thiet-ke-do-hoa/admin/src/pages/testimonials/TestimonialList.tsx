    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Hiển thị ở teaser trang chủ &amp; grid đầy đủ trang Về tôi ({items.length} đánh giá)</div>
        </div>
            ...
            <div key={t.id} className="card" style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
              {t.author_avatar && (
                <img src={t.author_avatar} alt={t.author_name}
                  style={{ width: 56, height: 56, objectFit: 'cover', borderRadius: '50%', flexShrink: 0, border: '1px solid var(--border)' }} />
              )}
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 4 }}>{t.author_name} <span style={{ fontWeight: 400, color: 'var(--text-3)' }}>— {t.author_title}</span></div>
                <div style={{ fontSize: 13, color: 'var(--text-2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.content}</div>
              </div>

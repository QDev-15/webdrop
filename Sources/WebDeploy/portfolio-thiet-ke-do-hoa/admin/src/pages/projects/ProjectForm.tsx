...
const empty: FormData = {
  title: '', slug: '', category: '', image: '', year: '', client_name: '',
  has_case_study: false,
  industry: '', duration: '', services_provided: '', key_result: '',
  challenge_title: '', challenge_details: '',
  solution_heading: '', solution_items: '', solution_image: '',
  gallery_images: '', results_items: '',
  ...
  useEffect(() => {
    ...
    api.get<Record<string, unknown>>(`/projects/${id}`)
      .then(d => setForm({
        ...
        image: String(d.image ?? ''), year: String(d.year ?? ''), client_name: String(d.client_name ?? ''),
        ...
        challenge_title: String(d.challenge_title ?? ''), challenge_details: String(d.challenge_details ?? ''),
        solution_heading: String(d.solution_heading ?? ''), solution_items: String(d.solution_items ?? ''), solution_image: String(d.solution_image ?? ''),
        gallery_images: String(d.gallery_images ?? ''), results_items: String(d.results_items ?? ''),
    ...
    <div style={{ maxWidth: 780 }}>
      ...
      <form onSubmit={handleSubmit} className="card">
        ...
        <div className="form-group">
          <label className="form-label" htmlFor="title">Tiêu đề * (vd: Mộc Coffee)</label>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="form-group">
            ...
            <input id="slug" type="text" className="form-control" value={form.slug} onChange={e => set('slug', e.target.value)} placeholder="moc-coffee" />
          </div>
          <div className="form-group">
            <label className="form-label" htmlFor="category">Danh mục</label>
            <input id="category" type="text" className="form-control" value={form.category} onChange={e => set('category', e.target.value)} placeholder="Brand + Packaging" />
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="form-group">
            <label className="form-label" htmlFor="client_name">Khách hàng</label>
            <input id="client_name" type="text" className="form-control" value={form.client_name} onChange={e => set('client_name', e.target.value)} placeholder="Mộc Coffee" />
          </div>
          <div className="form-group">
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div className="form-group">
                ...
                <input id="industry" type="text" className="form-control" value={form.industry} onChange={e => set('industry', e.target.value)} placeholder="F&B — Cà phê rang xay" />
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="duration">Thời gian thực hiện</label>
                <input id="duration" type="text" className="form-control" value={form.duration} onChange={e => set('duration', e.target.value)} placeholder="6 tuần" />
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="services_provided">Dịch vụ cung cấp</label>
                <input id="services_provided" type="text" className="form-control" value={form.services_provided} onChange={e => set('services_provided', e.target.value)} placeholder="Logo, Brand Guideline, Packaging" />
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="key_result">Kết quả chính</label>
                <input id="key_result" type="text" className="form-control" value={form.key_result} onChange={e => set('key_result', e.target.value)} placeholder="+42% nhận diện tại điểm bán" />
              </div>
            </div>
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="challenge_title">Tiêu đề (xuống dòng bằng Enter, dùng *từ* để in nghiêng màu nhấn)</label>
              <textarea id="challenge_title" className="form-control" rows={2} value={form.challenge_title} onChange={e => set('challenge_title', e.target.value)} placeholder={'Sản phẩm ngon\nnhưng *chìm* trên kệ.'} />
            </div>
            <div className="form-group">
              <label className="form-label" htmlFor="challenge_details">Nội dung — mỗi đoạn cách nhau 1 dòng trống</label>
              <textarea id="challenge_details" className="form-control" rows={5} value={form.challenge_details} onChange={e => set('challenge_details', e.target.value)} />
            </div>
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="solution_heading">Tiêu đề mục giải pháp (dùng *từ* để in nghiêng)</label>
              <textarea id="solution_heading" className="form-control" rows={2} value={form.solution_heading} onChange={e => set('solution_heading', e.target.value)} placeholder={'4 bước xây dựng\nhệ thống *nhận diện.*'} />
            </div>
            <div className="form-group">
              <label className="form-label" htmlFor="solution_items">Mỗi dòng: Tiêu đề bước|Mô tả</label>
              <textarea id="solution_items" className="form-control" rows={6} value={form.solution_items} onChange={e => set('solution_items', e.target.value)}
                placeholder={'Audit & Research|Khảo sát 15 thương hiệu...'} style={{ fontFamily: 'monospace', fontSize: 12.5 }} />
            </div>
            <div className="form-group">
              <ImageField label="Ảnh minh hoạ giải pháp" value={form.solution_image} onChange={v => set('solution_image', v)} />
            </div>

            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '.5px', margin: '16px 0 10px' }}>Gallery thành phẩm</div>
            <div className="form-group">
              <label className="form-label" htmlFor="gallery_images">Mỗi dòng: URL ảnh|Mô tả ảnh</label>
              <textarea id="gallery_images" className="form-control" rows={5} value={form.gallery_images} onChange={e => set('gallery_images', e.target.value)}
                placeholder={'https://...jpg|Bao bì túi cà phê Robusta'} style={{ fontFamily: 'monospace', fontSize: 12.5 }} />
            </div>
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="results_items">Mỗi dòng: giá trị|hậu tố|nhãn</label>
              <textarea id="results_items" className="form-control" rows={4} value={form.results_items} onChange={e => set('results_items', e.target.value)}
                placeholder={'42|%|Tăng nhận diện tại điểm bán'} style={{ fontFamily: 'monospace', fontSize: 12.5 }} />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div className="form-group">
                ...
                <input id="testimonial_role" type="text" className="form-control" value={form.testimonial_role} onChange={e => set('testimonial_role', e.target.value)} placeholder="Sáng lập, Mộc Coffee" />
              </div>
            </div>
            <div className="form-group">
              <ImageField label="Ảnh đại diện khách hàng" value={form.testimonial_avatar} onChange={v => set('testimonial_avatar', v)} />
            </div>

            <div className="form-group">
              <label className="form-label" htmlFor="cta_title">Tiêu đề CTA cuối trang (xuống dòng bằng Enter)</label>
              <textarea id="cta_title" className="form-control" rows={2} value={form.cta_title} onChange={e => set('cta_title', e.target.value)} placeholder={'Có sản phẩm cần\nbao bì ấn tượng hơn?'} />
            </div>
          </>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border-light)' }}>
          ...
          <div className="form-group">
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', marginTop: 28 }}>
              ...
              <span className="form-label" style={{ margin: 0 }}>Dự án nổi bật (bento trang chủ, tối đa 4)</span>
            </label>
          </div>
        </div>

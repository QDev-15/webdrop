    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Danh sách dự án ({items.length} dự án) — bật "Case Study" để có trang chi tiết đầy đủ</div>
        </div>
          ...
          {items.map(p => (
            <div key={p.id} className="card" style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 13, color: 'var(--text-2)' }}>{p.category} · {p.year}</div>
                <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 4, display: 'flex', gap: 8, alignItems: 'center' }}>
                  Thứ tự: {p.sort_order}
                  {!!p.featured && <span className="badge badge-published">Nổi bật (bento)</span>}

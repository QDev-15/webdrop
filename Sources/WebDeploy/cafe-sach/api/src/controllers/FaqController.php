...
class FaqController {
    ...
    public function store(array $p): void {
        ...
        if (empty($b['question']) || empty($b['answer'])) {
            Response::error('Câu hỏi và câu trả lời là bắt buộc.'); return;
        }
        $id = $this->db->execute(
            "INSERT INTO faqs (question, answer, sort_order, status) VALUES (?, ?, ?, ?)",
            [$b['question'], $b['answer'], (int)($b['sort_order'] ?? 0), $b['status'] ?? 'published']
        );
    ...
    public function update(array $p): void {
        ...
        $this->db->execute(
            ...
            [$b['question'], $b['answer'], (int)($b['sort_order'] ?? 0), $b['status'] ?? 'published', $p['id']]
        );

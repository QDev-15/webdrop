...
class PublicController {
    ...
    public function products(): void {
        ...
        $sizes       = array_values(array_filter(array_map('trim', explode(',', $_GET['sizes'] ?? ''))));
        ...
        if ($search !== '') {
            $where[] = '(p.name LIKE ? OR p.brand LIKE ? OR p.material LIKE ? OR p.description LIKE ?)';
        ...
        if ($sizes) {
            $clauses = [];
            foreach ($sizes as $s) { $clauses[] = "('|' || p.sizes || '|') LIKE ?"; $params[] = '%|' . $s . '|%'; }
            $where[] = '(' . implode(' OR ', $clauses) . ')';
        }
    ...
    public function productReviews(array $params): void {
        $slug    = $params['slug'] ?? '';
        $product = $this->db->queryOne("SELECT id FROM products WHERE slug = ?", [$slug]);
        if (!$product) { Response::error('Không tìm thấy sản phẩm', 404); return; }
        $rows = $this->db->query(
            "SELECT id, author_name, rating, variant_note, review_date, content
             FROM product_reviews WHERE product_id = ? ORDER BY sort_order ASC, created_at DESC",
            [$product['id']]
        );
        Response::json($rows);
    }
    ...
    private function lookupCoupon(string $code, int $subtotal): array {
        // Trả về ['ok'=>bool, 'error'=>?string, 'coupon'=>?array, 'discount'=>int]
    ...
    public function createOrder(): void {
        ...
        foreach ($items as $it) {
            ...
            $orderItems[] = [
                'product_id' => $pid, 'product_name' => $product['name'], 'color' => $color, 'size' => $size,
                'price' => $price, 'qty' => $qty, 'subtotal' => $lineSubtotal,
            ];
        }
        ...
        foreach ($orderItems as $it) {
            $this->db->execute(
                "INSERT INTO order_items (order_id, product_id, product_name, color, size, price, qty, subtotal) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                [$orderId, $it['product_id'], $it['product_name'], $it['color'], $it['size'], $it['price'], $it['qty'], $it['subtotal']]
            );
        }
    ...
    public function submitContact(): void {
        ...
        if (!$name || !$phone || !$message) {
            Response::error('Vui lòng điền họ tên, số điện thoại và nội dung tin nhắn', 422);
            return;
        }
        ...
        Response::json(['message' => 'Gửi tin nhắn thành công! Chúng tôi sẽ phản hồi trong vòng 24 giờ.']);
    }
    ...
    public function sitemap(): void {
        ...
        $staticRoutes = ['/', '/san-pham', '/bo-suu-tap', '/ve-chung-toi', '/lien-he', '/chinh-sach-bao-mat', '/dieu-khoan'];

                ...
                <tr key={p.id}>
                  <td>
                    {p.image ? (
                      ...
                      <div style={{ width: 48, height: 48, background: '#f0f0f0', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🛍</div>
                    )}
                  </td>
                  <td>
                    <strong>{p.name}</strong>
                    <div style={{ fontSize: 11, color: '#aaa' }}>
                      {p.is_featured ? '⭐ Nổi bật ' : ''}{p.is_new ? '🆕 Mới ' : ''}{p.badge ? `· ${p.badge}` : ''}
                    </div>
                  </td>
                  <td>{p.category_name ?? '—'}</td>
                  <td>{fmt(p.price)}</td>
                  <td style={{ color: p.price_sale ? '#0052ff' : undefined }}>{fmt(p.price_sale)}</td>
                  <td>
                    <span className={`status-badge ${p.in_stock ? 'done' : 'brief'}`}>

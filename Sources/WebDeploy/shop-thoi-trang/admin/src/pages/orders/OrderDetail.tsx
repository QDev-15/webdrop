      ...
      <div className="form-row" style={{ alignItems: 'flex-start', gap: 24 }}>
        <div style={{ flex: 2 }}>
          <div className="admin-table-wrap">
            <table className="admin-table">
              <thead>
                <tr><th>Sản phẩm</th><th>Biến thể</th><th>Đơn giá</th><th>SL</th><th>Tạm tính</th></tr>
              </thead>
              <tbody>
                {order.items.map(it => (
                  <tr key={it.id}>
          ...
          <div style={{ marginTop: 16, padding: 20, background: 'var(--bg)', borderRadius: 12 }}>
            ...
            {order.discount > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span>Giảm giá {order.coupon_code ? `(${order.coupon_code})` : ''}</span><span>-{fmt(order.discount)}</span>
              </div>
            )}
            <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 600, fontSize: 16, borderTop: '1px solid var(--border)', paddingTop: 8 }}><span>Tổng cộng</span><span>{fmt(order.total)}</span></div>
          </div>
        </div>
        ...
        <div style={{ flex: 1 }}>
          <div className="admin-form" style={{ marginBottom: 16 }}>
            ...
            {order.note && <p style={{ marginTop: 8, color: 'var(--text-3)' }}>Ghi chú: {order.note}</p>}
          </div>

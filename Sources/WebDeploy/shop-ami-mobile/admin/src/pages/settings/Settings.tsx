import { useEffect, useState } from 'react'
import { api } from '../../api/client'
import PaymentSettingsTab from '../../components/PaymentSettingsTab'

type SettingsMap = Record<string, string>

const TABS = [
  { id: 'general', label: 'Thông tin chung' },
  { id: 'home', label: 'Trang chủ' },
  { id: 'about', label: 'Về chúng tôi' },
  { id: 'promotions', label: 'Khuyến mãi' },
  { id: 'seo', label: 'SEO' },
  { id: 'social', label: 'Mạng xã hội' },
  { id: 'footer', label: 'Footer' },
  { id: 'shop', label: 'Cửa hàng' },
  { id: 'payment', label: '💳 Thanh toán' },
  { id: 'contact', label: 'Liên hệ' },
  { id: 'smtp', label: 'SMTP' },
  { id: 'system', label: 'Nâng cao' },
  { id: 'cloudinary', label: '☁️ Cloudinary' },
  { id: 'integrations', label: '🔌 Tích hợp' },
]

function Field({ label, name, value, onChange, type = 'text', placeholder = '' }: {
  label: string; name: string; value: string; onChange: (k: string, v: string) => void
  type?: string; placeholder?: string
}) {
  return (
    <div className="form-group">
      <label>{label}</label>
      {type === 'textarea' ? (
        <textarea rows={3} value={value} onChange={e => onChange(name, e.target.value)} placeholder={placeholder} />
      ) : (
        <input type={type} value={value} onChange={e => onChange(name, e.target.value)} placeholder={placeholder} />
      )}
    </div>
  )
}

function ToggleField({ label, name, value, onChange }: {
  label: string; name: string; value: string; onChange: (k: string, v: string) => void
}) {
  return (
    <label className="form-check">
      <input type="checkbox" checked={value === '1'} onChange={e => onChange(name, e.target.checked ? '1' : '0')} />
      <span>{label}</span>
    </label>
  )
}

export default function Settings() {
  const [settings, setSettings] = useState<SettingsMap>({})
  const [activeTab, setActiveTab] = useState('general')
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [saved, setSaved] = useState(false)
  const [error, setError] = useState('')

  useEffect(() => {
    api.get<SettingsMap>('/settings')
      .then(setSettings)
      .catch(() => {})
      .finally(() => setLoading(false))
  }, [])

  const set = (k: string, v: string) => setSettings(s => ({ ...s, [k]: v }))
  const val = (k: string) => settings[k] ?? ''

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault()
    setSaving(true)
    setError('')
    try {
      await api.post('/settings/update', settings)
      setSaved(true)
      setTimeout(() => setSaved(false), 3000)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Lưu thất bại, vui lòng thử lại')
    } finally {
      setSaving(false)
    }
  }

  if (loading) return <div className="admin-loading-box">Đang tải...</div>

  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <h1 className="admin-page-title">Cài đặt hệ thống</h1>
      </div>

      <div className="settings-tabs">
        {TABS.map(t => (
          <button key={t.id} className={'settings-tab' + (activeTab === t.id ? ' active' : '')} onClick={() => setActiveTab(t.id)}>
            {t.label}
          </button>
        ))}
      </div>

      <form onSubmit={handleSave} className="admin-form" style={{ marginTop: 24, maxWidth: 760 }}>
        {saved && <div className="form-success-banner">Đã lưu cài đặt thành công!</div>}
        {error && <div className="form-error-banner">{error}</div>}

        {activeTab === 'general' && <>
          <Field label="Tên cửa hàng" name="site_name" value={val('site_name')} onChange={set} placeholder="AMI Mobile" />
          <Field label="Slogan" name="site_slogan" value={val('site_slogan')} onChange={set} placeholder="Điện Thoại & Phụ Kiện Chính Hãng" />
          <Field label="Mô tả ngắn" name="site_description" value={val('site_description')} onChange={set} type="textarea" />
          <Field label="Logo URL" name="site_logo" value={val('site_logo')} onChange={set} placeholder="https://..." />
          <Field label="Favicon URL" name="site_favicon" value={val('site_favicon')} onChange={set} placeholder="https://..." />
          <Field label="Giờ mở cửa" name="working_hours" value={val('working_hours')} onChange={set} placeholder="Thứ 2 – Chủ nhật: 8:00 – 21:00" />
          <Field label="Số Zalo (không dấu cách)" name="zalo_number" value={val('zalo_number')} onChange={set} placeholder="0900000000" />
          <Field label="Bảo hành mặc định (tháng)" name="warranty_months" value={val('warranty_months')} onChange={set} placeholder="12" />
        </>}

        {activeTab === 'home' && <>
          <p style={{ color: 'var(--text-3)', fontSize: 13, marginBottom: 16 }}>
            Nội dung banner trang chủ (Intro Banner) — không dùng ảnh, chỉ có chữ lớn + dòng chạy (ticker).
          </p>
          <Field label="Nhãn nhỏ (stamp)" name="hero_stamp" value={val('hero_stamp')} onChange={set} placeholder="AMI Mobile 2025" />
          <div className="form-row">
            <Field label="Tiêu đề — dòng 1" name="hero_title_line1" value={val('hero_title_line1')} onChange={set} placeholder="ĐIỆN THOẠI" />
            <Field label="Tiêu đề — dòng 2 (nhấn mạnh)" name="hero_title_line2" value={val('hero_title_line2')} onChange={set} placeholder="CHẤT." />
            <Field label="Tiêu đề — dòng 3" name="hero_title_line3" value={val('hero_title_line3')} onChange={set} placeholder="GIÁ THẬT." />
          </div>
          <Field label="Mô tả" name="hero_desc" value={val('hero_desc')} onChange={set} type="textarea" />
          <div className="form-row">
            <Field label="Nút CTA 1" name="hero_cta_text" value={val('hero_cta_text')} onChange={set} placeholder="Mua ngay" />
            <Field label="Nút CTA 2" name="hero_cta2_text" value={val('hero_cta2_text')} onChange={set} placeholder="Xem khuyến mãi" />
          </div>

          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Dòng chạy (ticker) — 6 mục</p>
          {[1, 2, 3, 4, 5, 6].map(i => (
            <Field key={i} label={`Mục ${i}`} name={`ticker_${i}`} value={val(`ticker_${i}`)} onChange={set} />
          ))}

          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Thanh thống kê trang chủ (4 mục)</p>
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="form-row" style={{ marginBottom: 4 }}>
              <div className="form-group" style={{ flex: 1 }}>
                <label>Số {i}</label>
                <input type="number" value={val(`hstat${i}_num`)} onChange={e => set(`hstat${i}_num`, e.target.value)} />
              </div>
              <div className="form-group" style={{ flex: 1 }}>
                <label>Hậu tố</label>
                <input type="text" value={val(`hstat${i}_suffix`)} onChange={e => set(`hstat${i}_suffix`, e.target.value)} placeholder="+" />
              </div>
              <div className="form-group" style={{ flex: 2 }}>
                <label>Nhãn</label>
                <input type="text" value={val(`hstat${i}_label`)} onChange={e => set(`hstat${i}_label`, e.target.value)} />
              </div>
            </div>
          ))}
        </>}

        {activeTab === 'about' && <>
          <Field label="Ảnh minh họa" name="about_img" value={val('about_img')} onChange={set} placeholder="https://..." />
          <Field label="Đoạn giới thiệu 1" name="about_lead1" value={val('about_lead1')} onChange={set} type="textarea" />
          <Field label="Đoạn giới thiệu 2" name="about_lead2" value={val('about_lead2')} onChange={set} type="textarea" />

          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Thanh thống kê trang Về chúng tôi (4 mục)</p>
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="form-row" style={{ marginBottom: 4 }}>
              <div className="form-group" style={{ flex: 1 }}>
                <label>Số {i}</label>
                <input type="number" value={val(`astat${i}_num`)} onChange={e => set(`astat${i}_num`, e.target.value)} />
              </div>
              <div className="form-group" style={{ flex: 1 }}>
                <label>Hậu tố</label>
                <input type="text" value={val(`astat${i}_suffix`)} onChange={e => set(`astat${i}_suffix`, e.target.value)} placeholder="+" />
              </div>
              <div className="form-group" style={{ flex: 2 }}>
                <label>Nhãn</label>
                <input type="text" value={val(`astat${i}_label`)} onChange={e => set(`astat${i}_label`, e.target.value)} />
              </div>
            </div>
          ))}

          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Giá trị cốt lõi (3 mục)</p>
          {[1, 2, 3].map(i => (
            <div key={i} className="form-row" style={{ marginBottom: 4 }}>
              <div className="form-group" style={{ flex: 1 }}>
                <label>Icon (emoji)</label>
                <input type="text" value={val(`value${i}_icon`)} onChange={e => set(`value${i}_icon`, e.target.value)} placeholder="🏷️" />
              </div>
              <div className="form-group" style={{ flex: 2 }}>
                <label>Tiêu đề</label>
                <input type="text" value={val(`value${i}_title`)} onChange={e => set(`value${i}_title`, e.target.value)} />
              </div>
              <div className="form-group" style={{ flex: 3 }}>
                <label>Mô tả</label>
                <input type="text" value={val(`value${i}_desc`)} onChange={e => set(`value${i}_desc`, e.target.value)} />
              </div>
            </div>
          ))}

          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Đội ngũ (4 thành viên)</p>
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="form-row" style={{ marginBottom: 4 }}>
              <div className="form-group" style={{ flex: 2 }}>
                <label>Tên</label>
                <input type="text" value={val(`team${i}_name`)} onChange={e => set(`team${i}_name`, e.target.value)} />
              </div>
              <div className="form-group" style={{ flex: 2 }}>
                <label>Chức danh</label>
                <input type="text" value={val(`team${i}_role`)} onChange={e => set(`team${i}_role`, e.target.value)} />
              </div>
            </div>
          ))}
        </>}

        {activeTab === 'promotions' && <>
          <Field label="Phần trăm giảm (Flash Sale)" name="promo_percent" value={val('promo_percent')} onChange={set} placeholder="30" />
          <Field label="Mô tả chương trình" name="promo_desc" value={val('promo_desc')} onChange={set} type="textarea" />

          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Ưu đãi thêm khi mua (3 mục)</p>
          {[1, 2, 3].map(i => (
            <div key={i} className="form-row" style={{ marginBottom: 4 }}>
              <div className="form-group" style={{ flex: 1 }}>
                <label>Icon (emoji)</label>
                <input type="text" value={val(`promo_card${i}_icon`)} onChange={e => set(`promo_card${i}_icon`, e.target.value)} placeholder="📦" />
              </div>
              <div className="form-group" style={{ flex: 2 }}>
                <label>Tiêu đề</label>
                <input type="text" value={val(`promo_card${i}_title`)} onChange={e => set(`promo_card${i}_title`, e.target.value)} />
              </div>
              <div className="form-group" style={{ flex: 3 }}>
                <label>Mô tả</label>
                <input type="text" value={val(`promo_card${i}_desc`)} onChange={e => set(`promo_card${i}_desc`, e.target.value)} />
              </div>
            </div>
          ))}
        </>}

        {activeTab === 'seo' && <>
          <Field label="Meta Title" name="meta_title" value={val('meta_title')} onChange={set} placeholder="AMI Mobile — Điện Thoại & Phụ Kiện Chính Hãng" />
          <Field label="Meta Description" name="meta_description" value={val('meta_description')} onChange={set} type="textarea" />
          <Field label="OG Image URL" name="og_image" value={val('og_image')} onChange={set} placeholder="https://..." />
          <Field label="Google Analytics ID" name="ga_id" value={val('ga_id')} onChange={set} placeholder="G-XXXXXXXXXX" />
          <Field label="Google Tag Manager ID" name="gtm_id" value={val('gtm_id')} onChange={set} placeholder="GTM-XXXXXXX" />
        </>}

        {activeTab === 'social' && <>
          <Field label="Facebook URL" name="facebook" value={val('facebook')} onChange={set} placeholder="https://www.facebook.com/..." />
          <Field label="Instagram URL" name="instagram" value={val('instagram')} onChange={set} placeholder="https://www.instagram.com/..." />
          <Field label="YouTube URL" name="youtube" value={val('youtube')} onChange={set} placeholder="https://www.youtube.com/@..." />
          <Field label="TikTok URL" name="tiktok" value={val('tiktok')} onChange={set} placeholder="https://www.tiktok.com/@..." />
        </>}

        {activeTab === 'footer' && <>
          <Field label="Mô tả footer" name="footer_about" value={val('footer_about')} onChange={set} type="textarea" />
        </>}

        {activeTab === 'shop' && <>
          <Field label="Số ngày đổi trả" name="return_days" value={val('return_days')} onChange={set} placeholder="7" />
          <p style={{ color: 'var(--text-3)', fontSize: 12, margin: '4px 0 16px' }}>Phí vận chuyển / Miễn phí vận chuyển từ — cấu hình tại tab "💳 Thanh toán".</p>
        </>}

        {activeTab === 'payment' && <PaymentSettingsTab val={val} set={set} />}

        {activeTab === 'contact' && <>
          <Field label="Email liên hệ" name="site_email" value={val('site_email')} onChange={set} type="email" placeholder="hello@amimobile.vn" />
          <Field label="Số điện thoại" name="site_phone" value={val('site_phone')} onChange={set} placeholder="0900 000 000" />
          <Field label="Địa chỉ" name="site_address" value={val('site_address')} onChange={set} placeholder="123 Đường ABC, Quận 1, TP.HCM" />
          <Field label="Giới thiệu ngắn (trang Liên hệ)" name="contact_intro" value={val('contact_intro')} onChange={set} type="textarea" />
        </>}

        {activeTab === 'smtp' && <>
          <Field label="SMTP Host" name="smtp_host" value={val('smtp_host')} onChange={set} placeholder="smtp.gmail.com" />
          <Field label="SMTP Port" name="smtp_port" value={val('smtp_port')} onChange={set} placeholder="587" />
          <Field label="SMTP Username" name="smtp_user" value={val('smtp_user')} onChange={set} placeholder="you@gmail.com" />
          <Field label="SMTP Password" name="smtp_pass" value={val('smtp_pass')} onChange={set} type="password" placeholder="••••••••" />
          <Field label="From Email" name="smtp_from" value={val('smtp_from')} onChange={set} placeholder="no-reply@amimobile.vn" />
          <Field label="From Name" name="smtp_from_name" value={val('smtp_from_name')} onChange={set} placeholder="AMI Mobile" />
        </>}

        {activeTab === 'system' && <>
          <ToggleField label="Bật chế độ bảo trì (ẩn website, chỉ admin truy cập được)" name="maintenance_mode" value={val('maintenance_mode')} onChange={set} />
        </>}

        {activeTab === 'cloudinary' && <>
          <p style={{ color: 'var(--text-3)', fontSize: 13, marginBottom: 16 }}>Cấu hình Cloudinary để upload và quản lý ảnh sản phẩm.</p>
          <Field label="Cloud Name" name="cloudinary_cloud_name" value={val('cloudinary_cloud_name')} onChange={set} placeholder="mycloud" />
          <Field label="API Key" name="cloudinary_api_key" value={val('cloudinary_api_key')} onChange={set} placeholder="123456789012345" />
          <Field label="API Secret" name="cloudinary_api_secret" value={val('cloudinary_api_secret')} onChange={set} type="password" placeholder="••••••••" />
          <Field label="Thư mục lưu ảnh" name="cloudinary_folder" value={val('cloudinary_folder')} onChange={set} placeholder="ami-mobile" />
        </>}

        {activeTab === 'integrations' && <>
          <p style={{ color: 'var(--text-3)', fontSize: 13, marginBottom: 16 }}>API keys cho các tích hợp bên ngoài.</p>
          <Field label="Unsplash Access Key" name="unsplash_access_key" value={val('unsplash_access_key')} onChange={set} placeholder="Unsplash Access Key" />
          <Field label="Facebook Pixel ID" name="fb_pixel_id" value={val('fb_pixel_id')} onChange={set} placeholder="123456789012345" />
        </>}

        <div className="form-actions" style={{ marginTop: 32 }}>
          <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Đang lưu...' : 'Lưu cài đặt'}</button>
        </div>
      </form>
    </div>
  )
}

...
const COUPON_STORAGE_KEY = 'mb_coupon'
...
const Ctx = createContext<CartCtx>({
  ...
  applyCoupon: async () => ({ ok: false }), clearCoupon: () => {}, clear: () => {},
})
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  // couponDiscount chỉ lưu trong bộ nhớ — về 0 khi refresh trang; CartPage tự re-validate nếu cần
  ...
  const applyCoupon = async (code: string): Promise<{ ok: boolean; error?: string }> => {
    try {
      const result = await api.post<{ code: string; type: string; discount: number }>(
        '/public/coupons/validate',
        { code: code.trim().toUpperCase(), subtotal }
      )
      setCouponCodeState(result.code)
      setCouponDiscount(result.discount)
      try { localStorage.setItem(COUPON_STORAGE_KEY, result.code) } catch { /* ignore */ }
      return { ok: true }
    } catch (err) {
  ...
  return (
    <Ctx.Provider value={{ items, count, subtotal, couponCode, couponDiscount, addItem, updateQty, removeItem, applyCoupon, clearCoupon, clear }}>
      {children}
    </Ctx.Provider>
  )
}

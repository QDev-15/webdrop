...
class ShopPublicController {
    ...
    public function products(): void {
        ...
        // AI (shop-thu-cung): filter riêng cho thú cưng — loại thú cưng / kích cỡ / thương hiệu, khớp
        // đúng toolbar filter thật của index.html (dropdown checkbox Loại thú cưng/Kích cỡ/Thương hiệu).
        $petTypes    = array_values(array_filter(array_map('trim', explode(',', $_GET['pet_type'] ?? ''))));
        $sizes       = array_values(array_filter(array_map('trim', explode(',', $_GET['size'] ?? ''))));
        $brands      = array_values(array_filter(array_map('trim', explode(',', $_GET['brand'] ?? ''))));
        ...
        if ($petTypes) {
            $ph = implode(',', array_fill(0, count($petTypes), '?'));
            $where[] = "p.pet_type IN ($ph)";
            array_push($params, ...$petTypes);
        }
        if ($sizes) {
            // p.size lưu CSV "S,M,L" — match từng size riêng lẻ bằng LIKE để không nhầm "S" khớp "XS"
            $clauses = [];
            foreach ($sizes as $sz) { $clauses[] = "(',' || p.size || ',') LIKE ?"; $params[] = '%,' . $sz . ',%'; }
            $where[] = '(' . implode(' OR ', $clauses) . ')';
        }
        if ($brands) {

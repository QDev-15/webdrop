...
CREATE TABLE IF NOT EXISTS products (
    ...
    -- ▼ Cột riêng shop-thu-cung (thú cưng) — khớp UI thực trong template gốc (index.html toolbar filter)
    pet_type     TEXT NOT NULL DEFAULT '',      -- cho|meo|ca-hai
    brand        TEXT NOT NULL DEFAULT '',      -- PawFresh|PetKing|MeowMart|...
    size         TEXT NOT NULL DEFAULT '',      -- csv: S,M,L (rỗng nếu sản phẩm không phân loại size)
    sold         INTEGER NOT NULL DEFAULT 0,    -- số lượng đã bán (dùng cho sort "Bán chạy")
    gallery      TEXT NOT NULL DEFAULT ''       -- pipe-separated URL ảnh (trang chi tiết sản phẩm)
);
...
-- ═══════════════════════════════════════════════════════════════════════════
-- shop-thu-cung EXTENSION — đánh giá khách hàng hiển thị ở trang chủ (LIST-ELEGANT,
-- index.html mục "Khách hàng nói gì"). Không có bảng coupons vì khuyen-mai.html gốc
-- không có voucher-card, chỉ có stat-bar + lưới sản phẩm giảm giá.
-- ═══════════════════════════════════════════════════════════════════════════

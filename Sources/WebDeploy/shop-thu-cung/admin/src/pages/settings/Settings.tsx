  ...
  return (
    <div className="admin-page">
      ...
      <form onSubmit={handleSave} className="admin-form" style={{ marginTop: 24, maxWidth: 760 }}>
        ...
        {activeTab === 'general' && <>
          <Field label="Tên cửa hàng" name="site_name" value={val('site_name')} onChange={set} placeholder="Pet Haus" />
          <Field label="Slogan" name="site_tagline" value={val('site_tagline')} onChange={set} placeholder="Cửa hàng thú cưng chính hãng cho chó & mèo" />
          <Field label="Số điện thoại" name="site_phone" value={val('site_phone')} onChange={set} placeholder="1900 636 963" />
          <Field label="Email" name="site_email" value={val('site_email')} onChange={set} type="email" placeholder="hello@pethaus.vn" />
          <Field label="Địa chỉ cửa hàng" name="site_address" value={val('site_address')} onChange={set} placeholder="52 Nguyễn Văn Trỗi, Phường 15, Quận Phú Nhuận, TP.HCM" />
          <Field label="Giờ mở cửa" name="working_hours" value={val('working_hours')} onChange={set} placeholder="Thứ 2 – Chủ nhật: 8:00 – 20:00" />
        ...
        {activeTab === 'seo' && <>
          <Field label="Meta Title" name="meta_title" value={val('meta_title')} onChange={set} placeholder="Pet Haus — Cửa hàng thú cưng chính hãng cho chó & mèo" />
        ...
        {activeTab === 'smtp' && <>
          ...
          <Field label="From Email" name="smtp_from" value={val('smtp_from')} onChange={set} placeholder="no-reply@pethaus.vn" />
          <Field label="From Name" name="smtp_from_name" value={val('smtp_from_name')} onChange={set} placeholder="Pet Haus" />
        </>}
        ...
        {activeTab === 'cloudinary' && <>
          ...
          <Field label="Thư mục lưu ảnh" name="cloudinary_folder" value={val('cloudinary_folder')} onChange={set} placeholder="shop-thu-cung" />
        </>}

...
// Template gốc không có bộ lọc "Màu sắc" (thú cưng không có biến thể màu) — thay bằng
// Loại thú cưng/Thương hiệu/Kích cỡ, khớp đúng toolbar filter thật của index.html.
const PET_TYPE_OPTIONS = [
  { value: 'cho', label: 'Chó' },
  { value: 'meo', label: 'Mèo' },
  { value: 'ca-hai', label: 'Chó & Mèo' },
]

const BRAND_OPTIONS = ['PawFresh', 'PetKing', 'MeowMart', "Buddy's Choice", 'VetCare Pro', 'FurNest', 'PurePaw', 'Happy Tail']
...
const EMPTY: FormData = {
  ...
  badge: '', description: '', pet_type: 'cho', brand: BRAND_OPTIONS[0], size: '', sold: '0', gallery: '',
...
export default function ProductForm() {
    ...
    Promise.all([fetchCats, fetchProduct]).then(([cats, p]) => {
      ...
      if (p) {
        setForm({
          ...
          pet_type: String(p.pet_type ?? 'cho'),
          brand: String(p.brand ?? BRAND_OPTIONS[0]),
          size: String(p.size ?? ''),
  ...
  const selectedSizes = parseCsv(form.size)
  const toggleSize = (value: string) => {
    const next = selectedSizes.includes(value) ? selectedSizes.filter(s => s !== value) : [...selectedSizes, value]
    set('size', next.join(','))
  }
      ...
      <form onSubmit={handleSubmit} className="admin-form">
        ...
        <div className="form-row">
          <div className="form-group" style={{ flex: 2 }}>
            ...
            <input type="text" className="form-control" value={form.name} onChange={e => set('name', e.target.value)} placeholder="VD: Hạt khô cho chó trưởng thành vị gà 3kg" />
          </div>
        ...
        <div className="form-group">
          <label>Ảnh gallery (trang chi tiết — cách nhau dấu |)</label>
          <input type="text" className="form-control" value={form.gallery} onChange={e => set('gallery', e.target.value)} placeholder="https://.../a.jpg|https://.../b.jpg" />
        </div>

        <div className="form-row">
          <div className="form-group" style={{ flex: 1 }}>
            <label>Giá (VND)</label>
            <input type="number" className="form-control" value={form.price} onChange={e => set('price', e.target.value)} min={0} placeholder="VD: 285000" />
          </div>
          <div className="form-group" style={{ flex: 1 }}>
            <label>Giá sale (VND)</label>
            <input type="number" className="form-control" value={form.price_sale} onChange={e => set('price_sale', e.target.value)} min={0} placeholder="VD: 249000" />
          </div>
          <div className="form-group" style={{ flex: 1 }}>
            <label>Badge</label>
        ...
        <div className="form-group">
          <label>Kích cỡ có sẵn (dùng cho bộ lọc "Kích cỡ" ở trang chủ — để trống nếu không phân loại size)</label>

...
const PET_TYPE_LABEL: Record<string, string> = { cho: 'Chó', meo: 'Mèo', 'ca-hai': 'Chó & Mèo' }
    ...
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          ...
          <p className="admin-page-sub">Quản lý danh sách sản phẩm thú cưng của cửa hàng</p>
        </div>
      ...
      {loading ? <div className="admin-loading-box">Đang tải...</div> : (
        <div className="admin-table-wrap">
          <table className="admin-table">
            ...
            <tbody>
              {products.length === 0 ? (
                <tr><td colSpan={9} style={{ textAlign: 'center', color: '#aaa' }}>
                ...
                <tr key={p.id}>
                  <td>
                    {p.image ? (
                      ...
                      <div style={{ width: 48, height: 48, background: '#f3ede4', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🐾</div>
                    )}
                  </td>
                  <td>
                  ...
                  <td>{PET_TYPE_LABEL[p.pet_type] ?? p.pet_type ?? '—'}</td>
                  <td>{p.brand ?? '—'}</td>

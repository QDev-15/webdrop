...
class MenuItemController {
    ...
    public function store(array $p): void {
        ...
        $id = $this->db->execute(
            "INSERT INTO menu_items (category_id, name, name_fr, description, price, price_sale, image, badge, allergens, featured, sort_order, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
                $b['category_id'] ?? null,
                $b['name'],
                $b['name_fr'] ?? '',
    ...
    public function update(array $p): void {
        ...
        if (empty($b['name'])) { Response::error('Tên món ăn là bắt buộc.'); return; }
        $this->db->execute(
            "UPDATE menu_items SET category_id=?, name=?, name_fr=?, description=?, price=?, price_sale=?, image=?, badge=?, allergens=?, featured=?, sort_order=?, status=? WHERE id=?",
            [
                $b['category_id'] ?? null,
                $b['name'],
                $b['name_fr'] ?? '',

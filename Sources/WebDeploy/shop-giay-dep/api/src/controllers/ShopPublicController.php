...
class ShopPublicController {
    ...
    public function products(): void {
        ...
        // AI: products có thêm cột "sizes" (pipe-separated) — lọc theo size được chọn ở sidebar (rule 36).
        ...
        if ($sizes) {
            $clauses = [];
            foreach ($sizes as $sVal) { $clauses[] = "('|' || p.sizes || '|') LIKE ?"; $params[] = '%|' . $sVal . '|%'; }
            $where[] = '(' . implode(' OR ', $clauses) . ')';
        }
    ...
    // ── Coupon extension (site-specific — bảng `coupons` riêng, độc lập với hệ discount
    //    của System DB webdrop.store, xem rule 39 web-deploy-builder.md) ──
    ...
    public function sitemap(): void {
        ...
        // Route tĩnh theo website/src/App.tsx — không gồm /gio-hang, /thanh-toan (trang giao dịch, không có
        // nội dung riêng để index).
        $staticRoutes = [
            '/', '/san-pham', '/bo-suu-tap', '/ve-chung-toi', '/lien-he',

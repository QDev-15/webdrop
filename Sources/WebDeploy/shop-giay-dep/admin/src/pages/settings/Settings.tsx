  ...
  return (
    <div className="admin-page">
      ...
      <form onSubmit={handleSave} className="admin-form" style={{ marginTop: 24, maxWidth: 760 }}>
        ...
        {activeTab === 'general' && <>
          <Field label="Tên cửa hàng" name="site_name" value={val('site_name')} onChange={set} placeholder="Volt Kicks" />
          <Field label="Slogan" name="site_slogan" value={val('site_slogan')} onChange={set} placeholder="Giày Dép Chính Hãng, Phong Cách Đường Phố" />
          ...
          <Field label="Giờ mở cửa" name="working_hours" value={val('working_hours')} onChange={set} placeholder="9:00 – 21:00 · Tất cả các ngày" />
          <Field label="Số Zalo" name="zalo_number" value={val('zalo_number')} onChange={set} placeholder="0900123456" />
        </>}
        ...
        {activeTab === 'home' && <>
          ...
          <Field label="Tag (dòng nhỏ phía trên tiêu đề)" name="hero_tag" value={val('hero_tag')} onChange={set} placeholder="Bộ sưu tập Thu Đông 2025 vừa ra mắt" />
          <div className="form-row">
            <Field label="Chỉ số 1 — Số" name="hero_stat1_num" value={val('hero_stat1_num')} onChange={set} placeholder="120" />
            <Field label="Hậu tố" name="hero_stat1_suffix" value={val('hero_stat1_suffix')} onChange={set} placeholder="+" />
            <Field label="Nhãn" name="hero_stat1_label" value={val('hero_stat1_label')} onChange={set} placeholder="Mẫu giày" />
          </div>
          <div className="form-row">
            <Field label="Chỉ số 2 — Số" name="hero_stat2_num" value={val('hero_stat2_num')} onChange={set} placeholder="18000" />
            <Field label="Hậu tố" name="hero_stat2_suffix" value={val('hero_stat2_suffix')} onChange={set} placeholder="+" />
            <Field label="Nhãn" name="hero_stat2_label" value={val('hero_stat2_label')} onChange={set} placeholder="Khách hàng" />
          </div>
          <div className="form-row">
            <Field label="Chỉ số 3 — Số" name="hero_stat3_num" value={val('hero_stat3_num')} onChange={set} placeholder="4" />
            <Field label="Hậu tố" name="hero_stat3_suffix" value={val('hero_stat3_suffix')} onChange={set} placeholder=".9★" />
            <Field label="Nhãn" name="hero_stat3_label" value={val('hero_stat3_label')} onChange={set} placeholder="Đánh giá trung bình" />
          </div>

          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Vì sao chọn chúng tôi (4 mục)</p>
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="form-row" style={{ marginBottom: 4 }}>
              <div className="form-group" style={{ flex: 1 }}>
                <label>Mục {i} — Bootstrap Icon (không có tiền tố "bi-")</label>
                <input type="text" value={val(`value${i}_icon`)} onChange={e => set(`value${i}_icon`, e.target.value)} placeholder="patch-check-fill" />
              </div>
          ...
          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Banner khuyến mãi (Flash sale)</p>
          <div className="form-row">
            <Field label="Nhãn" name="promo_label" value={val('promo_label')} onChange={set} placeholder="Ưu đãi có giới hạn" />
            <Field label="Tiêu đề" name="promo_title" value={val('promo_title')} onChange={set} placeholder="Giảm đến" />
            <Field label="Phần trăm" name="promo_percent" value={val('promo_percent')} onChange={set} placeholder="35%" />
          </div>
          ...
          <Field label="Tiêu đề" name="newsletter_title" value={val('newsletter_title')} onChange={set} placeholder="Đăng ký nhận tin khuyến mãi" />
          <Field label="Mô tả" name="newsletter_desc" value={val('newsletter_desc')} onChange={set} placeholder="Nhận thông báo ưu đãi độc quyền..." />
        </>}

        {activeTab === 'reviews' && <>
          <p style={{ color: 'var(--text-3)', fontSize: 13, marginBottom: 16 }}>4 đánh giá hiển thị ở section "Khách hàng nói gì" trên trang chủ.</p>
          <div className="form-row">
            <Field label="Điểm trung bình" name="reviews_avg" value={val('reviews_avg')} onChange={set} placeholder="4.9" />
            <Field label="Tổng số đánh giá" name="reviews_count" value={val('reviews_count')} onChange={set} placeholder="3.200+" />
          </div>
          {[1, 2, 3, 4].map(i => (
            <div key={i} style={{ padding: '12px 0', borderTop: '1px solid var(--border)', marginTop: 8 }}>
              ...
              <div className="form-row">
                ...
                <Field label="Địa điểm / Thời gian" name={`review${i}_location`} value={val(`review${i}_location`)} onChange={set} placeholder="TP.HCM · Tháng 6/2025" />
        ...
        {activeTab === 'seo' && <>
          <Field label="Meta Title" name="meta_title" value={val('meta_title')} onChange={set} placeholder="Volt Kicks — Giày Dép Chính Hãng, Phong Cách Đường Phố" />
        ...
        {activeTab === 'social' && <>
          <Field label="Facebook URL" name="facebook" value={val('facebook')} onChange={set} placeholder="https://www.facebook.com/voltkicks" />
          <Field label="Instagram URL" name="instagram" value={val('instagram')} onChange={set} placeholder="https://www.instagram.com/voltkicks" />
          <Field label="TikTok URL" name="tiktok" value={val('tiktok')} onChange={set} placeholder="https://www.tiktok.com/@voltkicks" />
          <Field label="Zalo URL" name="zalo" value={val('zalo')} onChange={set} placeholder="https://zalo.me/0900123456" />
        </>}
        ...
        {activeTab === 'shop' && <>
          <Field label="Số ngày đổi trả" name="return_days" value={val('return_days')} onChange={set} placeholder="15" />
        ...
        {activeTab === 'contact' && <>
          <Field label="Email liên hệ" name="site_email" value={val('site_email')} onChange={set} type="email" placeholder="hello@voltkicks.vn" />
          <Field label="Số điện thoại" name="site_phone" value={val('site_phone')} onChange={set} placeholder="0900 123 456" />
          <Field label="Địa chỉ" name="site_address" value={val('site_address')} onChange={set} placeholder="88 Nguyễn Trãi, Quận 1, TP.HCM" />
        ...
        {activeTab === 'smtp' && <>
          ...
          <Field label="From Email" name="smtp_from" value={val('smtp_from')} onChange={set} placeholder="no-reply@voltkicks.vn" />
          <Field label="From Name" name="smtp_from_name" value={val('smtp_from_name')} onChange={set} placeholder="Volt Kicks" />
        </>}
        ...
        {activeTab === 'cloudinary' && <>
          ...
          <Field label="Thư mục lưu ảnh" name="cloudinary_folder" value={val('cloudinary_folder')} onChange={set} placeholder="volt-kicks" />
        </>}

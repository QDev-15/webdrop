...
export default function Settings() {
  ...
  const tabContent: Record<string, React.ReactNode> = {
    general: <>{f('Tên website','site_name')}{f('Tagline','site_tagline')}{f('Số điện thoại','site_phone')}{f('Email','site_email','email')}{ta('Địa chỉ','site_address',2)}{f('Giờ mở cửa','working_hours')}</>,
    ...
    hero: (
      <>
        <div style={{ color: 'var(--text-3)', fontSize: 12, marginBottom: 12 }}>Nội dung phần hero (tiêu đề lớn trang chủ)</div>
        {f('Badge text','hero_badge')}
        {f('Dòng tiêu đề 1','hero_title1')}
        {f('Dòng tiêu đề 2 (in đậm)','hero_title2')}
    ...
    stats: (
      <>
        <div style={{ color: 'var(--text-3)', fontSize: 12, marginBottom: 12 }}>Các con số thống kê hiển thị trong hero và stats bar</div>
        {f('Số khách hàng','stat_customers')}
        {f('Năm kinh nghiệm','stat_years')}
        {f('Mẫu nail độc quyền','stat_patterns')}
        {f('Đánh giá trung bình','stat_rating')}
      </>
    ),
    about: (
      <>
        {f('Tiêu đề section "Vì sao chọn chúng tôi"','about_title')}
        {ta('Mô tả','about_sub',3)}
    ...
    contact: (
      <>
        ...
        {f('Phương thức thanh toán','payment_methods')}
      </>
    ),

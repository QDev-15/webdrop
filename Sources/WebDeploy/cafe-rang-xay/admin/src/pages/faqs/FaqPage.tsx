...
const emptyForm: ItemForm = { question: '', answer: '', show_home: 0, sort_order: 0, status: 'published' }
...
export default function FaqPage() {
  ...
  async function load() {
    try { setItems(await api.get<Item[]>('/faqs')) }
  ...
  function openEdit(item: Item) {
    setEditing(item)
    setForm({ question: item.question, answer: item.answer ?? '', show_home: item.show_home ?? 0, sort_order: item.sort_order ?? 0, status: item.status ?? 'published' })
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Trang Giới thiệu hiện TẤT CẢ · trang chủ chỉ hiện câu đánh dấu "Hiện ở trang chủ" ({items.length} câu)</div>
        </div>
          ...
          <form onSubmit={handleSave}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, alignItems: 'end' }}>
              <div className="form-group" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <input type="checkbox" id="show_home" checked={form.show_home === 1} onChange={e => set('show_home', e.target.checked ? 1 : 0)} style={{ width: 16, height: 16 }} />
                <label htmlFor="show_home" className="form-label" style={{ margin: 0, cursor: 'pointer' }}>Hiện ở trang chủ</label>
              </div>
              <div className="form-group">
            ...
            <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', justifyContent: 'space-between' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
                  {item.question}
                  {item.show_home === 1 && <span style={{ fontSize: 11, background: 'var(--accent-light)', color: 'var(--accent)', padding: '2px 8px', borderRadius: 20 }}>Trang chủ</span>}
                </div>
                <div style={{ fontSize: 13, color: 'var(--text-2)', marginTop: 6, lineHeight: 1.6 }}>{item.answer}</div>
              </div>

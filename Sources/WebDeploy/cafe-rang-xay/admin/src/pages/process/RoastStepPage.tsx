...
const emptyForm: ItemForm = { icon: '🔥', name: '', description: '', sort_order: 0, status: 'published' }

export default function RoastStepPage() {
  ...
  async function load() {
    try { setItems(await api.get<Item[]>('/roast-steps')) }
  ...
  function openEdit(item: Item) {
    setEditing(item)
    setForm({ icon: item.icon ?? '🔥', name: item.name, description: item.description ?? '', sort_order: item.sort_order ?? 0, status: item.status ?? 'published' })
  ...
  async function handleSave(e: React.FormEvent) {
    e.preventDefault()
    if (!form.name.trim()) { setError('Tên bước là bắt buộc.'); return }
    setSaving(true); setError('')
    try {
      if (editing) { await api.put(`/roast-steps/${editing.id}`, form) }
      else { await api.post('/roast-steps', form) }
  ...
  async function handleDelete(id: number) {
    ...
    await api.delete(`/roast-steps/${id}`)
    load()
  }
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <div className="page-title">Quy trình chọn hạt</div>
          <div className="page-sub">Hiển thị ở trang Giới thiệu — mục "Quy trình chọn hạt" ({items.length} bước)</div>
        </div>
        <button onClick={openNew} className="btn-accent">+ Thêm bước</button>
      </div>
      ...
      {items.length === 0 && <div className="empty-state"><div className="empty-state-icon">🔥</div><div className="empty-state-text">Chưa có bước nào.</div></div>}
    </div>
  )
}

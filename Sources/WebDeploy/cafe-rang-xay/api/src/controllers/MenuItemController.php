...
class MenuItemController {
    ...
    public function store(array $p): void {
        ...
        $id = $this->db->execute(
            "INSERT INTO menu_items (category_id, name, slug, description, price, price_sale, image, badge, allergens, featured, sort_order, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
                $b['category_id'] ?? null,
                $b['name'],
                $slug,
    ...
    public function update(array $p): void {
        ...
        if (empty($b['name'])) { Response::error('Ten mon an la bat buoc.'); return; }
        $this->db->execute(
            "UPDATE menu_items SET category_id=?, name=?, description=?, price=?, price_sale=?, image=?, badge=?, allergens=?, featured=?, sort_order=?, status=? WHERE id=?",
            [
                $b['category_id'] ?? null,
                $b['name'],
                $b['description'] ?? '',

  ...
  useEffect(() => {
    const io = new IntersectionObserver(
      entries => entries.forEach(e => {
        if (e.isIntersecting) {
          e.target.classList.add('chd-visible')
    ...
    const observeNew = (root: ParentNode = document) => {
      root.querySelectorAll<Element>('[data-reveal]:not(.chd-visible)').forEach(el => io.observe(el))
    }
    ...
    const mo = new MutationObserver(mutations => {
      mutations.forEach(m => {
        m.addedNodes.forEach(node => {
          ...
          if (node.hasAttribute('data-reveal') && !node.classList.contains('chd-visible')) {
            io.observe(node)
          }
          node.querySelectorAll<Element>('[data-reveal]:not(.chd-visible)').forEach(el => io.observe(el))
        })
      })
    })

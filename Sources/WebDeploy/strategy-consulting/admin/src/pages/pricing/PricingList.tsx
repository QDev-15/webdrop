...
export default function PricingList() {
  ...
  async function handleDelete(id: number) {
    if (!confirm('Xóa hình thức hợp tác này?')) return
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <div className="page-title">Hình thức hợp tác</div>
          <div className="page-sub">Hiển thị ở trang Dịch vụ — thay cho bảng giá cố định ({items.length} mục)</div>
        </div>
        <Link to="/pricing/new" className="btn-accent">+ Thêm hình thức</Link>
      </div>

      {items.length === 0 ? (
        <div className="empty-state">
          <div className="empty-state-icon">💼</div>
          <div className="empty-state-text">Chưa có hình thức hợp tác nào. Thêm mục đầu tiên!</div>
        </div>
      ) : (

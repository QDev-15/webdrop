        ...
        <div className="card">
          ...
          {activeTab === 'general' && (
            <div>
              ...
              <div className="form-group">
                <label className="form-label">Tên logo (hiển thị trên nav &amp; footer)</label>
                <input className="form-control" value={s.site_logo_text ?? ''} onChange={e => set('site_logo_text', e.target.value)} placeholder="Strategy" />
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input className="form-control" value={s.site_address ?? ''} onChange={e => set('site_address', e.target.value)} />
              </div>
              <div className="form-group">
                <label className="form-label">Giờ làm việc (mỗi dòng 1 khung giờ)</label>
                <textarea className="form-control" rows={2} value={s.working_hours ?? ''} onChange={e => set('working_hours', e.target.value)} placeholder={'Thứ Hai - Thứ Sáu: 8:00 - 18:00\nThứ Bảy: 9:00 - 12:00'} />
              </div>
            </div>
          )}
          ...
          {activeTab === 'social' && (
            <div>
              ...
              <p style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 20 }}>Template chỉ dùng nút chat Zalo nổi (góc dưới bên phải) — chưa hiển thị icon mạng xã hội khác.</p>
              <div className="form-group">
                <label className="form-label">Số Zalo (chỉ số, dùng cho zalo.me/...)</label>
                <input className="form-control" value={s.zalo ?? ''} onChange={e => set('zalo', e.target.value)} placeholder="0908765432" />
              </div>
            </div>
          )}
          ...
          {activeTab === 'stats' && (
            <div>
              <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 20 }}>Thống kê trang chủ (dải số liệu)</h3>
              {[1, 2, 3, 4].map(i => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 2fr', gap: 12, marginBottom: 12 }}>
                  ...
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    ...
                    <input className="form-control" value={s[`stat${i}_suffix`] ?? ''} onChange={e => set(`stat${i}_suffix`, e.target.value)} placeholder="K, %, +" />
                  </div>
                  <div className="form-group" style={{ marginBottom: 0 }}>
          ...
          {activeTab === 'content' && (
            <div>
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '.5px', margin: '16px 0 10px' }}>Trang chủ — Dịch vụ chính</div>
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '.5px', margin: '20px 0 10px' }}>Trang chủ — CTA</div>
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '.5px', margin: '20px 0 10px' }}>Trang Về chúng tôi</div>
              ...
              <div className="form-group"><label className="form-label">Nội dung sứ mệnh</label><textarea className="form-control" rows={3} value={s.about_mission_text ?? ''} onChange={e => set('about_mission_text', e.target.value)} /></div>
              {[1, 2, 3].map(i => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '80px 1fr 2fr', gap: 12, marginBottom: 12 }}>
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    <label className="form-label">Icon</label>
                    <input className="form-control" value={s[`why_us_${i}_icon`] ?? ''} onChange={e => set(`why_us_${i}_icon`, e.target.value)} />
                  </div>
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    <label className="form-label">Lý do {i} — tiêu đề</label>
                    <input className="form-control" value={s[`why_us_${i}_title`] ?? ''} onChange={e => set(`why_us_${i}_title`, e.target.value)} />
                  </div>
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    <label className="form-label">Lý do {i} — mô tả</label>
                    <input className="form-control" value={s[`why_us_${i}_desc`] ?? ''} onChange={e => set(`why_us_${i}_desc`, e.target.value)} />
                  </div>
                </div>
              ))}
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '.5px', margin: '20px 0 10px' }}>Trang Dịch vụ</div>
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '.5px', margin: '20px 0 10px' }}>Trang Liên hệ</div>

# Strategy Consulting — Website Deploy

Website công ty tư vấn chiến lược kinh doanh (React SPA + PHP API + SQLite) — sẵn sàng deploy lên hosting hỗ trợ PHP + `pdo_sqlite`.
...
| Dịch vụ | 6 dịch vụ tư vấn chiến lược — hiển thị ở cả trang chủ và trang Dịch vụ |
| Liên hệ | Tin nhắn khách gửi từ form Liên hệ |
| Thư viện ảnh | Quản lý ảnh đã upload |
| Cài đặt | Thông tin chung, SEO, Mạng xã hội (Zalo), Footer, Liên hệ (bản đồ), Thống kê trang chủ, Nội dung từng trang (hero/CTA/sứ mệnh/tại sao chọn chúng tôi), Pháp lý (chính sách bảo mật/điều khoản), SMTP, Nâng cao, Cloudinary, Tích hợp (Unsplash) |

## Ghi chú kỹ thuật

- Database: SQLite, file tại `api/database/strategy-consulting.db` — tự động tạo + seed dữ liệu mẫu trong lần chạy đầu tiên (nội dung lấy từ template gốc: 4 slide carousel, 6 dịch vụ, thống kê, nội dung Về chúng tôi/Dịch vụ/Liên hệ, chính sách bảo mật/điều khoản).
...
- Template gốc không có mục Đội ngũ (Team), Dự án (Projects) hay Đánh giá khách hàng (Testimonials) — các module này không được tạo ở site này (đúng nguyên tắc bám sát nội dung template).

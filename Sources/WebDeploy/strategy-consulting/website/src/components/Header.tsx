      ...
      <nav id="nav">
        <div className="wd-container">
          <div className="sc-nav-inner">
            <Link to="/" className="sc-logo">{logoText}<span> &amp;Co</span></Link>
            <div className="sc-nav-links">
              {NAV_LINKS.map(l => (
                <Link key={l.to} to={l.to} className={location.pathname === l.to ? 'active' : ''}>{l.label}</Link>
              ))}
            </div>
            <Link to="/lien-he" className="sc-nav-cta">Tư vấn</Link>
            <button className="sc-nav-hamburger" id="navBurger" aria-label="Menu"><span></span><span></span><span></span></button>
          </div>
        </div>
      </nav>

      <div className="sc-nav-mobile" id="navMob">
        {NAV_LINKS.map(l => (
          <Link key={l.to} to={l.to}>{l.label}</Link>
        ))}
        <Link to="/lien-he" className="nm-cta">Tư vấn</Link>
      </div>
    </>
  )
}

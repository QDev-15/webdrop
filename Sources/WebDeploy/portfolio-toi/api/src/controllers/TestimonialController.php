...
class TestimonialController {
    ...
    public function store(array $p): void {
        ...
        $id = $this->db->execute(
            ...
            [$b['author_name'], $b['author_title'] ?? '', $b['author_avatar'] ?? '',
             $b['content'], (int)($b['rating'] ?? 5), (int)($b['sort_order'] ?? 0), $b['status'] ?? 'published']
        );
    ...
    public function update(array $p): void {
        ...
        $this->db->execute(
            ...
            [$b['author_name'], $b['author_title'] ?? '', $b['author_avatar'] ?? '',
             $b['content'], (int)($b['rating'] ?? 5), (int)($b['sort_order'] ?? 0), $b['status'] ?? 'published', $p['id']]
        );

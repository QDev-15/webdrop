...
class TestimonialController {
    ...
    public function store(): void {
        ...
        $author_location  = trim($data['author_location'] ?? '');
        ...
        $id = $this->db->execute(
            "INSERT INTO testimonials (author_name, author_avatar, author_location, content, stars, product_purchased, is_active, sort_order)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            [$author_name, $author_avatar, $author_location, $content, $stars, $product_purchased, $is_active, $sort_order]
        );
        $row = $this->db->queryOne("SELECT * FROM testimonials WHERE id = ?", [$id]);
    ...
    public function update(array $params): void {
        ...
        $author_location  = trim($data['author_location'] ?? ($row['author_location'] ?? ''));
        $content          = trim($data['content'] ?? $row['content']);
        $stars            = (int)($data['stars'] ?? $row['stars']);
        ...
        $this->db->execute(
            "UPDATE testimonials SET author_name=?, author_avatar=?, author_location=?, content=?, stars=?, product_purchased=?, is_active=?, sort_order=? WHERE id=?",
            [$author_name, $author_avatar, $author_location, $content, $stars, $product_purchased, $is_active, $sort_order, $id]
        );
        $row = $this->db->queryOne("SELECT * FROM testimonials WHERE id = ?", [$id]);
        Response::json($row);
    }

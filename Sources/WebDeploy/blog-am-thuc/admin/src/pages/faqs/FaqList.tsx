...
export default function FaqList() {
  ...
  async function load() {
    try { setItems(await api.get<Faq[]>('/faqs')) } finally { setLoading(false) }
  }
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">{items.length} câu hỏi — hiển thị ở trang chủ</div>
        </div>
      ...
      {items.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {items.map(f => (
            <div key={f.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start' }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
                <span className={`badge badge-${f.status}`}>{f.status === 'published' ? 'Hiện' : 'Ẩn'}</span>

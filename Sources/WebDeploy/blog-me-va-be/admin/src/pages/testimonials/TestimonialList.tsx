    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Hiển thị ở trang Về tôi ({items.length} đánh giá)</div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {items.map(item => (
            <div key={item.id} className="card" style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
              {item.author_avatar && <img src={item.author_avatar} alt={item.author_name} className="thumb" style={{ borderRadius: '50%' }} />}
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 14 }}>{item.author_name} <span style={{ fontWeight: 400, color: 'var(--text-3)', fontSize: 12 }}>· {item.author_meta}</span></div>
                <div style={{ fontSize: 13, color: 'var(--text-2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 560, marginTop: 4 }}>{item.content}</div>
              </div>

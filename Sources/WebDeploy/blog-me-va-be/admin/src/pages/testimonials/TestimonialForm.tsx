...
const empty: FormData = {
  author_name: '', author_avatar: '', author_meta: '', content: '', sort_order: 0, status: 'published',
}
...
export default function TestimonialForm() {
  ...
  useEffect(() => {
    ...
    api.get<FormData & { id: number }>(`/testimonials/${id}`)
      .then(d => setForm({
        author_name: d.author_name, author_avatar: d.author_avatar ?? '', author_meta: d.author_meta ?? '',
        content: d.content, sort_order: d.sort_order ?? 0, status: d.status ?? 'published',
      }))
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    if (!form.author_name.trim() || !form.content.trim()) { setError('Tên độc giả và nội dung là bắt buộc.'); return }
    setSaving(true)
    try {
      if (isEdit) {
      ...
      <form onSubmit={handleSubmit} className="card">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="form-group">
            ...
            <input type="text" className="form-control" value={form.author_name} onChange={e => set('author_name', e.target.value)} placeholder="Vd: Thu Trang" required />
          </div>
          <div className="form-group">
            <label className="form-label">Thông tin phụ</label>
            <input type="text" className="form-control" value={form.author_meta} onChange={e => set('author_meta', e.target.value)} placeholder="Mẹ bé Bống, 8 tháng tuổi" />
          </div>
        </div>
        <div className="form-group">

    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Quản lý carousel trang chủ ({slides.length} slides)</div>
        </div>
      ...
      {slides.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {slides.map(slide => {
            const firstImg = (slide.image || '').split(/\r?\n/)[0]
            const [label, ...rest] = (slide.subtitle || '').split('||')
            const desc = rest.join('||')
            return (
              <div key={slide.id} className="card" style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                {firstImg && (
                  <img src={firstImg} alt={slide.title}
                    style={{ width: 100, height: 60, objectFit: 'cover', borderRadius: 8, flexShrink: 0, border: '1px solid var(--border)' }} />
                )}
                <div style={{ flex: 1, minWidth: 0 }}>
                  {label && <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 2 }}>{label}</div>}
                  <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{slide.title.replace(/\*/g, '')}</div>
                  {desc && (
                    <div style={{ fontSize: 13, color: 'var(--text-2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{desc}</div>
                  )}

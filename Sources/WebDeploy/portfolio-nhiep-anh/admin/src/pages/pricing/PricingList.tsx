...
export default function PricingList() {
  ...
  async function load() {
    try { setItems(await api.get<Pricing[]>('/pricing-plans')) }
    ...
    <div>
      ...
      {items.length === 0 ? (
        <div className="empty-state">
          ...
          <div className="empty-state-text">Chưa có gói giá nào. Thêm gói giá đầu tiên!</div>
        </div>
      ) : (
          ...
          {items.map(item => (
            <div key={item.id} className="card" style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 15 }}>{item.name} {!!item.is_featured && <span className="badge badge-published" style={{ marginLeft: 8 }}>Nổi bật</span>}</div>
                <div style={{ fontSize: 13, color: 'var(--text-2)' }}>{item.price} {item.unit}</div>
              </div>

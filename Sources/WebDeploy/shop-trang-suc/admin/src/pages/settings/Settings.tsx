  ...
  return (
    <div className="admin-page">
      ...
      <form onSubmit={handleSave} className="admin-form" style={{ marginTop: 24, maxWidth: 760 }}>
        ...
        {activeTab === 'general' && <>
          <Field label="Tên cửa hàng" name="site_name" value={val('site_name')} onChange={set} placeholder="VIOLETTE" />
          <Field label="Slogan" name="site_tagline" value={val('site_tagline')} onChange={set} placeholder="Trang sức tinh xảo, chế tác từ trái tim" />
          <Field label="Số điện thoại" name="site_phone" value={val('site_phone')} onChange={set} placeholder="1900 2726" />
          <Field label="Email" name="site_email" value={val('site_email')} onChange={set} type="email" placeholder="hello@violette.vn" />
          <Field label="Địa chỉ cửa hàng (TP.HCM)" name="site_address" value={val('site_address')} onChange={set} placeholder="25 Đồng Khởi, Quận 1, TP.HCM" />
          <Field label="Địa chỉ cửa hàng (Hà Nội)" name="site_address_hn" value={val('site_address_hn')} onChange={set} placeholder="15 Tràng Tiền, Hoàn Kiếm, Hà Nội" />
          <Field label="Giờ mở cửa" name="working_hours" value={val('working_hours')} onChange={set} placeholder="Thứ 2 – Chủ nhật: 9:00 – 20:00" />
        ...
        {activeTab === 'seo' && <>
          <Field label="Meta Title" name="meta_title" value={val('meta_title')} onChange={set} placeholder="VIOLETTE — Trang sức tinh xảo, chế tác từ trái tim" />
        ...
        {activeTab === 'smtp' && <>
          ...
          <Field label="From Email" name="smtp_from" value={val('smtp_from')} onChange={set} placeholder="no-reply@violette.vn" />
          <Field label="From Name" name="smtp_from_name" value={val('smtp_from_name')} onChange={set} placeholder="VIOLETTE" />
        </>}
        ...
        {activeTab === 'cloudinary' && <>
          ...
          <Field label="Thư mục lưu ảnh" name="cloudinary_folder" value={val('cloudinary_folder')} onChange={set} placeholder="shop-trang-suc" />
        </>}

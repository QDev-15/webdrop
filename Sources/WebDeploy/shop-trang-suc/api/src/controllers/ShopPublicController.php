...
class ShopPublicController {
    ...
    public function products(): void {
        ...
        // AI (shop-trang-suc): catalog toolbar (san-pham.html) lọc danh mục theo slug (nhan, day-chuyen...)
        ...
        // AI (shop-trang-suc): filter riêng cho trang sức — chất liệu / dịp sử dụng (cột đơn giá trị,
        // dùng IN) + theme (CSV, dùng cho 4 khối THEMED-SECTIONS ở trang chủ, match theo dấu phẩy biên).
        $materials = array_values(array_filter(array_map('trim', explode(',', $_GET['material'] ?? ''))));
        ...
        if ($materials) {
            $ph = implode(',', array_fill(0, count($materials), '?'));
            $where[] = "p.material IN ($ph)";
            array_push($params, ...$materials);
        }
        if ($occasions) {
            $ph = implode(',', array_fill(0, count($occasions), '?'));
            $where[] = "p.occasion IN ($ph)";
            array_push($params, ...$occasions);
        }
        if ($theme !== '') {
            $where[] = "(',' || p.theme || ',') LIKE ?";
            $params[] = '%,' . $theme . ',%';

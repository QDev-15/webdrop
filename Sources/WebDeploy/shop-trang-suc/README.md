# VIOLETTE Fine Jewelry — shop-trang-suc (Website Deploy)

Website bán trang sức (nhẫn, dây chuyền, bông tai, lắc tay, bộ trang sức) — React SPA + React Admin SPA + PHP API + SQLite.
...
Kết quả build nằm ở `../_output-deploy/` (cùng cấp với thư mục `shop-trang-suc/`).
...
2. Mở `api/config.php` trên server, sửa `APP_URL` thành domain thật (không có dấu `/` cuối), ví dụ:
   ```php
   define('APP_URL', 'https://violette.vn');
   ```
...
9. Vào **Cài đặt → Thông tin chung** để cập nhật tên cửa hàng, số điện thoại, địa chỉ (TP.HCM + Hà Nội), giờ mở cửa, số Zalo, bản đồ Google Maps.
...
- Database SQLite tại `api/database/shop-trang-suc.db` — tự động khởi tạo + seed dữ liệu mẫu (5 danh mục, 45 sản phẩm trang sức, 3 đánh giá khách hàng) khi chạy lần đầu.
- Trang chủ (`/`) dùng bố cục THEMED-SECTIONS: 4 khối sản phẩm theo chủ đề (Mới về / Bán chạy nhất / Quà tặng ý nghĩa / Đang ưu đãi) — mỗi sản phẩm có thể thuộc nhiều chủ đề cùng lúc (cột `products.theme`, quản lý trong Admin → Sản phẩm).
- Trang `/san-pham` là catalog đầy đủ với toolbar lọc: danh mục (pill), chất liệu + dịp sử dụng (dropdown checkbox đa chọn), khoảng giá (range), sắp xếp — áp dụng tức thì, đồng bộ URL.

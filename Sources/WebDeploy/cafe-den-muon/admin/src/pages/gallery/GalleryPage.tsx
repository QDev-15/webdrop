      ...
      {showForm && (
        <div className="card" style={{ marginBottom: 24 }}>
          ...
          <form onSubmit={handleSave}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div className="form-group">
                ...
                <select className="form-control" value={form.category} onChange={e => set('category', e.target.value)}>
                  <option value="">Ảnh quán (thư viện ảnh chung)</option>
                  <option value="khu-vuc">Khu vực (area card — trang Không gian)</option>
                </select>
              </div>
              <div className="form-group">
            ...
            <div className="form-group">
              <label className="form-label">Mo ta {form.category === 'khu-vuc' ? '(dòng 1 = tagline ngắn, các dòng sau = mô tả đầy đủ)' : ''}</label>
              <textarea className="form-control" rows={form.category === 'khu-vuc' ? 3 : 2} value={form.description} onChange={e => set('description', e.target.value)}
                placeholder={form.category === 'khu-vuc' ? 'Bàn đơn · Ổ cắm 100% · Đèn bàn riêng\nDành cho làm việc một mình đến khuya...' : 'Mo ta anh'} />
            </div>

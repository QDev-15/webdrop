...
class ServiceController {
    ...
    public function store(array $p = []): void {
        ...
        $id = $this->db->execute(
            "INSERT INTO services (number, name, description, duration, price, badge, is_featured, sort_order)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            [
                $b['number']      ?? '',
                $b['name'],
    ...
    public function update(array $p = []): void {
        ...
        if (empty($b['name'])) { Response::error('Ten dich vu la bat buoc.', 422); return; }
        $this->db->execute(
            "UPDATE services SET number=?, name=?, description=?, duration=?, price=?, badge=?, is_featured=?, sort_order=?
             WHERE id=?",
            [
                $b['number']      ?? '',
                $b['name'],

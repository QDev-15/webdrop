      ...
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">
            {services.length} dịch vụ · <Link to="/services/categories" style={{ color: 'var(--accent)' }}>Quản lý nhóm</Link>
          </div>
        </div>
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {services.map(svc => (
              <tr key={svc.id}>
                <td style={{ fontSize: 12, color: 'var(--text-3)' }}>{svc.number || '—'}</td>
                <td>
                  <div style={{ fontWeight: 500 }}>{svc.name}</div>
                  {svc.description && (
                    <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 2, maxWidth: 320, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {svc.description}
                    </div>
                  )}
                </td>

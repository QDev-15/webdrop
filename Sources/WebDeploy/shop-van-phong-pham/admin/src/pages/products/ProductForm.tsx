...
// Ghi chú: template san-pham (index.html) của site này KHÔNG có filter "Màu sắc" — trường colors
// vẫn giữ trong schema core cho đồng bộ, nhưng không có bộ lọc tương ứng ở ProductsPage.tsx.
const COLOR_SWATCHES = [
  { name: 'Đen', hex: '#1e1e1e' },
  { name: 'Trắng', hex: '#ffffff' },
  { name: 'Xanh dương', hex: '#2563a8' },
  { name: 'Đỏ', hex: '#dc2626' },
]
...
interface FormData {
  ...
  status: string
  sort_order: string
  // ▼ AI: field thêm cho shop-van-phong-pham (khớp cột brand/theme/sold trong schema.sql)
  brand: string
  theme: string
  sold: string
}
...
function serializeThemes(themes: string[]): string {
...
export default function ProductForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      price_sale: form.price_sale.trim() ? Number(form.price_sale) : null,
      ...
      <form onSubmit={handleSubmit} className="admin-form">
        ...
        <div className="form-row">
          <div className="form-group" style={{ flex: 1 }}>
            ...
            <input type="text" value={form.brand} onChange={e => set('brand', e.target.value)} placeholder="VD: Thiên Long, Pentel, Deli" />
          </div>
          <div className="form-group" style={{ flex: 1 }}>
            <label>Đã bán (dùng cho sắp xếp "Bán chạy")</label>
            <input type="number" value={form.sold} onChange={e => set('sold', e.target.value)} min={0} placeholder="0" />
          </div>
        </div>

        <div className="form-group">
          <label>Section trang chủ (dùng cho bộ lọc "theme" — tùy chọn)</label>
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginTop: 4 }}>
            {THEME_OPTIONS.map(t => {
              const themes = parseThemes(form.theme)
              ...
              return (
                <label key={t.value} className="form-check">
                  <input
                    ...
                    checked={active}
                    onChange={() => {
                      const next = active ? themes.filter(x => x !== t.value) : [...themes, t.value]
                      set('theme', serializeThemes(next))
                    }}
                  />
                  <span>{t.label}</span>
                </label>
        ...
        <div className="form-group">
          <label>Màu sắc (tùy chọn — template này không có bộ lọc "Màu sắc" trên trang Sản phẩm)</label>

...
export default function TeamList() {
  ...
  async function toggleStatus(m: TeamMember) {
    await api.put(`/team/${m.id}`, { status: m.status === 'published' ? 'draft' : 'published' })
    load()
  }
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <div className="page-title">Đội ngũ bác sĩ chuyên khoa</div>
          <div className="page-sub">{items.length} bác sĩ</div>
        </div>
            ...
            <div key={m.id} className="card" style={{ padding: 0, overflow: 'hidden' }}>
              {m.image && (
                <div style={{ height: 180, overflow: 'hidden' }}>
                  <img src={m.image} alt={m.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} loading="lazy" />
                </div>
              )}
              <div style={{ padding: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <button
                    ...
                    className={`badge ${m.status === 'published' ? 'badge-published' : 'badge-draft'}`}
                    style={{ cursor: 'pointer', border: 'none' }}
                  >
                    {m.status === 'published' ? 'Hiển thị' : 'Ẩn'}
                  </button>

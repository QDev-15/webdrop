  ...
  return (
    <>
      <div className="page-header">
        <div>
          <div className="page-title">Bác sĩ gia đình</div>
            ...
            <tbody>
              {items.map(d => (
                <tr key={d.id}>
                  ...
                  <td style={{ color: 'var(--text-3)', fontSize: '12px' }}>{d.tags}</td>
                  <td>{d.is_active ? <span style={{ color: 'var(--accent)' }}>Có</span> : <span style={{ color: 'var(--text-3)' }}>Ẩn</span>}</td>
                  <td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--text-3)', padding: '32px' }}>Chưa có bác sĩ nào</td></tr>
              )}
            </tbody>
          </table>
        </div>

...
export default function FaqList() {
  ...
  async function load() {
    try {
      setItems(await api.get<Faq[]>('/faqs'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Quản lý accordion câu hỏi thường gặp hiển thị trên trang Dịch vụ</div>
        </div>
        ...
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {items.map(item => (
                <tr key={item.id}>
                  <td style={{ fontWeight: 500, maxWidth: 420 }}>{item.question}</td>
                  <td style={{ fontSize: 13, color: 'var(--text-2)' }}>{item.page}</td>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <Link to={`/faqs/${item.id}`} className="btn-ghost btn-sm">Sửa</Link>

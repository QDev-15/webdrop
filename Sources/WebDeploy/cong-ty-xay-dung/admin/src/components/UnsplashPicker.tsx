        ...
        <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          <div style={{ flex: 1, position: 'relative' }}>
            ...
            <input ref={inputRef} value={query} onChange={e => handleQueryChange(e.target.value)}
              placeholder="Tìm ảnh Unsplash... (vd: construction, building, architecture)"
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          {!query && !loading && (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#9ca3af' }}>
              ...
              <div style={{ fontSize: 12, marginTop: 8 }}>Gợi ý: construction · building · architecture · team · office</div>
            </div>
          )}
          {error && (
          ...
          {loading && results.length === 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
              {Array.from({ length: 9 }).map((_, i) => (
                <div key={i} style={{ aspectRatio: '4/3', borderRadius: 8, background: '#f3f4f6' }} />
              ))}
            </div>
          )}
          {results.length > 0 && (
            <>
              ...
              <div style={{ textAlign: 'center', marginTop: 14, fontSize: 11, color: '#9ca3af' }}>
                Ảnh từ <a href="https://unsplash.com" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent)', textDecoration: 'none' }}>Unsplash</a>
              </div>
            </>
          )}
          {!loading && query && results.length === 0 && !error && (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#9ca3af', fontSize: 13 }}>
              Không tìm thấy ảnh cho "{query}". Thử từ khóa khác.
            </div>
          )}
        </div>
      </div>

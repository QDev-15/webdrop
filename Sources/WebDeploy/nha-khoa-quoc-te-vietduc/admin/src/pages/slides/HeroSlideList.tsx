    ...
    <div className="adm-page">
      <div className="adm-page-header">
        <div>
          ...
          <p className="adm-page-sub">Quản lý ảnh slider trang chủ ({slides.length} slides)</p>
        </div>
        <Link to="/hero/new" className="adm-btn-primary">+ Thêm slide</Link>
      </div>
          ...
          {slides.map(slide => (
            <div key={slide.id} className="card" style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
                ...
                <Link to={`/hero/${slide.id}/edit`} className="adm-btn-ghost adm-btn-sm">Sửa</Link>
                <button onClick={() => handleDelete(slide.id)} className="adm-btn-danger adm-btn-sm">Xóa</button>
              </div>
            </div>
          ))}
        </div>

  ...
  return (
    <div className="adm-page">
      <div className="adm-page-header">
        <div>
          <h1 className="adm-page-title">Liên hệ</h1>
          <p className="adm-page-sub">{items.filter(i => i.status === 'new').length} tin nhắn mới · {items.length} tổng</p>
        </div>
      </div>

      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {['', 'new', 'read', 'replied'].map(s => (
          <button key={s} onClick={() => setFilter(s)} className={filter === s ? 'adm-btn-primary adm-btn-sm' : 'adm-btn-ghost adm-btn-sm'}>
              ...
              {filtered.map(c => (
                <tr key={c.id} onClick={() => handleView(c)} style={{ cursor: 'pointer', background: detail?.id === c.id ? 'var(--accent-light)' : undefined, fontWeight: c.status === 'new' ? 600 : undefined }}>
                  ...
                  <td onClick={e => e.stopPropagation()}>
                    <button onClick={() => handleDelete(c.id)} className="adm-btn-ghost adm-btn-sm">Xóa</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {detail && (
          <div className="adm-detail-panel">
            <div className="adm-detail-header">
              <span style={{ fontWeight: 600, fontSize: 15 }}>Chi tiết tin nhắn</span>
            ...
            <div className="adm-detail-actions" style={{ marginTop: 14 }}>
              {detail.status !== 'replied' && (
                <button onClick={() => markReplied(detail.id)} className="adm-btn-primary adm-btn-sm">Đánh dấu đã trả lời</button>
              )}
              <button onClick={() => handleDelete(detail.id)} className="adm-btn-ghost adm-btn-sm">Xóa</button>
            </div>
          </div>
        )}
      </div>

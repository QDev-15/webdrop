...
export default function TestimonialForm() {
  ...
  useEffect(() => {
    if (!id) return
    api.get<TestimonialFormData & { id: number }>(`/testimonials/${id}`)
      .then(d => setForm({
      ...
      <form onSubmit={handleSubmit} className="card">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="form-group">
            ...
            <input type="text" className="form-control" value={form.author_name} onChange={e => set('author_name', e.target.value)} placeholder="Trần Thu Hà" required />
          </div>
          <div className="form-group">
            <label className="form-label">Chức vụ / Công ty</label>
            <input type="text" className="form-control" value={form.author_title} onChange={e => set('author_title', e.target.value)} placeholder="CEO, E-commerce Store" />
          </div>
        </div>
        <div className="form-group">
          <ImageField label="Ảnh đại diện (hoặc dùng emoji, vd: 👩 / 👨)" value={form.author_avatar} onChange={v => set('author_avatar', v)} placeholder="https://... hoặc emoji 👩" />
        </div>
        <div className="form-group">
          <label className="form-label">Nội dung đánh giá *</label>
          <textarea className="form-control" value={form.content} onChange={e => set('content', e.target.value)} placeholder="Nội dung đánh giá của khách hàng..." rows={4} required />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
          <div className="form-group">
            <label className="form-label">Số sao</label>
            <select className="form-control" value={form.rating} onChange={e => set('rating', parseInt(e.target.value) || 5)}>

...
const empty: ServiceForm = {
  icon: '📊', title: '', short_desc: '', long_desc: '', image: '', sort_order: 0, status: 'published',
}
...
export default function ServiceForm() {
  ...
  useEffect(() => {
    ...
    api.get<ServiceForm & { id: number }>(`/services/${id}`)
      .then(d => setForm({
        icon: d.icon ?? '', title: d.title, short_desc: d.short_desc ?? '', long_desc: d.long_desc ?? '',
        image: d.image ?? '', sort_order: d.sort_order ?? 0, status: d.status ?? 'published',
      }))
    ...
    <div style={{ maxWidth: 640 }}>
      ...
      <form onSubmit={handleSubmit} className="card">
        ...
        <div className="form-group">
          ...
          <input type="text" className="form-control" value={form.title} onChange={e => set('title', e.target.value)} placeholder="Vd: Chiến lược Marketing" required />
        </div>
        <div className="form-group">
          <label className="form-label">Mô tả ngắn (hiển thị ở Trang chủ)</label>
          <textarea className="form-control" value={form.short_desc} onChange={e => set('short_desc', e.target.value)} placeholder="Mô tả ngắn gọn về dịch vụ" rows={3} />
        </div>
        <div className="form-group">
          <label className="form-label">Mô tả chi tiết (hiển thị ở trang Dịch vụ)</label>
          <textarea className="form-control" value={form.long_desc} onChange={e => set('long_desc', e.target.value)} placeholder="Mô tả chi tiết đầy đủ về dịch vụ" rows={5} />
        </div>
        <div className="form-group">
          <ImageField label="Ảnh minh họa (tuỳ chọn)" value={form.image} onChange={v => set('image', v)} />
        </div>

...
class PublicController {
    ...
    public function submitBooking(array $p = []): void {
        ...
        Response::json(['message' => 'Đặt lịch thành công. Chúng tôi sẽ liên hệ xác nhận trong 24 giờ làm việc.'], 201);
    }
    ...
    public function submitContact(array $p = []): void {
        ...
        $name = trim($b['name'] ?? '');
        if (!$name) {
            Response::error('Họ tên là bắt buộc.', 422);
            return;
        }
        $this->db->execute(
        ...
        Response::json(['message' => 'Cảm ơn bạn đã liên hệ. Chúng tôi sẽ phản hồi trong 24 giờ làm việc.'], 201);
    }
    ...
    public function sitemap(array $p = []): void {
        ...
        $staticRoutes = ['/', '/dich-vu-implant', '/cong-nghe-3d', '/bac-si', '/dat-lich', '/lien-he'];

  ...
  return (
    <>
      <div className="page-header">
        <div>
          <div className="page-title">{isEdit ? 'Sửa dịch vụ' : 'Thêm dịch vụ Implant'}</div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="form-card">
        ...
        <div className="form-field">
          ...
          <input type="text" value={form.name} onChange={e => set('name', e.target.value)} required placeholder="Implant một răng" />
        </div>

        <div className="form-field">
          <label>Mô tả</label>
          <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={3} placeholder="Mô tả ngắn về dịch vụ Implant..." />
        </div>

        <div className="form-field">
          <label>Tính năng / Điểm nổi bật</label>
          <textarea value={form.features} onChange={e => set('features', e.target.value)} rows={3}
            placeholder="Titanium Grade 4|Osseotite Surface|15 năm bảo hành" />
          <span className="form-hint">Phân cách các tính năng bằng ký tự |</span>
        </div>

        <div className="form-field">
          <label>Giá</label>
          <input type="text" value={form.price} onChange={e => set('price', e.target.value)} placeholder="Từ 18.000.000đ" />
        </div>

...
CREATE TABLE IF NOT EXISTS products (
    ...
    -- ▼ Cột riêng cho shop-noi-that (khớp template: dropdown Chất liệu/Không gian, trang Bộ sưu tập, sort "Bán chạy")
    material     TEXT NOT NULL DEFAULT '',   -- slug: go-tu-nhien|go-cong-nghiep|kim-loai|vai-boc|da|may-tre|khac
    room         TEXT NOT NULL DEFAULT '',   -- slug: phong-khach|phong-an|phong-ngu|phong-lam-viec|ban-cong-san-vuon
    collection_id INTEGER REFERENCES collections(id) ON DELETE SET NULL,
    sold         INTEGER NOT NULL DEFAULT 0  -- số lượng đã bán — dùng cho sort "Bán chạy" + hiển thị "Đã bán N"
);

-- Bộ sưu tập (trang bo-suu-tap.html) — 4 phong cách: Bắc Âu / Nhật Bản Zen / Công nghiệp / Vintage
...
-- Đánh giá khách hàng (trang ve-chung-toi.html — LIST-ELEGANT testimonials)
...
-- Mã giảm giá hiển thị ở trang khuyen-mai.html — chỉ "sao chép" (đúng hành vi template gốc,
-- KHÔNG áp dụng trừ tiền tự động ở giỏ hàng vì template không có ô nhập mã ở gio-hang.html).

...
export default function UnsplashPicker({ onSelect, onClose }: Props) {
  ...
  const handleSelect = (photo: UnsplashPhoto) => {
    ...
    if (photo.downloadLocation) {
      api.post('/unsplash/download', { downloadLocation: photo.downloadLocation }).catch(() => {})
    }
    onClose()
  }
        ...
        <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          <div style={{ flex: 1, position: 'relative' }}>
            ...
            <input ref={inputRef} value={query} onChange={e => handleQueryChange(e.target.value)}
              placeholder="Tìm ảnh Unsplash... (vd: coffee shop, interior, food)"
          ...
          <button onClick={onClose} style={{ padding: '8px 10px', background: '#f3f4f6', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 16, lineHeight: 1 }} title="Đóng">✕</button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          {!query && !loading && (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#9ca3af' }}>
              ...
              <div style={{ fontSize: 12, marginTop: 8 }}>Gợi ý: coffee · restaurant · interior · team · office</div>
            </div>
          )}
          {error && (

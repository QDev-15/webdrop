  ...
  return (
    <div className="admin-page">
      ...
      <form onSubmit={handleSave} className="admin-form" style={{ marginTop: 24, maxWidth: 760 }}>
        ...
        {activeTab === 'general' && <>
          <Field label="Tên cửa hàng" name="site_name" value={val('site_name')} onChange={set} placeholder="MỘC AN" />
          <Field label="Slogan" name="site_tagline" value={val('site_tagline')} onChange={set} placeholder="Nội thất tối giản cho không gian sống chậm" />
          <Field label="Số điện thoại" name="site_phone" value={val('site_phone')} onChange={set} placeholder="028 3636 5858" />
          <Field label="Email" name="site_email" value={val('site_email')} onChange={set} type="email" placeholder="hello@mocan.vn" />
          <Field label="Địa chỉ showroom" name="site_address" value={val('site_address')} onChange={set} placeholder="58 Đường số 8, Phường Tăng Nhơn Phú, TP. Thủ Đức, TP.HCM" />
          <Field label="Giờ mở cửa" name="working_hours" value={val('working_hours')} onChange={set} placeholder="Thứ 2 – Chủ nhật: 8:30 – 20:00" />
          <Field label="Số Zalo (không dấu cách)" name="zalo_number" value={val('zalo_number')} onChange={set} placeholder="0283636585" />
        ...
        {activeTab === 'hero' && <>
          ...
          <Field label="Mục 1" name="hero_topbar_1" value={val('hero_topbar_1')} onChange={set} placeholder="Miễn phí giao hàng nội thành cho đơn từ 5.000.000₫" />
          <Field label="Mục 2" name="hero_topbar_2" value={val('hero_topbar_2')} onChange={set} placeholder="Bảo hành 24 tháng" />
          <Field label="Mục 3" name="hero_topbar_3" value={val('hero_topbar_3')} onChange={set} placeholder="Lắp đặt tận nơi toàn quốc" />
        </>}

        {activeTab === 'seo' && <>
          <Field label="Meta Title" name="meta_title" value={val('meta_title')} onChange={set} placeholder="MỘC AN — Nội thất tối giản cho không gian sống chậm" />
        ...
        {activeTab === 'social' && <>
          ...
          <Field label="Pinterest URL" name="pinterest" value={val('pinterest')} onChange={set} placeholder="https://www.pinterest.com/..." />
        </>}
        ...
        {activeTab === 'smtp' && <>
          ...
          <Field label="From Email" name="smtp_from" value={val('smtp_from')} onChange={set} placeholder="no-reply@mocan.vn" />
          <Field label="From Name" name="smtp_from_name" value={val('smtp_from_name')} onChange={set} placeholder="MỘC AN" />
        </>}
        ...
        {activeTab === 'cloudinary' && <>
          ...
          <Field label="Thư mục lưu ảnh" name="cloudinary_folder" value={val('cloudinary_folder')} onChange={set} placeholder="shop-noi-that" />
        </>}

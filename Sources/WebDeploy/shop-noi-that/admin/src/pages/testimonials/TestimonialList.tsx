    ...
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          ...
          <p className="admin-page-sub">Quản lý các đánh giá hiển thị ở trang "Giới thiệu"</p>
        </div>
      ...
      {loading ? <div className="admin-loading-box">Đang tải...</div> : (
        <div className="admin-table-wrap">
          <table className="admin-table">
            ...
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', color: '#aaa' }}>Chưa có đánh giá nào</td></tr>
              ) : items.map(item => (
                <tr key={item.id}>
                  <td>
                    {item.author_avatar ? (
                      ...
                      <div style={{ width: 44, height: 44, background: '#f0ebe3', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>💬</div>
                    )}
                  </td>
                  <td>
                    <strong>{item.author_name}</strong>
                    <div style={{ fontSize: 12, color: '#aaa' }}>{item.author_location}</div>
                  </td>

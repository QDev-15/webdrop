  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        ...
        <p className="admin-page-sub">Tổng quan cửa hàng MỘC AN</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card">
            <div className="stat-icon">💰</div>
            <div className="stat-info">
              <div className="stat-num" style={{ fontSize: 20 }}>{fmt(stats?.revenue ?? 0)}</div>
              <div className="stat-label">Doanh thu đã thanh toán</div>
            </div>
            <Link to="/orders" className="stat-link">Xem →</Link>
          </div>
        </div>
      ...
      <div className="admin-quick-links" style={{ marginTop: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          ...
          <Link to="/collections/new" className="btn btn-outline">+ Thêm bộ sưu tập</Link>
          <Link to="/testimonials/new" className="btn btn-outline">+ Thêm đánh giá</Link>
          <Link to="/coupons/new" className="btn btn-outline">+ Thêm mã giảm giá</Link>
        </div>
      </div>
    </div>
  )

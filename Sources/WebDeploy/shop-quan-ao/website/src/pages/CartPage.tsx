...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <main>
        <div className="qa-cart-wrap">
          <div className="qa-container" style={{ textAlign: 'center' }}>
            <i className="bi bi-bag-x" style={{ fontSize: 56, color: 'var(--text-3)' }} />
            <h1 className="qa-cart-title" style={{ marginTop: 24 }}>Giỏ hàng đang trống</h1>
            <p style={{ color: 'var(--text-2)', marginBottom: 32 }}>Hãy khám phá bộ sưu tập của chúng tôi để tìm sản phẩm yêu thích.</p>
    ...
    <main>
      <div className="qa-cart-wrap">
        <div className="qa-container">
          ...
          <div className="qa-cart-layout">
            <div>
              <h1 className="qa-cart-title">Giỏ hàng của bạn</h1>

              <div className="qa-cart-head">
                <span>Sản phẩm</span><span>Đơn giá</span><span>Số lượng</span><span>Tạm tính</span><span />
              </div>

              {items.map(item => (
                <div className="qa-cart-item" key={`${item.product_id}-${item.color ?? ''}-${item.size ?? ''}`}>
                  <div className="qa-cart-prod">
                    <div className="qa-cart-thumb"><img src={item.image} alt={item.name} loading="lazy" /></div>
                    <div>
                      <div className="qa-cart-prod-name">{item.name}</div>
                      {(item.color || item.size) && (
                        <div className="qa-cart-prod-var">
                          {item.color && <>Màu: {item.color}</>}{item.color && item.size && ' · '}{item.size && <>Size: {item.size}</>}
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="qa-cart-price">{fmt(item.price)}</div>
                  <div className="qa-cart-qty" role="group" aria-label="Số lượng">
                  ...
                  <div className="qa-cart-subtotal">{fmt(item.price * item.qty)}</div>
                  <button className="qa-remove-btn" aria-label="Xóa sản phẩm khỏi giỏ" onClick={() => removeItem(item.product_id, item.color, item.size)}>
                    <i className="bi bi-x" />
                  </button>
                </div>
              ))}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 28, flexWrap: 'wrap', gap: 12 }}>
                <Link to="/san-pham" className="qa-btn qa-btn-ghost"><i className="bi bi-arrow-left me-1" />Tiếp tục mua sắm</Link>
                <button type="button" className="qa-btn qa-btn-butter" onClick={handleUpdateCart}>
                  {updated ? <><i className="bi bi-check-circle me-1" />Đã cập nhật</> : <><i className="bi bi-arrow-repeat me-1" />Cập nhật giỏ hàng</>}
                </button>
              </div>

              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, marginTop: 36, padding: 24, background: 'var(--surface)', borderRadius: 24, boxShadow: '0 4px 18px rgba(185,139,209,.08)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 13, color: 'var(--text-2)' }}>
                  ...
                  <div><strong style={{ display: 'block', color: 'var(--text)' }}>Đổi trả dễ dàng</strong>Trong vòng {settings.return_days || 15} ngày</div>
                </div>
              </div>
            </div>

            <aside>
              <div className="qa-order-summary">
                <h2 className="qa-summary-title">Tóm tắt đơn hàng</h2>
                <div className="qa-summary-row"><span>Tạm tính ({items.reduce((s, i) => s + i.qty, 0)} sản phẩm)</span><span>{fmt(subtotal)}</span></div>
                ...
                {discount > 0 && (
                  <div className="qa-summary-row"><span>Giảm giá ({couponCode})</span><span style={{ color: 'var(--accent)' }}>−{fmt(discount)}</span></div>
                )}
                {couponCode ? (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, fontSize: 12, color: 'var(--text-2)', marginBottom: 12 }}>
                    ...
                    <button type="button" onClick={removeCoupon} style={{ background: 'none', border: 'none', color: 'var(--danger, #e24b4a)', cursor: 'pointer', fontSize: 12, textDecoration: 'underline' }}>Gỡ mã</button>
                  </div>
                ) : (
                  <div className="qa-coupon-input">
                ...
                {couponError && <p style={{ fontSize: 12, color: 'var(--danger, #e24b4a)', marginTop: -10, marginBottom: 12 }}>{couponError}</p>}
                <div className="qa-summary-row total"><span>Tổng cộng</span><span>{fmt(total)}</span></div>
                <Link to="/thanh-toan" className="qa-btn-checkout"><i className="bi bi-lock-fill" />Thanh toán ngay</Link>
                <div className="qa-secure-badges">
                  <div className="qa-secure-badge"><i className="bi bi-shield-check" /> Bảo mật</div>
                  <div className="qa-secure-badge"><i className="bi bi-credit-card" /> VISA / MasterCard</div>
                  <div className="qa-secure-badge"><i className="bi bi-phone" /> MoMo / ZaloPay</div>
                </div>
              </div>
            </aside>
          </div>
          ...
          {related.length > 0 && (
            <div style={{ marginTop: 64 }}>
              <div className="qa-eyebrow" data-reveal>Gợi ý thêm</div>
              <h2 className="qa-sec-title" data-reveal data-delay="1">Bạn cũng có thể thích</h2>
              <div className="qa-related-grid" style={{ marginTop: 32 }}>
                {related.map((p, i) => (
                  <div className="qa-prod-card" data-reveal data-delay={String((i % 4) + 1)} key={p.id}>

...
export default function ProductDetailPage() {
  ...
  useDocumentMeta({
    title: product ? `${product.name} — Lys Chic` : 'Lys Chic',
  ...
  if (notFound || !product) {
    return (
      <div style={{ paddingTop: 180, textAlign: 'center', color: 'var(--text-3)', minHeight: '50vh' }}>
        ...
        <Link to="/san-pham" className="qa-btn qa-btn-outline" style={{ marginTop: 24, display: 'inline-flex' }}>← Quay lại</Link>
      </div>
    )
  }
  ...
  return (
    <main>
      <div className="qa-container">
        <nav className="qa-breadcrumb" style={{ marginTop: 150 }} aria-label="Breadcrumb">
        ...
        <div className="qa-detail-layout">
          ...
          <div data-reveal data-delay="1">
            <div className="qa-detail-cat">{product.category_name}</div>
            <h1 className="qa-detail-title">{product.name}</h1>
            <div className="qa-detail-rating">
              <span style={{ color: 'var(--butter)' }}>{'★'.repeat(Math.round(product.rating))}{'☆'.repeat(5 - Math.round(product.rating))}</span>
              <span>{product.rating.toFixed(1)}/5</span>
              <span>·</span>
              <span style={{ color: product.in_stock ? 'var(--accent)' : 'var(--sale)' }}>{product.in_stock ? 'Còn hàng' : 'Tạm hết hàng'}</span>
            </div>
            <div className="qa-detail-price">
              <span className="qa-detail-price-new">{fmt(product.price_sale || product.price)}</span>
              {!!product.price_sale && <span className="qa-detail-price-old">{fmt(product.price)}</span>}
            </div>
            <p className="qa-detail-desc">{product.description}</p>

            {colorOptions.length > 0 && (
              <div className="qa-option-block">
                <div className="qa-option-label"><span>Màu sắc</span><span style={{ fontWeight: 400, color: 'var(--text-3)' }}>{selectedColor}</span></div>
                <div className="qa-color-swatches">
                  {colorOptions.map(c => (
                    <div
                      key={c.name}
                      className={`qa-color-swatch${selectedColor === c.name ? ' selected' : ''}`}
            ...
            {sizeOptions.length > 0 && (
              <div className="qa-option-block">
                <div className="qa-option-label"><span>Kích cỡ</span><Link to="/lien-he" style={{ color: 'var(--accent)', fontWeight: 400 }}>Hướng dẫn chọn size</Link></div>
                <div className="qa-size-swatches">
                  {sizeOptions.map(s => (
                    <div
                      key={s}
                      className={`qa-size-swatch${selectedSize === s ? ' selected' : ''}`}
                      role="button"
                      tabIndex={0}
            ...
            <div className="qa-option-block">
              <div className="qa-option-label"><span>Số lượng</span></div>
              <div className="qa-qty-selector">
            ...
            <div className="qa-detail-actions">
              <button type="button" className="qa-btn qa-btn-primary" style={{ flex: 1, justifyContent: 'center' }} disabled={!product.in_stock} onClick={handleAddToCart}>
                {added ? <><i className="bi bi-check-circle-fill" /> Đã thêm vào giỏ</> : <><i className="bi bi-bag-plus" /> Thêm vào giỏ</>}
              </button>
              <button type="button" className="qa-btn qa-btn-outline" aria-label="Yêu thích sản phẩm"><i className="bi bi-heart" /></button>
            </div>

            <div className="qa-detail-features">
              <div className="qa-detail-feature"><i className="bi bi-flower2" /> Chất liệu mềm mại, thoáng mát</div>
              <div className="qa-detail-feature"><i className="bi bi-arrow-repeat" /> Đổi size miễn phí trong 15 ngày</div>
              <div className="qa-detail-feature"><i className="bi bi-truck" /> Giao hàng toàn quốc 1–3 ngày</div>
            </div>

            <div className="qa-detail-tabs" role="tablist">
              <button className={`qa-detail-tab${activeTab === 'desc' ? ' active' : ''}`} onClick={() => setActiveTab('desc')} role="tab" aria-selected={activeTab === 'desc'}>Mô tả</button>
              <button className={`qa-detail-tab${activeTab === 'spec' ? ' active' : ''}`} onClick={() => setActiveTab('spec')} role="tab" aria-selected={activeTab === 'spec'}>Thông số</button>
              <button className={`qa-detail-tab${activeTab === 'review' ? ' active' : ''}`} onClick={() => setActiveTab('review')} role="tab" aria-selected={activeTab === 'review'}>Đánh giá</button>
            </div>
            {activeTab === 'desc' && (
        ...
        {relatedItems.length > 0 && (
          <div style={{ margin: '64px 0 80px' }}>
            ...
            <h2 className="qa-sec-title" data-reveal data-delay="1">Sản phẩm <strong>liên quan</strong></h2>

  ...
  return (
    <div className="admin-page">
      ...
      <form onSubmit={handleSave} className="admin-form" style={{ marginTop: 24, maxWidth: 760 }}>
        ...
        {activeTab === 'general' && <>
          <Field label="Tên cửa hàng" name="site_name" value={val('site_name')} onChange={set} placeholder="Lys Chic" />
          <Field label="Slogan" name="site_slogan" value={val('site_slogan')} onChange={set} placeholder="Vẻ đẹp nhẹ nhàng trong từng thớ vải" />
          ...
          <Field label="Số Zalo" name="zalo_number" value={val('zalo_number')} onChange={set} placeholder="0938123456" />
        </>}
        ...
        {activeTab === 'home' && <>
          ...
          <Field label="Tag (dòng nhỏ phía trên tiêu đề)" name="hero_tag" value={val('hero_tag')} onChange={set} placeholder="Bộ sưu tập Xuân Hè 2026" />
          <Field label="Ghi chú dưới nút CTA" name="hero_note" value={val('hero_note')} onChange={set} placeholder="Miễn phí giao hàng đơn từ 300.000đ · Đổi trả trong 15 ngày" />
          <div className="form-row">
            <Field label="Điểm đánh giá" name="hero_rating_score" value={val('hero_rating_score')} onChange={set} placeholder="4.9/5" />
            <Field label="Số lượng đánh giá" name="hero_rating_count" value={val('hero_rating_count')} onChange={set} placeholder="2.100+" />
            <Field label="% Giảm giá (badge tròn)" name="hero_sale_percent" value={val('hero_sale_percent')} onChange={set} placeholder="30" />
          </div>
          ...
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="form-row" style={{ marginBottom: 4 }}>
              <div className="form-group" style={{ flex: 1 }}>
                ...
                <input type="text" value={val(`value${i}_icon`)} onChange={e => set(`value${i}_icon`, e.target.value)} placeholder="flower2" />
              </div>
          ...
          <div className="form-row">
            ...
            <Field label="Phần trăm" name="promo_percent" value={val('promo_percent')} onChange={set} placeholder="30%" />
          </div>
        ...
        {activeTab === 'reviews' && <>
          ...
          <div className="form-row">
            ...
            <Field label="Tổng số đánh giá" name="reviews_count" value={val('reviews_count')} onChange={set} placeholder="2.100+" />
          </div>
          {[1, 2, 3, 4].map(i => (
            <div key={i} style={{ padding: '12px 0', borderTop: '1px solid var(--border)', marginTop: 8 }}>
              ...
              <div className="form-row">
                ...
                <Field label="Địa điểm / Thời gian" name={`review${i}_location`} value={val(`review${i}_location`)} onChange={set} placeholder="TP.HCM · Tháng 6/2026" />
        ...
        {activeTab === 'seo' && <>
          <Field label="Meta Title" name="meta_title" value={val('meta_title')} onChange={set} placeholder="Lys Chic — Thời Trang Nữ Nhẹ Nhàng, Tinh Tế" />
        ...
        {activeTab === 'social' && <>
          <Field label="Facebook URL" name="facebook" value={val('facebook')} onChange={set} placeholder="https://www.facebook.com/lyschic" />
          <Field label="Instagram URL" name="instagram" value={val('instagram')} onChange={set} placeholder="https://www.instagram.com/lyschic" />
          <Field label="TikTok URL" name="tiktok" value={val('tiktok')} onChange={set} placeholder="https://www.tiktok.com/@lyschic" />
          <Field label="Zalo URL" name="zalo" value={val('zalo')} onChange={set} placeholder="https://zalo.me/0938123456" />
        </>}
        ...
        {activeTab === 'shop' && <>
          ...
          <Field label="Tiêu đề newsletter" name="newsletter_title" value={val('newsletter_title')} onChange={set} placeholder="Đăng ký nhận tin khuyến mãi" />
          <Field label="Mô tả newsletter" name="newsletter_desc" value={val('newsletter_desc')} onChange={set} placeholder="Nhận thông báo ưu đãi độc quyền..." />
        </>}

        {activeTab === 'payment' && <PaymentSettingsTab val={val} set={set} />}

        {activeTab === 'contact' && <>
          <Field label="Email liên hệ" name="site_email" value={val('site_email')} onChange={set} type="email" placeholder="hello@lyschic.vn" />
          <Field label="Số điện thoại" name="site_phone" value={val('site_phone')} onChange={set} placeholder="0938 123 456" />
          <Field label="Địa chỉ" name="site_address" value={val('site_address')} onChange={set} placeholder="12 Nguyễn Trãi, Quận 1, TP.HCM" />
        ...
        {activeTab === 'smtp' && <>
          ...
          <Field label="From Email" name="smtp_from" value={val('smtp_from')} onChange={set} placeholder="no-reply@lyschic.vn" />
          <Field label="From Name" name="smtp_from_name" value={val('smtp_from_name')} onChange={set} placeholder="Lys Chic" />
        </>}
        ...
        {activeTab === 'cloudinary' && <>
          ...
          <Field label="Thư mục lưu ảnh" name="cloudinary_folder" value={val('cloudinary_folder')} onChange={set} placeholder="lys-chic" />
        </>}

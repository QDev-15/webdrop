...
const empty: FormData = { name: '', role: '', avatar: '', content: '', sort_order: 0, status: 'published' }
...
export default function TestimonialForm() {
  ...
  useEffect(() => {
    ...
    api.get<Record<string, unknown>>(`/testimonials/${id}`)
      .then(d => setForm({
        name: String(d.name ?? ''), role: String(d.role ?? ''), avatar: String(d.avatar ?? ''),
        content: String(d.content ?? ''), sort_order: Number(d.sort_order ?? 0), status: String(d.status ?? 'published'),
      }))
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    if (!form.name.trim()) { setError('Tên độc giả là bắt buộc.'); return }
    setSaving(true)
    try {
      ...
      <form onSubmit={handleSubmit} className="card">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="form-group">
            <label className="form-label" htmlFor="name">Tên độc giả *</label>
            <input id="name" type="text" className="form-control" value={form.name} onChange={e => set('name', e.target.value)} placeholder="Quốc Huy" required />
          </div>
          <div className="form-group">
            <label className="form-label" htmlFor="role">Vai trò / bối cảnh</label>
            <input id="role" type="text" className="form-control" value={form.role} onChange={e => set('role', e.target.value)} placeholder="Nhân viên văn phòng, 27 tuổi" />
          </div>
        </div>
        <div className="form-group">

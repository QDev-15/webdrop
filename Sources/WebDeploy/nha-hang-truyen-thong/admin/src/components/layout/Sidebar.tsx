  ...
  useEffect(() => {
    api.get<{ new_contacts: number; pending_reservations: number }>('/stats')
      .then(s => {
        ...
        setPendingRes(s.pending_reservations ?? 0)
      })
      .catch(() => {})
  }, [])
  ...
  return (
    <div className="admin-sidebar">
      <div className="sidebar-logo">
        <span>🍲</span>
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#fff', letterSpacing: '-.3px' }}>Nhà Hàng</div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,.35)', marginTop: 1 }}>Ẩm Thực Truyền Thống</div>
        </div>
      </div>
      ...
      <div className="sidebar-footer">
        ...
        <button
          onClick={logout}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,.35)', fontSize: 16, padding: '4px 8px', borderRadius: 6, transition: 'color .2s' }}
          title="Đăng xuất"

...
CREATE TABLE IF NOT EXISTS products (
    ...
    colors       TEXT NOT NULL DEFAULT '',      -- pipe-separated "Tên:#hex" — dùng làm tông màu nông sản (Xanh/Đỏ/Cam/Tím/Vàng) cho bộ lọc "Màu sắc"
    ...
    -- ▼ Cột riêng cho shop-rau-xanh (template chi-tiet-san-pham.html/san-pham.html có các field này):
    unit         TEXT NOT NULL DEFAULT '',      -- đơn vị hiển thị cạnh giá: "bó", "kg", "túi 300g", "combo"...
    gallery      TEXT NOT NULL DEFAULT '',      -- pipe-separated URL ảnh phụ (ngoài ảnh chính `image`) cho gallery trang chi tiết
    tags         TEXT NOT NULL DEFAULT '',      -- pipe-separated đặc điểm: "Hữu cơ|VietGAP|Không thuốc BVTV" — dùng cho badge chi tiết + filter "Đặc điểm"
    nutrition    TEXT NOT NULL DEFAULT '',      -- pipe-separated "Tên:Giá trị" — bảng dinh dưỡng tab "Thông tin dinh dưỡng"
    sold_count   INTEGER NOT NULL DEFAULT 0,    -- "Đã bán 980+" ở trang chi tiết
    stock_qty    INTEGER NOT NULL DEFAULT 0     -- "còn 42 túi" ở trang chi tiết
);
...
-- ═══════════════════════════════════════════════════════════════════════════
-- COUPON EXTENSION — mã giảm giá, độc lập với hệ discount System DB webdrop.store.
-- Khớp 1-1 với CouponController.php (CRUD) + ShopPublicController::lookupCoupon()/validateCoupon().
-- ═══════════════════════════════════════════════════════════════════════════

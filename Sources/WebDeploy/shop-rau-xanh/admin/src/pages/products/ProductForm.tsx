...
// Bộ màu là TÔNG MÀU NÔNG SẢN (không phải màu sản phẩm thời trang) — dùng cho bộ lọc "Màu sắc"
// ở trang Sản phẩm (vd: Cà chua → Đỏ, Bí đỏ/Cà rốt → Cam, rau lá → Xanh lá).
...
// gallery / tags: pipe-separated string ↔ mảng dòng nhập trong textarea (mỗi dòng 1 giá trị)
...
function textToNutrition(v: string): string {
  return v.split('\n').map(s => s.trim()).filter(Boolean).join('|')
}
...
const EMPTY: FormData = {
  ...
  unit: '', gallery: '', tags: '', nutrition: '', sold_count: '0', stock_qty: '0',
}
    ...
    <div className="admin-page">
      ...
      <form onSubmit={handleSubmit} className="admin-form">
        ...
        <div className="form-row">
          <div className="form-group" style={{ flex: 1 }}>
            <label>Giá (VND)</label>
            <input type="number" value={form.price} onChange={e => set('price', e.target.value)} min={0} placeholder="VD: 33000" />
          </div>
          <div className="form-group" style={{ flex: 1 }}>
            <label>Giá sale (VND)</label>
            <input type="number" value={form.price_sale} onChange={e => set('price_sale', e.target.value)} min={0} placeholder="VD: 28000" />
          </div>
          <div className="form-group" style={{ flex: 1 }}>
            <label>Đơn vị</label>
            <input type="text" value={form.unit} onChange={e => set('unit', e.target.value)} placeholder="VD: bó, kg, túi 300g, combo" />
          </div>
          <div className="form-group" style={{ flex: 1 }}>
            <label>Badge</label>
        ...
        <div className="form-group">
          <label>Đặc điểm (mỗi dòng 1 nhãn — hiển thị badge ở trang chi tiết, vd: Hữu cơ, VietGAP, Không thuốc BVTV)</label>
          <textarea rows={3} value={linesToText(form.tags)} onChange={e => set('tags', textToLines(e.target.value))} placeholder={'Hữu cơ\nVietGAP\nKhông thuốc BVTV'} />
        </div>

        <div className="form-group">
          <label>Thông tin dinh dưỡng (mỗi dòng "Tên:Giá trị" — hiển thị ở tab "Thông tin dinh dưỡng")</label>
          <textarea rows={4} value={nutritionToText(form.nutrition)} onChange={e => set('nutrition', textToNutrition(e.target.value))} placeholder={'Năng lượng:23 kcal / 100g\nVitamin C:55% nhu cầu hàng ngày'} />
        </div>

        <div className="form-group">
          <label>Màu sắc (tông màu nông sản — dùng cho bộ lọc "Màu sắc" ở trang Sản phẩm)</label>

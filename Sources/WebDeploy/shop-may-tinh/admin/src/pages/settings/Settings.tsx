  ...
  return (
    <div className="admin-page">
      ...
      <form onSubmit={handleSave} className="admin-form" style={{ marginTop: 24, maxWidth: 760 }}>
        ...
        {activeTab === 'general' && <>
          <Field label="Tên cửa hàng" name="site_name" value={val('site_name')} onChange={set} placeholder="NovaTech" />
          <Field label="Slogan" name="site_slogan" value={val('site_slogan')} onChange={set} placeholder="Laptop, PC Gaming & Linh Kiện Máy Tính Chính Hãng" />
          ...
          <Field label="Giờ mở cửa" name="working_hours" value={val('working_hours')} onChange={set} placeholder="8:00 – 21:00 · Tất cả các ngày" />
        ...
        {activeTab === 'home' && <>
          <p style={{ color: 'var(--text-3)', fontSize: 13, marginBottom: 16 }}>
            Nội dung trang chủ. Ảnh nền Hero (slider crossfade) quản lý tại menu "Hero Slides" — các trường dưới đây chỉ là phần chữ phủ lên trên.
          </p>
          <p style={{ fontWeight: 600, fontSize: 13, margin: '4px 0 8px' }}>Hero</p>
          <Field label="Badge (dòng nhỏ phía trên tiêu đề)" name="hero_badge" value={val('hero_badge')} onChange={set} placeholder="Ưu đãi tháng 7 — Giảm đến 20%" />
          <div className="form-row">
            <Field label="Tiêu đề — đoạn đầu" name="hero_title_pre" value={val('hero_title_pre')} onChange={set} placeholder="Dàn máy" />
            <Field label="Tiêu đề — đoạn nhấn mạnh" name="hero_title_strong" value={val('hero_title_strong')} onChange={set} placeholder="hiệu năng đỉnh cao" />
          </div>
          <Field label="Tiêu đề — dòng dưới" name="hero_title_post" value={val('hero_title_post')} onChange={set} placeholder="cho công việc & giải trí" />
          <Field label="Mô tả" name="hero_desc" value={val('hero_desc')} onChange={set} type="textarea" />
          <div className="form-row">
            <Field label="Trust item 1" name="hero_trust1" value={val('hero_trust1')} onChange={set} placeholder="Bảo hành chính hãng 24 tháng" />
            <Field label="Trust item 2" name="hero_trust2" value={val('hero_trust2')} onChange={set} placeholder="Trả góp 0% lãi suất" />
            <Field label="Trust item 3" name="hero_trust3" value={val('hero_trust3')} onChange={set} placeholder="Giao hàng toàn quốc" />
          </div>

          <p style={{ fontWeight: 600, fontSize: 13, margin: '20px 0 8px' }}>Cam kết với khách hàng (4 mục)</p>
          {[1, 2, 3, 4].map(i => (
          ...
          <Field label="Tiêu đề" name="newsletter_title" value={val('newsletter_title')} onChange={set} placeholder="Đăng ký nhận tin công nghệ" />
          <Field label="Mô tả" name="newsletter_desc" value={val('newsletter_desc')} onChange={set} placeholder="Nhận thông báo sản phẩm mới, ưu đãi độc quyền..." />
        </>}

        {activeTab === 'reviews' && <>
          <p style={{ color: 'var(--text-3)', fontSize: 13, marginBottom: 16 }}>4 đánh giá hiển thị ở section "Khách hàng nói gì" trên trang chủ.</p>
          <div className="form-row">
            <Field label="Điểm trung bình" name="reviews_avg" value={val('reviews_avg')} onChange={set} placeholder="4.8" />
            <Field label="Tổng số đánh giá" name="reviews_count" value={val('reviews_count')} onChange={set} placeholder="1850" />
          </div>
          {[1, 2, 3, 4].map(i => (
            <div key={i} style={{ padding: '12px 0', borderTop: '1px solid var(--border)', marginTop: 8 }}>
              ...
              <div className="form-row">
                ...
                <Field label="Địa điểm" name={`review${i}_location`} value={val(`review${i}_location`)} onChange={set} placeholder="TP.HCM" />
        ...
        {activeTab === 'seo' && <>
          <Field label="Meta Title" name="meta_title" value={val('meta_title')} onChange={set} placeholder="NovaTech — Laptop, PC Gaming & Linh Kiện Máy Tính Chính Hãng" />
        ...
        {activeTab === 'social' && <>
          <Field label="Facebook URL" name="facebook" value={val('facebook')} onChange={set} placeholder="https://www.facebook.com/novatech.vn" />
          <Field label="Instagram URL" name="instagram" value={val('instagram')} onChange={set} placeholder="https://www.instagram.com/novatech.vn" />
          <Field label="YouTube URL" name="youtube" value={val('youtube')} onChange={set} placeholder="https://www.youtube.com/@novatech.vn" />
        ...
        {activeTab === 'contact' && <>
          <Field label="Email liên hệ" name="site_email" value={val('site_email')} onChange={set} type="email" placeholder="cskh@novatech.vn" />
          <Field label="Số điện thoại" name="site_phone" value={val('site_phone')} onChange={set} placeholder="0900 123 456" />
          <Field label="Địa chỉ" name="site_address" value={val('site_address')} onChange={set} placeholder="72 Nguyễn Văn Linh, Quận 7, TP.HCM" />
        ...
        {activeTab === 'smtp' && <>
          ...
          <Field label="From Email" name="smtp_from" value={val('smtp_from')} onChange={set} placeholder="no-reply@novatech.vn" />
          <Field label="From Name" name="smtp_from_name" value={val('smtp_from_name')} onChange={set} placeholder="NovaTech" />
        </>}
        ...
        {activeTab === 'cloudinary' && <>
          ...
          <Field label="Thư mục lưu ảnh" name="cloudinary_folder" value={val('cloudinary_folder')} onChange={set} placeholder="novatech" />
        </>}

...
class ShopPublicController {
    ...
    public function products(): void {
        ...
        // AI: cột riêng shop-dong-ho (đồng hồ) — brand/material/style multi-value (comma), limited bool
        ...
        $styles      = array_values(array_filter(array_map('trim', explode(',', $_GET['style'] ?? ''))));
        $limitedOnly = ($_GET['limited'] ?? '') === '1';
        ...
        if ($styles) {
            $ph = implode(',', array_fill(0, count($styles), '?'));
            $where[] = "p.style IN ($ph)";
            array_push($params, ...$styles);
        }
        if ($limitedOnly) { $where[] = "p.limited = 1"; }
    ...
    public function sitemap(): void {
        ...
        $staticRoutes = ['/', '/san-pham', '/bo-suu-tap', '/ve-chung-toi', '/lien-he', '/gio-hang', '/chinh-sach-bao-mat', '/dieu-khoan'];

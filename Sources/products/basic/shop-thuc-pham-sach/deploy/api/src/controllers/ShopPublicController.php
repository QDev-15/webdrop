...
class ShopPublicController {
    ...
    public function products(): void {
        ...
        $certs       = array_values(array_filter(array_map('trim', explode(',', $_GET['certs'] ?? ''))));
        ...
        // Chứng nhận (cột riêng shop-thuc-pham-sach — VietGAP/Organic/GlobalGAP, xem schema.sql)
        if ($certs) {
            $clauses = [];
            foreach ($certs as $c) { $clauses[] = "p.certs LIKE ?"; $params[] = '%' . $c . '%'; }
            $where[] = '(' . implode(' OR ', $clauses) . ')';
        }
    ...
    //    của System DB webdrop.store, xem CLAUDE.md mục "Module Phiếu giảm giá") ──

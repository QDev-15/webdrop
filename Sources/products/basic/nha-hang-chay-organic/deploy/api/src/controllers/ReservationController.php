...
class ReservationController {
    ...
    public function store(array $p): void {
        ...
        $id = $this->db->execute(
            "INSERT INTO reservations (name, phone, email, date, time, guests, occasion, note, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
    ...
    public function update(array $p): void {
        ...
        $this->db->execute(
            "UPDATE reservations SET name=?, phone=?, email=?, date=?, time=?, guests=?, occasion=?, note=?, status=? WHERE id=?",
            [
                $b['name'] ?? '', $b['phone'] ?? '', $b['email'] ?? '',
                $b['date'] ?? '', $b['time'] ?? '', (int)($b['guests'] ?? 2),
                $b['occasion'] ?? '', $b['note'] ?? '',

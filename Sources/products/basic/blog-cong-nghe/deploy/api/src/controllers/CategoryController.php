...
class CategoryController {
    ...
    public function store(array $p): void {
        ...
        $b = bodyJson();
        $name = trim($b['name'] ?? '');
        if (!$name) { Response::error('Tên chuyên mục không được để trống.'); return; }
        $slug = !empty($b['slug']) ? slugify($b['slug']) : slugify($name);
        $id = $this->db->execute(
            "INSERT INTO post_categories (name, slug, icon, sort_order) VALUES (?, ?, ?, ?)",
            [$name, $slug, $b['icon'] ?? '', (int)($b['sort_order'] ?? 0)]
        );
    ...
    public function update(array $p): void {
        ...
        $slug = !empty($b['slug']) ? slugify($b['slug']) : slugify($name);
        $this->db->execute(
            "UPDATE post_categories SET name=?, slug=?, icon=?, sort_order=? WHERE id=?",
            [$name, $slug, $b['icon'] ?? '', (int)($b['sort_order'] ?? 0), $p['id']]
        );

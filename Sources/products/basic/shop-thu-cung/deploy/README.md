# Pet Haus — shop-thu-cung (Website Deploy)

Website cửa hàng thú cưng chính hãng (React SPA + React Admin SPA + PHP API + SQLite).

## Cấu trúc

```
shop-thu-cung/
├── website/    React SPA — trang chủ, bộ sưu tập, khuyến mãi, giới thiệu, liên hệ, giỏ hàng, thanh toán...
...
Kết quả build nằm ở `../_output-deploy/` (cùng cấp với thư mục `shop-thu-cung/`).
...
2. Mở `api/config.php` trên server, sửa `APP_URL` thành domain thật (không có dấu `/` cuối), ví dụ:
   ```php
   define('APP_URL', 'https://pethaus.vn');
   ```
...
9. Vào **Cài đặt → Thông tin chung** để cập nhật tên cửa hàng, số điện thoại, địa chỉ, giờ mở cửa, số Zalo, bản đồ Google Maps.
...
- Database SQLite tại `api/database/shop-thu-cung.db` — tự động khởi tạo + seed dữ liệu mẫu (5 danh mục, 42 sản phẩm thú cưng, 3 đánh giá khách hàng) khi chạy lần đầu.
- Trang chủ (`/`) hiển thị toàn bộ catalog sản phẩm với toolbar lọc ngang (Danh mục/Khoảng giá/Loại thú cưng/Kích cỡ/Thương hiệu) — không có trang `/san-pham` riêng, đúng thiết kế Mode A Catalog-Unified của template gốc.

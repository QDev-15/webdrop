...
class ProductController {
    ...
    private const BASE_FIELDS = [
        ...
        // AI thêm cột riêng — schema.sql có thêm material/room/sold cho site này
        'material' => 's', 'room' => 's', 'sold' => 'i',
    ];
    ...
    public function index(): void {
        ...
        $rows  = $this->db->query(
            "SELECT p.*, pc.name as category_name, c.name as collection_name
             FROM products p
    ...
    public function show(array $params): void {
        ...
        $row = $this->db->queryOne(
            "SELECT p.*, pc.name as category_name, c.name as collection_name FROM products p
             LEFT JOIN product_categories pc ON pc.id = p.category_id
             LEFT JOIN collections c ON c.id = p.collection_id
             WHERE p.id = ?",
            [$id]
    ...
    public function store(): void {
        ...
        $cols   = ['category_id', 'collection_id'];
        $marks  = ['?', '?'];
        $values = [$categoryId, $collectionId];
    ...
    public function update(array $params): void {
        ...
        $categoryId = isset($data['category_id'])
            ...
            : $row['category_id'];
        $collectionId = isset($data['collection_id'])
            ? ($data['collection_id'] ? (int)$data['collection_id'] : null)
            : $row['collection_id'];

        $sets   = ['category_id = ?', 'collection_id = ?'];
        $values = [$categoryId, $collectionId];
        foreach (self::BASE_FIELDS as $field => $type) {
            $sets[]   = "$field = ?";

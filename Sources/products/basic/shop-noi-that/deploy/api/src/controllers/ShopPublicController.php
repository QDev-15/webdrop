...
class ShopPublicController {
    ...
    public function products(): void {
        ...
        // AI (shop-noi-that): thêm facet Chất liệu / Không gian / Bộ sưu tập — khớp dropdown
        // "Chất liệu"/"Không gian" + trang bo-suu-tap.html của template gốc.
        $materials      = array_values(array_filter(array_map('trim', explode(',', $_GET['material'] ?? ''))));
        $rooms          = array_values(array_filter(array_map('trim', explode(',', $_GET['room'] ?? ''))));
        ...
        if ($rooms) {
            ...
            $where[] = "p.room IN ($ph)";
            array_push($params, ...$rooms);
        }
        if ($collectionSlug !== '') {
            $where[] = "p.collection_id = (SELECT id FROM collections WHERE slug = ?)";
    ...
    public function sitemap(): void {
        ...
        // Trang chủ ("/") đã là catalog đầy đủ (bám đúng index.html gốc — không có san-pham.html riêng).

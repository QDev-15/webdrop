...
class ServiceCategoryController {
    ...
    public function store(): void {
        ...
        if (!$name) Response::error('Tên danh mục không được để trống.', 422);

        $this->db->execute(
            "INSERT INTO service_categories (name, icon, sort_order) VALUES (?,?,?)",
            [$name, $icon, $sortOrder]
        );
        Response::json(['id' => (int)$this->db->lastInsertId(), 'message' => 'Đã thêm danh mục.']);
    }
    ...
    public function update(array $params): void {
        ...
        Response::json(['message' => 'Đã cập nhật danh mục.']);
    }

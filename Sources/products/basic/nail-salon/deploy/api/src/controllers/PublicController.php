...
class PublicController {
    ...
    public function submitContact(): void {
        ...
        Response::json(['message' => 'Đã gửi thành công!']);
    }
    ...
    public function submitBooking(): void {
        ...
        $technician = trim($body['technician'] ?? '');
        $date       = trim($body['date']       ?? '');
        $time       = trim($body['time']       ?? '');
        $note       = trim($body['note']       ?? '');
        ...
        $this->db->execute(
            "INSERT INTO bookings (name, phone, service, technician, date, time, note) VALUES (?,?,?,?,?,?,?)",
            [$name, $phone, $service, $technician, $date, $time, $note]
        );

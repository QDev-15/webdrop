...
class TestimonialController {
    ...
    public function store(array $p): void {
        ...
        $this->db->execute(
            "INSERT INTO testimonials (customer_name, avatar, service_name, rating, content, sort_order, status) VALUES (?,?,?,?,?,?,?)",
            [
                $name,
                trim($b['avatar'] ?? ''),
                trim($b['service_name'] ?? ''),
        ...
        Response::json($this->db->queryOne("SELECT * FROM testimonials WHERE id = ?", [$this->db->lastInsertId()]), 201);
    }
    ...
    public function update(array $p): void {
        ...
        $name    = trim($b['customer_name'] ?? $row['customer_name']);
        $content = trim($b['content'] ?? $row['content']);
        if (!$name || !$content) {
            Response::error('Tên khách hàng và nội dung không được để trống.'); return;
        }
        $this->db->execute(
            "UPDATE testimonials SET customer_name=?, avatar=?, service_name=?, rating=?, content=?, sort_order=?, status=? WHERE id=?",
            [
                $name,
                trim($b['avatar'] ?? $row['avatar']),
                trim($b['service_name'] ?? $row['service_name']),
                (int)($b['rating'] ?? $row['rating']),
                $content,

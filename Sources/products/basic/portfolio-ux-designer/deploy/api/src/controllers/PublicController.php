...
class PublicController {
    ...
    public function submitContact(array $p): void {
        ...
        if (!$name || !$email) { Response::error('Họ tên và email không được để trống.'); return; }

        // Gộp loại dự án + ngân sách vào message vì bảng contacts core chỉ có
        // name/email/phone/subject/message/status (không mở rộng cột).
        $projectType = trim($b['project_type'] ?? '');
        $budget = trim($b['budget'] ?? '');
        $extra = [];
        if ($projectType) $extra[] = "Loại dự án: $projectType";
        if ($budget) $extra[] = "Ngân sách dự kiến: $budget";
        $fullMessage = trim((implode("\n", $extra) ? implode("\n", $extra) . "\n\n" : '') . $message);

        $this->db->execute(
            "INSERT INTO contacts (name, email, phone, subject, message, status) VALUES (?, ?, ?, ?, ?, 'new')",
            [$name, $email, trim($b['phone'] ?? ''), 'Yêu cầu tư vấn thiết kế UX/UI', $fullMessage]
        );
        Response::json(['ok' => true, 'message' => 'Cảm ơn bạn đã gửi thông tin! Tôi sẽ phản hồi trong vòng 24 giờ.'], 201);
    }

# Tuấn Đặng Studio — Website Deploy (portfolio-kien-truc-su)

Website portfolio kiến trúc sư độc lập — React SPA (frontend + admin) + PHP API + SQLite.

## 1. Build

```bash
cd Sources/WebDeploy/portfolio-kien-truc-su
# Windows
build.bat
# Linux/Mac
bash build.sh
...
2. Mở `api/config.php` trên server, sửa `APP_URL` thành domain thật của bạn, ví dụ:
   ```php
   define('APP_URL', 'https://tuandang.studio');
   ```
...
| Hero Slides | Carousel 4 ảnh trang chủ — mỗi slide có nhãn nhỏ, tiêu đề (dùng `*từ*` để in nghiêng), mô tả, nút chính, ảnh |
| Dự án (Case Study) | Danh sách công trình — mỗi dự án có thể bật thêm phần Case Study đầy đủ (bối cảnh/thách thức, giải pháp thiết kế, gallery, kết quả, đánh giá) để có trang chi tiết riêng |
| Đánh giá | Nhận xét khách hàng — hiển thị ở trang chủ (teaser) và trang Về tôi |
| Bảng giá | 3 gói dịch vụ (Tư Vấn Thiết Kế / Thiết Kế + Giám Sát / Thi Công Trọn Gói) |
...
- Font: Cormorant Garamond (heading) + DM Sans (body), qua Bunny Fonts

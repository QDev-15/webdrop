...
class GalleryController {
    ...
    public function index(array $p): void {
        Auth::require();
        $items = $this->db->query("SELECT * FROM gallery_items ORDER BY sort_order, id");
        Response::json($items);
    }
    ...
    public function store(array $p): void {
        Auth::require();
        $b = bodyJson();
        if (empty($b['image'])) { Response::error('URL ảnh là bắt buộc.'); return; }
        $id = $this->db->execute(
            "INSERT INTO gallery_items (title, description, image, category, sort_order, status) VALUES (?, ?, ?, ?, ?, ?)",
            [$b['title'] ?? '', $b['description'] ?? '', $b['image'], $b['category'] ?? '', (int)($b['sort_order'] ?? 0), $b['status'] ?? 'published']
        );
    ...
    public function update(array $p): void {
        ...
        $this->db->execute(
            ...
            [$b['title'] ?? '', $b['description'] ?? '', $b['image'], $b['category'] ?? '', (int)($b['sort_order'] ?? 0), $b['status'] ?? 'published', $p['id']]
        );
    ...
    public function reorder(array $p): void {
        ...
        foreach (($b['ids'] ?? []) as $i => $id) {
            $this->db->execute("UPDATE gallery_items SET sort_order = ? WHERE id = ?", [$i, $id]);
        }
        Response::json(['ok' => true]);
    }
}

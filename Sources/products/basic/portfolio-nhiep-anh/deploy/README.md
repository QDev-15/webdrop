# Đăng Photography — Website Deploy (portfolio-nhiep-anh)

Website portfolio nhiếp ảnh gia cưới — React SPA (frontend + admin) + PHP API + SQLite.

## 1. Build

```bash
cd Sources/WebDeploy/portfolio-nhiep-anh
# Windows
build.bat
# Linux/Mac
bash build.sh
...
2. Mở `api/config.php` trên server, sửa `APP_URL` thành domain thật của bạn, ví dụ:
   ```php
   define('APP_URL', 'https://dangphotography.vn');
   ```
...
| Hero Slides | Carousel 4 ảnh trang chủ — mỗi slide có nhãn nhỏ, tiêu đề (dùng `*từ*` để in nghiêng), mô tả, nút phụ, 5 ảnh lưới |
| Dự án (Case Study) | Danh sách lễ cưới đã ghi hình — mỗi dự án có thể bật thêm phần Case Study đầy đủ (thách thức/giải pháp/gallery/số liệu/đánh giá) để có trang chi tiết riêng |
| Đánh giá | Nhận xét khách hàng — hiển thị ở trang chủ (teaser) và trang Về tôi |
| Bảng giá | 3 gói dịch vụ (Cơ Bản / Nâng Cao / Trọn Gói) |
...
- Font: Fraunces (heading) + DM Sans (body), qua Bunny Fonts

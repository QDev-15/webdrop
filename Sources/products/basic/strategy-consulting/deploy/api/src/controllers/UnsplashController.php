...
class UnsplashController {
    ...
    public function trackDownload(array $p): void {
        ...
        $isValidUnsplashUrl = $downloadLocation
            && parse_url($downloadLocation, PHP_URL_SCHEME) === 'https'
            && parse_url($downloadLocation, PHP_URL_HOST) === 'api.unsplash.com';

        if ($isValidUnsplashUrl && ($accessKey = $this->getAccessKey())) {

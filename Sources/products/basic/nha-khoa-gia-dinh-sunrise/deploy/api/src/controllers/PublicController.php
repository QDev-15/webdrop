...
class PublicController {
    ...
    public function submitBooking(array $p = []): void {
        ...
        $this->db->execute(
            "INSERT INTO bookings (fullname, phone, email, service, member_count, date, time, note, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'new')",
            [
                ...
                htmlspecialchars(trim($b['member_count'] ?? '1'),      ENT_QUOTES, 'UTF-8'),
                htmlspecialchars(trim($b['date']         ?? ''),       ENT_QUOTES, 'UTF-8'),
                htmlspecialchars(trim($b['time']         ?? ''),       ENT_QUOTES, 'UTF-8'),
                htmlspecialchars(trim($b['note']         ?? ''),       ENT_QUOTES, 'UTF-8'),
            ]
        );
        Response::json(['message' => 'Dat lich thanh cong. Sunrise se lien he xac nhan trong 30 phut.'], 201);
    }
    ...
    public function submitContact(array $p = []): void {
        ...
        Response::json(['message' => 'Cam on ban da lien he. Sunrise se phan hoi trong 24 gio lam viec.'], 201);
    }
    ...
    public function sitemap(array $p = []): void {
        ...
        $staticRoutes = ['/', '/dich-vu', '/bac-si', '/dat-lich', '/lien-he'];

...
class ShopPublicController {
    ...
    public function products(): void {
        ...
        // ▼ EXTRA FILTERS (shop-quan-ao-ami — AMI Fashion): sizes = san-pham.html checkbox Size,
        // theme = 4 section trang chủ (hang-moi/ban-chay/giam-gia) + ?theme= trên trang Sản phẩm.
        ...
        if ($sizes) {
            // p.sizes lưu dạng padded "|XS|S|M|L|XL|" — LIKE '%|S|%' tránh khớp nhầm "S" vào "XS"
            $clauses = [];
            foreach ($sizes as $sz) { $clauses[] = "p.sizes LIKE ?"; $params[] = '%|' . $sz . '|%'; }
            $where[] = '(' . implode(' OR ', $clauses) . ')';
        }
        if ($themes) {
            $clauses = [];

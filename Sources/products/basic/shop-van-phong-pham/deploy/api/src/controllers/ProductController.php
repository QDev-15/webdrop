...
class ProductController {
    ...
    private const BASE_FIELDS = [
        ...
        // ▼ Extra fields cho shop-van-phong-pham (khớp cột thêm trong schema.sql)
    ...
    public function store(): void {
        ...
        foreach (self::BASE_FIELDS as $field => $type) {
            $cols[]   = $field;
            $marks[]  = '?';
            $default  = $field === 'status' ? 'published' : ($field === 'rating' ? 5 : ($field === 'in_stock' ? 1 : ($field === 'price_sale' ? null : '')));

# Lam Trúc Illustration — Website Deploy (portfolio-minh-hoa)

Website portfolio họa sĩ minh họa (character design, sách tranh thiếu nhi, editorial illustration) — React SPA (frontend + admin) + PHP API + SQLite.

## 1. Build

```bash
cd Sources/WebDeploy/portfolio-minh-hoa
# Windows
build.bat
# Linux/Mac
bash build.sh
...
2. Mở `api/config.php` trên server, sửa `APP_URL` thành domain thật của bạn, ví dụ:
   ```php
   define('APP_URL', 'https://lamtruc-illustration.vn');
   ```
...
| Hero Slides | Carousel 4 ảnh trang chủ — mỗi slide có nhãn nhỏ, tiêu đề (dùng `*từ*` để tô màu nhấn), mô tả, ảnh lệch phải, 2 nút (primary + outline) |
| Dự án (Case Study) | Danh sách 12 dự án minh họa — mỗi dự án có thể bật thêm phần Case Study đầy đủ (thách thức/quy trình/gallery/kết quả/đánh giá) để có trang chi tiết riêng |
| Đánh giá | Nhận xét khách hàng — hiển thị ở trang chủ (teaser) và trang Về tôi |
| Bảng giá | 3 gói dịch vụ (Minh Họa Đơn / Bộ Nhân Vật / Trọn Dự Án Sách) |
| FAQ | Câu hỏi thường gặp — hiển thị ở trang Dịch vụ |
| Liên hệ | Danh sách yêu cầu hợp tác gửi từ form Liên hệ |
...
- Font: Space Grotesk, qua Bunny Fonts

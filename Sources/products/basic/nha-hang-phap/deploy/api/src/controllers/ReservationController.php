...
class ReservationController {
    ...
    public function store(array $p): void {
        ...
        if (empty($b['name']) || empty($b['phone']) || empty($b['date']) || empty($b['time'])) {
            Response::error('Vui lòng điền đầy đủ thông tin bắt buộc.'); return;
        }
        $id = $this->db->execute(
            "INSERT INTO reservations (name, phone, email, date, time, guests, occasion, menu_pkg, note, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
                $b['name'], $b['phone'], $b['email'] ?? '',
                $b['date'], $b['time'], (int)($b['guests'] ?? 2),
                $b['occasion'] ?? '', $b['menu_pkg'] ?? '', $b['note'] ?? '',
                $b['status'] ?? 'pending',
            ]
        );
    ...
    public function update(array $p): void {
        ...
        $this->db->execute(
            "UPDATE reservations SET name=?, phone=?, email=?, date=?, time=?, guests=?, occasion=?, menu_pkg=?, note=?, status=? WHERE id=?",
            [
                $b['name'] ?? '', $b['phone'] ?? '', $b['email'] ?? '',
                $b['date'] ?? '', $b['time'] ?? '', (int)($b['guests'] ?? 2),
                $b['occasion'] ?? '', $b['menu_pkg'] ?? '', $b['note'] ?? '',

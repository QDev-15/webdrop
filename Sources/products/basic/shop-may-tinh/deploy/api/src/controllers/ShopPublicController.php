...
class ShopPublicController {
    ...
    // ── Coupon extension (bảng `coupons` riêng, độc lập với hệ discount của System DB webdrop.store) ──
    ...
    public function createOrder(): void {
        ...
        $orderId = $this->db->execute(
            "INSERT INTO orders (order_code, customer_name, phone, email, address, note, subtotal, shipping_fee, discount, coupon_code, total, payment_method, payment_status, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')",
            [$orderCode, $name, $phone, $email, $address, $note, $subtotal, $shippingFee, $discount, $coupon ? $coupon['code'] : '', $total, $paymentMethod, $paymentStatus]
        );
    ...
    // Sitemap XML động — route GET /sitemap.xml (đăng ký ở bootstrap.php). Ghi đè Content-Type
    // JSON mặc định của Router. $staticRoutes khớp đúng danh sách route thật trong website/src/App.tsx
    // của shop-may-tinh (có /dich-vu, /khuyen-mai riêng — không có /ve-chung-toi/bo-suu-tap).
    public function sitemap(): void {
        ...
        $staticRoutes = ['/', '/san-pham', '/dich-vu', '/khuyen-mai', '/lien-he', '/chinh-sach-bao-mat', '/dieu-khoan'];

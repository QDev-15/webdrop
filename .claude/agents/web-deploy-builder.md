...
23. **[P0] Phân trang (pagination) bắt buộc cho MỌI danh sách sản phẩm — cả website (public storefront) LẪN admin panel, không ngoại lệ.** Không được tải toàn bộ sản phẩm một lần ở bất kỳ nơi nào hiển thị danh sách sản phẩm.
    ...
    - **Admin — tìm kiếm (thêm 2026-08-07, đi kèm bắt buộc với phân trang admin ở trên)**: `ProductController::index()` nhận thêm `$_GET['q']`, lọc `(p.name LIKE ? OR p.description LIKE ?)` với `substr($search,0,100)` chặn input quá dài (đúng kỹ thuật đã dùng ở `ShopPublicController::products()` phía public — dùng lại nguyên xi, không phát minh cách khác) — áp dụng WHERE này cho CẢ câu `COUNT(*)` lẫn câu `SELECT` chính (tổng số phải khớp đúng kết quả đã lọc, không phải tổng toàn bộ bảng). `ProductList.tsx` thêm ô `<input type="search">` (class `form-control` có sẵn trong `admin.css`) phía trên bảng, debounce 400ms, mỗi lần đổi từ khóa reset `page` về 1, gọi `getPaged` kèm `&q=`. Empty-state phải phân biệt "Chưa có sản phẩm nào" (chưa từng thêm) và "Không tìm thấy sản phẩm nào khớp "X"" (có sản phẩm nhưng search không khớp) — không dùng chung 1 câu.

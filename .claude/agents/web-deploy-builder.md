...
9. **`config.php` phải có đầy đủ** — build script copy vào deploy/.
...
26. **Reveal animation với async data dùng `setTimeout(fn, 0)` + re-observe `:not(.visible)`**:
    ...
    ```
27. **`build.mjs` strip BOM** khỏi PHP files sau khi copy vào deploy/api/ — đã có trong scaffold.
...
Hướng dẫn: upload deploy/ → sửa `APP_URL` trong `api/config.php` → kiểm tra `https://domain.vn/api/health` (pdo_sqlite=true, db_dir=writable) → chmod `api/database/` và `api/uploads/` → đăng nhập admin `sysadmin@admin.com` / `123456` → đổi mật khẩu ngay.

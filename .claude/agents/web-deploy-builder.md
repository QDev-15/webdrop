...
23. **[P0] Phân trang (pagination) bắt buộc cho trang danh sách sản phẩm** — template có component `sb-pagination` (nút trước/sau + số trang + dấu "…"). Không được tải toàn bộ sản phẩm một lần:
    - API: `GET /public/products?page=1&per_page=12&category_id=&sort=&min_price=&max_price=` — vẫn trả **array thuần** (đúng chuẩn PublicController của dự án, xem Bước 4 § "PublicController — Luôn trả array thuần"); tổng số bản ghi để tính số trang trả qua HTTP header `X-Total-Count` — KHÔNG bọc object

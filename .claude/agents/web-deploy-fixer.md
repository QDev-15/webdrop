...
- [ ] **[Thêm 2026-08-07] `admin/src/pages/products/ProductList.tsx` cũng PHẢI phân trang** — quy tắc "danh sách sản phẩm phải phân trang" áp dụng cho CẢ website LẪN admin, không chỉ riêng public storefront. Kiểm tra: `ProductController::index()` (admin, PHP) có nhận `page`/`per_page` + `LIMIT/OFFSET` + `header('X-Total-Count: ...')` không, hay đang `SELECT p.*` không giới hạn rồi trả hết 1 lần. `admin/src/api/client.ts` có method `getPaged` (đọc header, không phải chỉ `api.get` thường) không. Nếu thiếu — đây là bug cần fix giống hệt cách xử lý bên website (xem 2 mục checklist ngay trên), không phải P2 tùy chọn. **⚠️ Cảnh báo quy trình**: đây là loại bug `tsc`/`php -l` KHÔNG bắt được (frontend gọi đúng API, backend không đáp ứng đúng — cả 2 phía "biên dịch sạch") — từng có agent tự báo "PHP đã có sẵn phân trang, không cần sửa" nhưng thực ra đọc nhầm file `ShopPublicController.php` (public, đã có sẵn) sang tưởng luôn cả `ProductController.php` (admin, chưa có) — PHẢI tự đọc trực tiếp `index()` xác nhận có đúng `LIMIT`/`header('X-Total-Count'...)` chưa, không tin báo cáo "đã có sẵn" của lần kiểm tra trước mà không tự verify lại.
- [ ] **[Thêm 2026-08-07] `admin/src/pages/products/ProductList.tsx` cũng PHẢI có ô tìm kiếm** — `ProductController::index()` nhận `$_GET['q']`, lọc `(p.name LIKE ? OR p.description LIKE ?)` (đúng kỹ thuật `ShopPublicController::products()` phía public đã dùng), áp dụng WHERE cho cả `COUNT(*)` lẫn `SELECT` chính. `ProductList.tsx` có `<input type="search">` debounce 400ms, reset `page` về 1 khi đổi từ khóa. Thiếu → cần fix.

...
- **Blog nội dung**: chiến lược nội dung là "Kiến thức/Cẩm nang" (case study từ WebDeploy projects + hướng dẫn theo ngách + so sánh template nội bộ) — KHÔNG đăng lại/dịch tin tức từ nguồn khác để tránh vi phạm bản quyền (Luật SHTT + Nghị định 72/2013). Bài đầu tiên (category `cam-nang`, slug `website-nha-khoa-tang-dat-lich`) seed qua `prisma/seed.ts` (upsert Category + Post, `createdBy: admin.id`) — nội dung `content` viết theo đúng convention parser `renderMarkdown()` trong `blog/[slug]/page.tsx`: dòng đứng riêng dạng `**Text**` → H2, `- ` đầu dòng → list item, `**bold**` inline → strong.

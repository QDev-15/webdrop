...
│   ├── WebDeploy/                  ← Generated by web-deploy-builder agent
│   │   └── [template-slug]/        ← Website deploy cho từng template
│   │       ├── website/            → React SPA public site (ported từ template)
│   │       ├── admin/              → React SPA admin (webdrop design system)
│   │       ├── api/                → PHP backend + SQLite
│   │       ├── build.bat           → Windows build → deploy/
│   │       └── build.sh            → Linux/Mac build → deploy/
│   ├── products/

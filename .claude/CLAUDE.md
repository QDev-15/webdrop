...
- [x] Shop bán hàng — **DONE** (9 templates, 7 trang mỗi template — 7 template đầu đã có bản WebDeploy đầy đủ: `shop-ban-hang/` ORGANIC-EARTH, `shop-thoi-trang/` BOLD-EDITORIAL, `shop-giay-dep/` DARK-ENERGY, `shop-quan-ao/` SOFT-PASTEL, `shop-rau-xanh/` WARM-ARTISAN, `shop-thuc-pham-sach/` FRESH-MINIMAL, `shop-tui-sach/` LUXE-DARK — xem bảng **WebDeploy Projects**; 2 template còn lại — template-only, chưa có WebDeploy: `shop-may-anh/` GEOMETRIC-MODERN, `shop-may-tinh/` GLASS-MODERN)
...
| `shop-may-tinh` | Template only | Shop máy tính & laptop | GLASS-MODERN, Indigo `#6d5ef8` + Cyan `#22d3ee` | `mt-` | Inter (unified), Nav-1 transparent→scrolled nhưng style glass (backdrop-blur, translucent) khác hẳn bản flat của `shop-thoi-trang`, Hero H7 slider crossfade 4 ảnh + dot indicator (auto-cycle + click-to-jump), glassmorphism card (`backdrop-filter:blur`, border rgba trắng), 3 blob gradient nền cố định phía sau các panel kính · **7 trang** (thêm `dich-vu.html`/`khuyen-mai.html` 2026-07-17; cart page có tính lại tạm tính/xóa sản phẩm thật qua `recalcCartTotals()`/`removeCartItem()`, giá đổi từ `[bracket]` placeholder sang số thật để tính toán được) |

...
- [x] Shop bán hàng — **DONE** (10 templates — 9 template cũ có bản WebDeploy đầy đủ + 1 template mới `shop-ami-mobile` RETRO-BOLD template-only: `shop-ban-hang/` ORGANIC-EARTH, `shop-thoi-trang/` BOLD-EDITORIAL, `shop-giay-dep/` DARK-ENERGY, `shop-quan-ao/` SOFT-PASTEL, `shop-rau-xanh/` WARM-ARTISAN, `shop-thuc-pham-sach/` FRESH-MINIMAL, `shop-tui-sach/` LUXE-DARK, `shop-may-tinh/` GLASS-MODERN, `shop-may-anh/` GEOMETRIC-MODERN, `shop-ami-mobile/` RETRO-BOLD — xem bảng **WebDeploy Projects**)
...
│       ├── shop-may-anh/           ← ✅ 1 template + WebDeploy đầy đủ (GEOMETRIC-MODERN identity — xem ghi chú kỹ thuật)
│       ├── shop-may-tinh/          ← ✅ 1 template + WebDeploy đầy đủ (GLASS-MODERN identity — xem ghi chú kỹ thuật)
│       ├── shop-ami-mobile/        ← ✅ 1 template (RETRO-BOLD identity, Mode B themed-sections, CSS prefix mb- — xem bảng tra nhanh)

...
- [x] Nhà hàng / Quán ăn / Cafe — **DONE** (10 `Restaurants/` + 6 `Cafes/`. Cả 5 template Cafe mới batch 2026-08-22 đã có WebDeploy đầy đủ, build 2026-09-07, xem ghi chú kỹ thuật)
- [x] Spa / Thẩm mỹ / Làm đẹp — **DONE** (10 templates: `Spa-Services/`)
- [x] Bất động sản — **IN PROGRESS** (3 template tĩnh: `Real-Estate/green-valley-residence` Loại hình B GLASS-MODERN, `Real-Estate/nha-dat-viet` Loại hình A CLEAN-CORPORATE (build cũ, sai ý so với mô hình marketplace — CHƯA retrofit theo Bước A đã sửa, nhưng bản WebDeploy vẫn build đúng nguyên mô hình đơn giản của bản tĩnh hiện tại, xem dưới), `Real-Estate/rao-nha` Loại hình A FRESH-MINIMAL (build đúng theo Bước A đã sửa 2026-08-23, tham khảo đúng mô hình marketplace). WebDeploy: cả 3 site đã có bản đầy đủ — `rao-nha` (kiến trúc dual-auth admin/`accounts` + ví credit/gói VIP thật, xem block ghi chú kỹ thuật bên dưới), `green-valley-residence` (build 2026-08-24, nền `company` + 7 entity BĐS mới thay Services/Team/Projects, xem `Sources/WebDeploy/green-valley-residence/`), và `nha-dat-viet` (build/verify hoàn chỉnh 2026-09-03, nền `company`, mô hình đơn giản 1 agency quản lý qua 1 admin panel — KHÔNG dual-auth/ví/VIP như `rao-nha`, xem block ghi chú kỹ thuật bên dưới). DB `Sources/system`: cả 3 đã có record + industry `real-estate`, `hasWebsite` khớp đúng thực tế trên đĩa (`green-valley-residence`/`rao-nha`/`nha-dat-viet`: true) — `nha-dat-viet` cần đồng bộ lại `hasWebsite=true` (giá trị cũ đồng bộ 2026-08-24 qua agent `db-template-sync` đã lỗi thời)
...
│       ├── Real-Estate/            ← 🔲 IN PROGRESS — 3 template (`green-valley-residence` GLASS-MODERN Loại hình B, `nha-dat-viet` CLEAN-CORPORATE Loại hình A build cũ sai ý (WebDeploy đã build/verify đầy đủ theo đúng mô hình đơn giản hiện có), `rao-nha` FRESH-MINIMAL Loại hình A build đúng marketplace, xem ghi chú kỹ thuật)
...
**`shop-dong-ho` — WebDeploy đầy đủ (2026-09-03)**: React SPA (website + admin) + PHP API + SQLite cho site "MERIDIAN — Đồng hồ chính hãng đa thương hiệu" (Identity `GLASS-MODERN` variant Deep Teal `#0d7377`, prefix `dh-`). 8 trang website (trang chủ carousel hero 4 slide + stat-bar + brand horizontal-scroll + 4 themed section Nam/Nữ/Limited/Bán chạy + bento + quy trình + testimonials + FAQ 7 câu + CTA band, `san-pham` catalog toolbar ngang (category pill + 3 dropdown checkbox Chất liệu/Thương hiệu/Phong cách + price slider + sort + pagination cổ điển + offcanvas mobile — KHÔNG dùng sidebar 5-block vì template gốc không có, đúng rule 1 template fidelity), `san-pham/:slug` chi tiết, `bo-suu-tap` 4 block tĩnh theo filter cố định, `ve-chung-toi`, `lien-he`, `gio-hang`, `/thanh-toan` checkout mới dựng theo rule 26/27) + 2 trang pháp lý. Extension schema `products` thêm 10 cột riêng ngành đồng hồ (`brand`/`material`/`style`/`warranty`/`movement`/`water_resist`/`diameter`/`sold`/`limited`/`gallery`) + bảng `testimonials` — cột `colors` (base shop schema) giữ nguyên nhưng không dùng vì template không có bộ lọc màu. 44 sản phẩm seed 1:1 từ `RAW_WATCHES` trong `products-data.js` gốc (port công thức tính `price_sale`/`badge`/`rating`/`stock`/`movement`/`water_resist`/`diameter` y hệt JS sang PHP, verify bằng smoke test khớp từng giá trị). **Bug scaffold tái diễn tự fix**: `_scaffold/types/shop/api/src/Database.php` (`class Database extends \Database`) thiếu class cha thật (bản base bị ghi đè khi scaffolder copy stub shop) — viết lại hoàn toàn thành class độc lập theo đúng pattern đã fix ở `shop-noi-that`; đồng thời fix luôn `build.mjs` bị lặp block "Build admin" + `mkdirSync`/`.gitkeep` + dòng log hướng dẫn (bug scaffold đã gặp nhiều lần ở batch Portfolio). Do harness không có Task/Agent dispatch tool, bước 8.5 chỉ thực hiện ở dạng self-check thủ công đầy đủ (PHP syntax, TS build 0 lỗi cả website/admin, prepared statements, `Auth::require()` mọi controller admin, filter nhóm `payment`/`smtp`/`cloudinary`/`integrations` khỏi `/public/settings` — verify bằng curl thật không thấy `sepay_webhook_secret`, PRAGMA FK, BOM, `/media/upload` trả 401 không phải 404, sitemap.xml đủ 44 sản phẩm + 8 route tĩnh) — smoke test thật qua PHP built-in server với router mô phỏng `.htaccess` (SPA fallback + `/api` rewrite) trên chính thư mục `_output-deploy/` đã build, xác nhận health/settings/products/orders/login/sitemap đều hoạt động đúng trên artifact triển khai thật.
...
**Cả 5 site đã có WebDeploy đầy đủ (build 2026-09-07)**: React SPA (website+admin) + PHP API + SQLite mỗi site, dùng `node scaffolder.mjs [slug] cafe`. Do lỗi rate-limit/session-limit giữa chừng khi build song song 10 site (5 cafe + 5 blog cùng đợt), một số agent bị dừng và phải resume lại — verify độc lập sau cùng bởi orchestrator (không phải tự agent báo cáo) phát hiện và tự fix 4 vấn đề còn sót (2 vấn đề đầu phát hiện lúc verify ban đầu, vấn đề 3-4 phát hiện thêm sau đó qua báo cáo người dùng — xem cuối đoạn): (1) `blog-am-thuc/website/src/utils.ts` chứa JSX (`<em>`, `<Fragment>`) nhưng đặt sai đuôi `.ts` thay vì `.tsx` — TypeScript không parse JSX trong file `.ts`, gây lỗi cú pháp cascade; đổi đuôi file, không cần sửa import (các nơi gọi đều import không kèm đuôi). (2) `cafe-rang-xay/website/src/App.tsx` + `About.tsx` + `Contact.tsx` bị bỏ dở dạng `// TODO: AI-generated` (agent dừng giữa chừng trước khi viết xong) — viết lại đầy đủ theo đúng nội dung thật từ bản tĩnh (`gioi-thieu.html`/`lien-he.html`/`khong-gian.html`), khớp đúng convention riêng của site này (`SiteProvider` nằm trong `contexts/SiteContext.tsx` import vào chứ không định nghĩa inline trong `App.tsx` như một số site khác cùng batch, `BrowserRouter` đã wrap sẵn ở `main.tsx`). Verify cả 10 site sau cùng: `php -l` sạch toàn bộ, `tsc --noEmit` + `vite build` sạch cả website/admin (cần `npm install` bổ sung cho 4 site thiếu `node_modules`: `blog-cong-nghe`, `blog-du-lich`, `blog-tai-chinh`, `cafe-rang-xay`), test migration `schema.sql` qua SQLite tạm cho cả 10 site (đủ core table `users/settings/hero_slides/contacts/media`, không lỗi cú pháp), grep xác nhận không còn thiếu `className="form-control"` trong admin. DB `Sources/system`: cả 10 slug đã cập nhật `hasWebsite: true`, đồng bộ `seed.ts`.

**(3) Phát hiện thêm sau đó qua báo cáo người dùng**: `blog-am-thuc/website/App.tsx` cũng bị bỏ dở `// TODO: AI-generated`, và cả `website/index.html` lẫn `website/src/main.tsx` (file entry — không phải file AI-fill) hoàn toàn chưa được tạo — lọt qua bước verify ban đầu vì grep lúc đó chỉ quét `*.tsx/*.ts/*.php` và chỉ chạy cho `cafe-rang-xay`, không bắt được file `.html` thiếu nội dung lẫn file bị thiếu hẳn (không tồn tại thay vì chứa TODO). Đã viết lại đủ 3 file theo đúng nội dung/route thật từ bản tĩnh (`index.html`/`chuyen-muc.html`/`cong-thuc-nau-an.html`/2 trang pháp lý) và đúng convention riêng của site (không có thư mục `pages/` — `HomePage`/`CategoryPage`/`PrivacyPage`/`TermsPage` định nghĩa inline trong `App.tsx`, khác `blog-me-va-be` dùng `pages/` riêng). Verify: `tsc --noEmit` + `vite build` sạch, grep `TODO: AI-generated` toàn bộ site (mọi phần mở rộng, không giới hạn `.ts/.tsx/.php`) không còn kết quả nào.
...
**`nha-dat-viet` — WebDeploy đầy đủ, build/verify 2026-09-03**: React SPA (website + admin) + PHP API + SQLite, nền `company` viết lại. **Cố ý port ĐÚNG NGUYÊN mô hình đơn giản của bản tĩnh hiện tại** (1 agency quản lý catalog qua 1 admin panel) — KHÔNG áp dụng kiến trúc dual-auth/ví credit/gói VIP/trang "Đăng tin" công khai của `rao-nha` (site đó đã được retrofit theo Bước A marketplace, `nha-dat-viet` thì chưa). Extension schema (append cuối `schema.sql`, core 5 bảng giữ nguyên): `agents` (6 môi giới), `properties` (42 tin đăng — cột đúng field template: `listing_type`/`property_type`/`price`/`price_unit`/`area`/`bedrooms`/`bathrooms`/`direction`/`legal_status`/`furnishing`/`district`/`street`/`lat`/`lng`/`badge`/`posted_date`/`agent_id`/`description`/`features` pipe-separated/`images` pipe-separated), `projects` (4 dự án hợp tác phân phối, KHÔNG phải chủ đầu tư — `du-an.html`), `testimonials`, `faqs` (8 câu). `PropertyController` decorate `images`/`features` thành array + nested `agent` object qua `PublicController::decorateProperty()`. Trang `PropertiesPage.tsx` port 100% đúng 5 khối filter sidebar/toolbar của template gốc (Nhu cầu pill, Loại hình + Khu vực dropdown checkbox multi-select, Khoảng giá đổi động theo Nhu cầu, Số phòng ngủ, "Thêm bộ lọc" gộp Hướng nhà+Pháp lý+Diện tích+Tình trạng nội thất) + active-filter chips + offcanvas mobile đồng bộ 2 chiều + phân trang PER_PAGE=12 + URL sync qua `history.replaceState` — không rút gọn. `PropertyDetailPage.tsx` đủ 9 mục đúng template (gallery+lightbox, quick-facts, mô tả, đặc điểm nổi bật, bản đồ Google Maps embed+tiện ích, `MortgageCalculator`/`RentEstimator` component port nguyên công thức `calcMonthlyPayment()` từ `main.js` gốc, agent card, booking form, BĐS tương tự). `App.tsx` đã đúng chuẩn `AppShell` (rule 18) gồm cả `useEffect(() => window.scrollTo(0,0), [location.pathname])`. Verify: `php -l` toàn bộ PHP 0 lỗi, không BOM, `tsc -b && vite build` cả website/admin 0 lỗi, live smoke-test qua PHP built-in server (`/health`, `/public/*` đúng 42 properties/6 agents/4 projects/3 testimonials/8 faqs/4 hero-slides, `/public/settings` không lộ `smtp`/`cloudinary`/`integrations`, login+session+`/auth/me` hoạt động, route admin trả 401 khi chưa đăng nhập, `X-Total-Count` phân trang đúng), build `_output-deploy` thật qua `build.mjs` và re-verify `/health`+`/public/properties` trên chính output đó. 0 thay đổi code cần thiết — bản build đã đạt chuẩn từ trước, task chỉ xác nhận lại toàn bộ.
...
### Bug pattern: thẻ sản phẩm — giá bị co theo độ dài tên/mô tả (fix bulk 2026-09-07)

Người dùng báo ở `shop-trang-suc`: trong lưới catalog, khối giá của mỗi thẻ nằm ngay dưới tên/mô tả nên độ cao thay đổi theo nội dung → giá các thẻ cùng hàng KHÔNG canh thẳng nhau. Đúng ra giá phải luôn nằm sát đáy thẻ bất kể mô tả dài/ngắn/không có.

**Nguyên nhân**: thẻ sản phẩm ở hầu hết template shop (cả bản tĩnh `Sources/templates/web/Shops/` lẫn bản WebDeploy) không dùng flexbox để đẩy khối giá xuống đáy — thẻ chỉ là block bình thường, các phần tử xếp chồng tự nhiên.

**Fix chuẩn (đã áp dụng cho `Sources/templates/web/Shops/shop-trang-suc/` + TOÀN BỘ 21 site `Sources/WebDeploy/shop-*/`)** — 4 khai báo CSS thêm vào `website/src/styles/template.css` (class name khác nhau tùy prefix từng site, `_prod-card`/`_card`, `_prod-thumb`/`_img-wrap`/`_img`, `_prod-info`/`_prod-body`, `_prod-footer`/`_prod-price`/`_card-price`):
1. Thẻ (`.xx-prod-card`): `display:flex; flex-direction:column; height:100%` — grid item `align-items:stretch` mặc định làm thẻ giãn bằng chiều cao hàng.
2. Ảnh (`.xx-prod-thumb`): `flex-shrink:0` — giữ tỉ lệ, không bị bóp khi body cao.
3. Vùng nội dung (`.xx-prod-body`/`_info`): `flex:1 1 auto; display:flex; flex-direction:column`.
4. Khối giá/footer (phần tử cuối cùng nên bám đáy): `margin-top:auto`.

Vài site đã có sẵn cơ chế đúng (vd `shop-dong-ho`/`shop-ruou-vang` có `margin-top:auto` từ lúc build; `shop-van-phong-pham` dùng `.vp-prod-name{flex:1}` để đẩy giá xuống) — chỉ bổ sung phần thiếu. `shop-do-choi` trang chủ dùng thẻ inline-style toàn bộ → thêm 4 rule con dùng child-selector (`.dc-prod-card > a > div:last-child ...`). `shop-giay-dep` bento thẻ đầu (`grid-row: span 2`, thumb `height:100%`) → đổi thumb sang `flex:1 1 auto; height:auto`. Verify: build `tsc -b && vite build` pass (giay-dep/do-choi/trang-suc/my-pham), Playwright screenshot xác nhận giá canh thẳng khi tên 1 vs 3 dòng.

**Checklist mới trong `.claude/agents/shop-template-builder.md`** (mục Checklist bổ sung) — template shop mới bắt buộc làm đúng pattern này từ đầu.

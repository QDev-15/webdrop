...
*Cập nhật lần cuối: 2026-07-06 — `shop-ban-hang` admin Cài đặt → Thanh toán: verify kết nối SePay THẬT khi lưu. Thêm setting `sepay_api_token` (lấy từ my.sepay.vn → Cài đặt công ty → API Access) + `SettingsController::verifySepayConnection()` gọi `GET https://my.sepay.vn/userapi/bankaccounts/list` (Authorization: Bearer token, API docs: docs.sepay.vn) để xác nhận API Token hợp lệ và số tài khoản/mã ngân hàng cấu hình khớp với tài khoản đã liên kết trên SePay — nếu không (token sai, tài khoản không khớp, không kết nối được, SePay rate-limit) thì trả lỗi 422 và KHÔNG lưu. Chỉ verify khi bật SePay từ tắt→bật hoặc khi đổi token/mã NH/số TK (không gọi API lại mỗi lần save không liên quan, tránh phụ thuộc ngoại vi không cần thiết). Nhân tiện fix bug `Settings.tsx` nuốt lỗi im lặng (`catch { /* ignore */ }`) khiến admin không thấy lý do lưu thất bại — nay hiển thị qua `form-error-banner`. PHP toàn bộ OK, TS admin 56 modules 0 lỗi.*

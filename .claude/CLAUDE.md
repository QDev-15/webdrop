...
8. Đối với các task nặng mà Claude Code tự động tạo agent chạy nền (background agent): mặc định các agent chạy nền đó phải chạy ở model Claude Haiku 4.5.
---
...
**Batch 5 template `shop-catalog-builder` (2026-07-27/28) — thêm vào `prisma/seed.ts`:** `shop-my-pham` (`hasWebsite: true` — có WebDeploy đầy đủ, xem block riêng), `shop-the-thao`/`shop-do-choi`/`shop-do-gia-dung`/`shop-van-phong-pham` (template-only, không set `hasWebsite`). Đã chạy `db:seed`. Tổng template: 58 (57 web + 1 admin).
...
### Giỏ hàng & Checkout nhiều sản phẩm (2026-07-28)

- **`CartContext`** (`src/contexts/CartContext.tsx`, client, `CartProvider` bọc `{children}` trong `app/layout.tsx`) — state giỏ hàng lưu `localStorage` (key `wd_cart_v1`), hydrate sau mount để tránh SSR mismatch (`hydrated` flag — mọi nơi đọc `items`/`itemCount` trước khi hydrate xong phải tự xử lý trạng thái rỗng tạm thời, không coi là giỏ hàng thật sự trống). Mỗi item: `{slug, name, image, price, websitePrice?, hasWebsite, purchaseType: 'template'|'website'}` — `addItem()` không thêm trùng slug (đã có thì bỏ qua, không tăng số lượng — sản phẩm số hóa không có khái niệm "mua 2 bản giống nhau"). Icon giỏ hàng + badge số lượng trong `NavBar.tsx` (`.nav-cart-btn`/`.nav-cart-badge`), trỏ `/gio-hang`.
- **Trang `/gio-hang`** (`app/(site)/gio-hang/page.tsx`) — danh sách item, đổi loại template/website (chỉ hiện toggle nếu `hasWebsite && websitePrice`), xóa item, tổng tiền, nút "Tiến hành thanh toán" → `/checkout` (không kèm `?slug=`).
- **`CheckoutClient.tsx`** giờ có 2 nhánh tách biệt hoàn toàn: có `?slug=` → luồng mua ngay 1 sản phẩm CŨ (giữ nguyên 100%, không đổi 1 dòng logic); không có `slug` → return sớm sang **`CartCheckoutClient.tsx`** (file mới, 2 bước Thông tin→Thanh toán thay vì 3 bước vì loại sản phẩm đã chọn sẵn theo từng item ở trang giỏ hàng). Cả 2 dùng chung `/api/discounts/validate` và style class có sẵn (`.panel`, `.checkout-summary`, `.btn-primary-wd`...).

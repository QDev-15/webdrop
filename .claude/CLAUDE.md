...
*Cập nhật lần cuối: 2026-07-06 — fix 2 bug nghiêm trọng `shop-ban-hang`, cả 2 đã verify trực tiếp bằng cách chạy thật site (PHP built-in server + Vite dev server + Playwright headless browser + curl), không chỉ đọc code tĩnh:*
...
- DB seed: 4 product categories, 12 products (hữu cơ/thủ công, có colors/rating), 5 testimonials, 50+ settings keys (thêm nhóm `payment`, `design`)

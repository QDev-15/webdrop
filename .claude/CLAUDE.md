...
- [x] Shop bán hàng — **DONE** (11 templates — `shop-ban-hang/` ORGANIC-EARTH, `shop-thoi-trang/` BOLD-EDITORIAL, `shop-giay-dep/` DARK-ENERGY, `shop-quan-ao/` SOFT-PASTEL, `shop-rau-xanh/` WARM-ARTISAN, `shop-thuc-pham-sach/` FRESH-MINIMAL, `shop-tui-sach/` LUXE-DARK, `shop-may-tinh/` GLASS-MODERN, `shop-may-anh/` GEOMETRIC-MODERN, `shop-ami-mobile/` RETRO-BOLD (10 có WebDeploy đầy đủ) + `shop-quan-ao-ami/` ZEN-MINIMAL (template-only, shop-catalog-builder) — xem bảng **WebDeploy Projects**)
...
│       ├── Shops/                  ← ✅ 11 shop templates (xem bảng tra nhanh cho chi tiết từng site)
...
│       │   └── shop-quan-ao-ami/   (ZEN-MINIMAL, template-only, css prefix am-, shop-catalog-builder)

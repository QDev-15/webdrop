...
- **Related posts** (`blog/[slug]/page.tsx`): `getRelatedPosts(excludeId, categoryId)` lấy tối đa 6 bài cùng category (loại trừ bài hiện tại), nếu thiếu thì lấy thêm bài mới nhất **bất kỳ category nào** để đủ 6 — luôn có nội dung để hiển thị kể cả category chỉ có 1-2 bài. Biến `fromDb: boolean` phân biệt bài lấy từ DB thật hay bài mock fallback (khi DB offline) — chỉ query related posts + gọi Unsplash khi `fromDb=true`, không đụng DB ở nhánh mock.
...
- **⚠️ Phát hiện qua reviewer, KHÔNG thuộc phạm vi thay đổi hôm nay (xác nhận qua `git diff` — code có từ trước)**: hàm `renderMarkdown()` trong `blog/[slug]/page.tsx` dùng `dangerouslySetInnerHTML` chỉ escape `**bold**`, phần còn lại của `post.content` (nhập từ Admin Post editor) được inject nguyên văn — nếu tài khoản có quyền tạo bài viết (`user` role) nhập `<script>`/`<img onerror=...>` vào content sẽ chạy được trên trang public cho MỌI khách truy cập (stored XSS). Rủi ro thực tế phụ thuộc mức tin cậy của tài khoản `user` role hiện tại — báo cáo để chủ dự án quyết định có cần sanitize (vd DOMPurify) hay không, chưa tự sửa vì ngoài phạm vi task.
